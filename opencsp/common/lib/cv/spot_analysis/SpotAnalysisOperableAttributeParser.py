...
class SpotAnalysisOperableAttributeParser(iap.ImageAttributeParser):
    def __init__(self, operable: sao.SpotAnalysisOperable = None, spot_analysis=None):
        ...
        current_image_source = tt.default(
            lambda: operable.primary_image.source_path, None
        )
        ...
        self.image_processors: list[str] = [
            processor.name for processor in image_processors
        ]

        # retrieve any available attributes from the associated attributes file
        if self._previous_attr != None:
            self.set_defaults(
                self._previous_attr.get_parser(SpotAnalysisOperableAttributeParser)
            )
    ...
    def set_defaults(self, other: 'SpotAnalysisOperableAttributeParser'):
        ...
        self.image_processors = tt.default(
            self.image_processors, other.image_processors
        )
