...
class PopulationStatisticsImageProcessor(AbstractSpotAnalysisImagesProcessor):
    def __init__(self, min_pop_size = 1, target_rolling_window_size = 1):
        ...
        if min_pop_size > 0:
            if target_rolling_window_size > min_pop_size:
                lt.warn("Warning in PopulationStatisticsImageProcessor: " + \
    ...
    def _calculate_rolling_window(self, curr_stats: SpotAnalysisPopulationStatistics, operable: SpotAnalysisOperable, window: list[SpotAnalysisOperable]):
        ...
        if not found_operable:
            ...
            self._rolling_window_stats.append(_RollingWindowOperableStats(
                operable,
                sum_per_color,
                dims
            ))
    ...
    def _execute(self, operable: SpotAnalysisOperable, is_last: bool) -> list[SpotAnalysisOperable]:
        ...
        if self.curr_stats == None:
            ...
            for prior_operable in self.initial_operables:
                self.curr_stats = self._calculate_rolling_window(self.curr_stats, prior_operable, self.rolling_window_operables)
