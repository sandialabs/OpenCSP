...
class TestView3dImageProcessor(unittest.TestCase):
    @classmethod
    ...
    def test_transform_coordinates_tblocator(self):
        ...
        for result in sa:
            for i, visualization_image in enumerate(result.visualization_images[processors["v3d"]]):
        ...
        csproc: View3dImageProcessor = processors["v3d"]
        csproc.close_figures()

    @unittest.skip(skip_msg)
    def test_transform_coordinates_crop(self):
        processors = {"cropping": CroppingImageProcessor.by_region((100, 500, 200, 400)), "v3d": View3dImageProcessor()}
