...
from opencsp.common.lib.cv.spot_analysis.image_processor.AbstractSpotAnalysisImageProcessorLeger import AbstractSpotAnalysisImagesProcessorLeger
...
class AbstractSpotAnalysisImagesProcessor(Iterator[SpotAnalysisOperable], AbstractSpotAnalysisImagesProcessorLeger):
    ...
    def run(self, operables: ImagesIterable|ImagesStream | SpotAnalysisImagesStream | Union['AbstractSpotAnalysisImagesProcessor',list[SpotAnalysisOperable],Iterator[SpotAnalysisOperable]]) -> list[CacheableImage]:
    ...
    def process_image(self, input_operable: SpotAnalysisOperable, is_last: bool=False) -> list[SpotAnalysisOperable]:
        ...
        if not isinstance(ret, list):
            lt.error_and_raise(TypeError, f"Error in AbstractSpotAnalysisImageProcessor.process_image() ({self.name}): " + \
    ...
    def __iter__(self):
        if self.all_processed_results != None:
            ...
            if not self.finished_processing:
                lt.error_and_raise(RuntimeError, f"Programmer error in AbstractSpotAnalaysisImageProcess.__iter__: " + \
    ...
    def __next__(self):
        ...
        while not is_last:
            ...
            if input_operable == None:
                lt.error_and_raise(RuntimeError, f"Programmer error in AbstractSpotAnalysisImageProcessor.__next__() ({self.name}): " + 
        ...
        if len(output_operables) == 0:
            if not is_last:
                lt.error_and_raise(RuntimeError, f"Programmer error in SpotAnalysisAbstractImagesProcessor.__next__() ({self.name}): " + \
    ...
    @tt.strict_types
    def get_processed_image_save_callback(self, dir: str, name_prefix: str=None, ext="jpg") -> Callable[[SpotAnalysisOperable], str]:
