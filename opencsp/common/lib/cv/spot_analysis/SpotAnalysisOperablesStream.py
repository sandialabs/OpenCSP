...
class SpotAnalysisOperablesStream(Iterator[SpotAnalysisOperable]):
    ...
    def __next_operable(self):
        ...
        if isinstance(val, SpotAnalysisOperable):
            ...
            return SpotAnalysisOperable(
                primary_image, supporting_images=supporting_images
            )
        else:
            primary_image = val
    ...
    def __next__(self):
        ...
        operable = operable.replace_use_default_values(
            self.default_support_images, self.default_data
        )
        return operable
