...
class CacheableImage():
    def __init__(self, array: np.ndarray=None, cache_path: str=None, source_path: str=None):
        ...
        if array is None and cache_path == None and source_path == None:
            lt.error_and_raise(ValueError, "Error in CacheableImage.__init__(): must provide one of array, cache_path, or source_path!")
    ...
    def from_single_source(cls, array_or_path: Union[np.ndarray,str,'CacheableImage']):
        ...
        if isinstance(array_or_path, CacheableImage):
            ...
            lt.error_and_raise(TypeError, f"Error in CacheableImage.from_single_source(): unexpected type {type(array_or_path)}")
    ...
    def validate_cache_path(self, cache_path: Optional[str], caller_name: str):
        ...
        if not cache_path.lower().endswith(".npy"):
            ...
            lt.error_and_raise(ValueError, f"Error in CacheableImage.{caller_name}(): cache_path must end with '.npy' but instead the extension is {ext}")
    ...
    def __load_image(self):
        if not self._array is None:
            ...
            lt.error_and_raise(RuntimeError, f"Error in CacheableImage.__load_image(): Can't load image! {self._array=}, {self.cache_path=}, {self.source_path=}")
    ...
    def cache(self, cache_path: str=None):
        ...
        if cache_path == None:
            if self.cache_path == None:
                lt.error_and_raise(ValueError, "Error in CacheableImage.cache(): " + \
