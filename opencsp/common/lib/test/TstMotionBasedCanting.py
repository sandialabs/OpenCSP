...
class TestMotionBasedCanting(to.TestOutput):
    ...
    def test_9W1_heliostat(self) -> None:
        ...
        intersection_points = intersection.plane_lines_intersection(
            (facet_origins, facet_normals), (tower.target_loc, Uxyz([0, 1, 0]))
        )
    ...
    def test_all_facets(self) -> None:
        ...
        for f_index, f_index in enumerate(heliostat.facet_dict):
            ...
            # style setup
            surface_normal_facet_style = rch.normal_facet_outlines()
            surface_normal_facet_style.facet_styles.add_special_names(
                [int(f_index)], rcf.normal_outline_long(color='b')
            )
            solar_field_style = rcsf.heliostat_blanks()
            solar_field_style.heliostat_styles.add_special_names(test_heliostat, surface_normal_facet_style)
            tower_control = rct.normal_tower()
