...
def svg_lines_are_equal(line_1:str, line_2:str) -> bool:
    ...
    if line_1 == line_2:
        ...
        return False
    # svg string analysis.
    if lines_share_an_ignore_string(line_1, line_2, 
                                    ['<dc:date>',
                                     'clip-path',
                                     'clipPath',
                                     'path id',
...
def verify_output_file_matches_expected(file_created:str, actual_output_dir:str, expected_output_dir:str) -> None:
    ...
    if not ft.file_exists(actual_dir_body_ext):
        lt.error_and_raise(FileNotFoundError,
...
def verify_output_files_match_expected(files_created:list[str], actual_output_dir:str, expected_output_dir:str) -> None:
    """
...
def show_save_and_check_figure(fig_record:RenderControlFigureRecord, 
    ...
    # Show.
    if show_figs:
        fig_record.view.show(equal=fig_record.equal, x_limits=fig_record.x_limits, y_limits=fig_record.y_limits, z_limits=fig_record.z_limits)
    # Save.
...
def save_and_check_image(image,  # ?? SCAFFOLDING RCB -- ADD TYPE CUE
    ...
    """
    # Save.
    file_created = ti.save_image(image, output_dpm, actual_output_dir, output_file_body, output_ext)  # ?? SCAFFOLDING RCB -- REPLACE WITH GENERAL IMAGE SAVE ROUTINE
...
def compare_actual_expected_images(actual_location: str,
...
def load_solar_field_partition(heliostat_names: list, partitioned_csv_file_name: str) -> sf.SolarField:
    ...
    return sf.sf_from_csv_files(name='Sandia NSTTF Partition',
                                short_name='NSTTF',
