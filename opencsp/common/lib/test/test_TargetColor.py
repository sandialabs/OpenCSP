...
class TestTargetColor(to.TestOutput):
    ...
    def setup_class(self,
                    ...
                    verify:bool=True):
        # Generic setup.
        super(TestTargetColor, self).setup_class(source_file_body=source_file_body,
...
# self.image_height_in = 5.0 # 96.0 # inch
        ...
        # For debugging.
        self.image_width_in  = 2.0 # 48.0 # inch
        self.image_height_in = 2.0 # 96.0 # inch
        self.dpi = 5 #100 # 100
    ...
    def execute_test_linear_color_bar(self, color_below_min:Color, color_bar, color_bar_name:str, color_above_max:Color, generate_all:bool) -> None:

        if generate_all:
            # Linear color bar in x, continuous.
            target = tc.construct_target_linear_color_bar(self.image_width, self.image_height, self.dpm,
                                                        color_below_min, color_bar, color_bar_name, color_above_max, 'x', 'continuous')
            self.save_and_check_image(target.image, self.dpm, target.description_inch(), '.png')

        # Linear color bar in y, discrete.
        target = tc.construct_target_linear_color_bar(self.image_width, self.image_height, self.dpm,
                                                    color_below_min, color_bar, color_bar_name, color_above_max, 'y', 'discrete')
        self.save_and_check_image(target.image, self.dpm, target.description_inch(), '.png')


    # ?? SCAFFOLDING RCB -- ADD COLOR_BAR TYPE TIP BELOW
    def execute_test_polar_color_bar(self, color_below_min:Color, color_bar, color_bar_name:str, color_above_max:Color) -> None:
        # Default.
        ...
        #                                                  pattern_boundary='image_boundary',
        #                                                  radial_gradient_type='saturated_center_to_white',
        #                                                  radial_gradient_name='s2w',
        #                                                  saturated_center_to_white_exponent=exponent)
        #     self.save_and_check_image(target.image, self.dpm, target.description_inch(), '.png')
        #     # Continuous.
        #     target = tc.construct_target_polar_color_bar(self.image_width, self.image_height, self.dpm, 
        #                                                  discrete_or_continuous='continuous',
        ...
        # # Light center to saturated boundary, varying level of center saturation.
        # for min_saturation in np.arange(0, 1.0, 0.1):
        #     # Discrete.
        #     target = tc.construct_target_polar_color_bar(self.image_width, self.image_height, self.dpm, 
        #                                                  discrete_or_continuous='discrete',
        #                                                  pattern_boundary='image_boundary',
        #                                                  radial_gradient_type='light_center_to_saturated',
        #                                                  radial_gradient_name='l2s',
        #                                                  light_center_to_saturated_saturation_min=min_saturation)
        #     self.save_and_check_image(target.image, self.dpm, target.description_inch(), '.png')
        #     # Continuous.
        #     target = tc.construct_target_polar_color_bar(self.image_width, self.image_height, self.dpm, 
        #                                                  discrete_or_continuous='continuous',
    ...
    def execute_test_rgb_cube_inscribed_square(self, project_to_cube:bool) -> None:
        # Construct target.
        target = tc.construct_target_rgb_cube_inscribed_square(self.image_width, self.image_height, self.dpm, project_to_cube)
        self.save_and_check_image(target.image, self.dpm, target.description_inch(), '.png')


    # TARGET MODIFICATION TESTS

    def execute_test_adjust_color_saturation(self, saturation_fraction:float) -> None:
        # Construct target.
        target = tc.construct_target_blue_under_red_cross_green(self.image_width, self.image_height, self.dpm)
    ...
    def execute_test_extend_target(self) -> None:
        # Target.
        target = tc.construct_target_blue_under_red_cross_green(self.image_width, self.image_height, self.dpm)
        # Border all around.
        ...
        self.save_and_check_image(extended_target_left_right_top_bottom_border.image, self.dpm, extended_target_left_right_top_bottom_border.description_inch(), '.png')


    # TARGET COMBINATION TESTS

    def execute_test_splice_targets_above_below(self) -> None:
        ...
        target_2 = tc.construct_target_polar_color_bar(self.image_width, self.image_height, self.dpm, 
                                                       discrete_or_continuous='continuous',
                                                       pattern_boundary='image_boundary',
                                                       radial_gradient_type='light_center_to_saturated',
                                                       radial_gradient_name='l2s',
    ...
    def execute_test_cascade_target_A(self) -> None:
        ...
        cascade_target = tc.construct_linear_color_bar_cascade(# Dimensions.
                                                               ...
                                                               list_of_discrete_or_continuous_lists=[['continuous'], 
                                                                                                     # 2-stack
                                                                                                     ['continuous',
                                                                                                      'continuous'],
                                                                                                     # 5-stack
                                                                                                     ...
                                                                                                     ['discrete',
                                                                                                      ...
                                                                                                      'continuous',
                                                                                                      'continuous']],
                                                               list_of_saturation_spec_lists=[ [[None,None,None,None]],
                                                                                               # 2-stack
                                                                                               [['light_to_saturated',None,0.4,1.0], 
                                                                                                ['saturated_to_white',1.25,None,None]],
                                                                                               # 5-stack
                                                                                               [[None,None,None,None],
                                                               ...
                                                               # Space between bars.
                                                               gap_between_bars_pix=round(gap_between_bars*composite_dpm),  # Pixels  # ?? SCAFFOLDING RCB -- SHOULD THIS BE IN INCHES?
...
#         self.execute_test_linear_color_bar(color_below_min, color_bar, color_bar_name, color_above_max, True)
    ...
    def test_corner_tour_closed_equal_angle(self) -> None:
        # Initialize test.
        self.start_test()
...
# color_bar_name = 'O'  # ?? SCAFFOLDING RCB -- THIS SHOULD BE A CLASS MEMBER
# color_above_max = Color.white()  # White background for "saturated data."
# self.execute_test_linear_color_bar(color_below_min, color_bar, color_bar_name, color_above_max)

    def test_polar_color_bar(self) -> None:
        # Initialize test.
        self.start_test()
    ...
    def test_blue_under_red_cross_green(self) -> None:
        ...
        self.execute_test_blue_under_red_cross_green()


    def test_rgb_cube_inscribed_square_False(self) -> None:
        # Initialize test.
        self.start_test()
        # Square inscribed in the [R,G,B] space basis vector hexagon.
        self.execute_test_rgb_cube_inscribed_square(False)


    def test_rgb_cube_inscribed_square_True(self) -> None:
        # Initialize test.
        self.start_test()
        # Square inscribed in the [R,G,B] space basis vector hexagon, with rays projected to cube surface.
        self.execute_test_rgb_cube_inscribed_square(True)


        # # Adjust color saturation.
        # self.execute_test_adjust_color_saturation(0.0)  # saturation_fraction
        # self.execute_test_adjust_color_saturation(0.1)  # saturation_fraction
        # self.execute_test_adjust_color_saturation(0.2)  # saturation_fraction
        # self.execute_test_adjust_color_saturation(0.3)  # saturation_fraction
        # self.execute_test_adjust_color_saturation(0.4)  # saturation_fraction
        # self.execute_test_adjust_color_saturation(0.5)  # saturation_fraction
        # self.execute_test_adjust_color_saturation(0.6)  # saturation_fraction
        # self.execute_test_adjust_color_saturation(0.7)  # saturation_fraction
        # self.execute_test_adjust_color_saturation(0.8)  # saturation_fraction
        # self.execute_test_adjust_color_saturation(0.9)  # saturation_fraction
        # self.execute_test_adjust_color_saturation(1.0)  # saturation_fraction
        # self.execute_test_adjust_color_saturation(1.1)  # saturation_fraction
        # self.execute_test_adjust_color_saturation(1.2)  # saturation_fraction
        # self.execute_test_adjust_color_saturation(1.3)  # saturation_fraction
        # self.execute_test_adjust_color_saturation(1.4)  # saturation_fraction
        # self.execute_test_adjust_color_saturation(1.5)  # saturation_fraction
        # self.execute_test_adjust_color_saturation(1.6)  # saturation_fraction
        # self.execute_test_adjust_color_saturation(1.7)  # saturation_fraction
        # self.execute_test_adjust_color_saturation(1.8)  # saturation_fraction
        # self.execute_test_adjust_color_saturation(1.9)  # saturation_fraction
        # self.execute_test_adjust_color_saturation(2.0)  # saturation_fraction
        # self.execute_test_adjust_color_saturation(2.1)  # saturation_fraction
        # self.execute_test_adjust_color_saturation(2.2)  # saturation_fraction


    def test_extend_target(self) -> None:
        # Initialize test.
        self.start_test()
        # Add border pixels to a target.
        self.execute_test_extend_target()


    def test_splice_targets_above_below(self) -> None:
        # Initialize test.
        self.start_test()
        # Combine squares into a single image.
        self.execute_test_splice_targets_above_below()


    def test_cascade_target_A(self) -> None:
        # Initialize test.
        self.start_test()
        # Composite image #1.
        self.execute_test_cascade_target_A()
...
if __name__ == "__main__":
    ...
    test_object.setup_class(interactive=interactive,
                             verify=verify)
    # Tests.
    ...
    test_object.test_polar_color_bar()
    test_object.test_blue_under_red_cross_green()
    test_object.test_rgb_cube_inscribed_square_False()
    test_object.test_rgb_cube_inscribed_square_True()
