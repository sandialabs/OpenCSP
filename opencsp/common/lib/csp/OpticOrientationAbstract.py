...
class OpticOrientationAbstract(ABC):
    ...
    def add_child(self, new_child: 'OpticOrientationAbstract',
                  new_child_to_self_transform=TransformXYZ.identity()):
        if new_child.parent is not None:
            raise ValueError("Cannot add a child optic if that child is already owned by a parent: \n"
    ...
    def get_transform_relative_to(self: 'OpticOrientationAbstract',
                                  target: 'OpticOrientationAbstract') -> TransformXYZ:
        """
        ...
        raise ValueError("target is not an in the parent-child"
                         " tree of self.")
