...
def standard_output(optic_meas: MirrorAbstract,
    ...
    image_meas, xv_meas, yv_meas = rt.histogram_image(bin_res=vis_options.hist_bin_res, extent=vis_options.hist_extent, pts=ray_pts_meas)
    ...
    if plot_reference:
        ...
        image_ref, xv_ref, yv_ref = rt.histogram_image(bin_res=vis_options.hist_bin_res, extent=vis_options.hist_extent, pts=ray_pts_ref)
    ...
    optic_meas.plot_orthorectified_slope(vis_options.slope_map_resolution, type_='magnitude',
                                         quiver_density=vis_options.slope_map_quiver_density,
                                         clim=vis_options.slope_clim, axis=fig_rec.axis)
    if vis_options.to_save:
        fig_rec.save(vis_options.output_dir, dpi=vis_options.save_dpi, format='png', close_after_save=vis_options.close_after_save)

    fig_rec = fm.setup_figure(fig_control, axis_control, name="Measured X Slope")
    optic_meas.plot_orthorectified_slope(vis_options.slope_map_resolution, type_='x',
                                         quiver_density=vis_options.slope_map_quiver_density,
                                         clim=vis_options.slope_clim, axis=fig_rec.axis)
    if vis_options.to_save:
        fig_rec.save(vis_options.output_dir, dpi=vis_options.save_dpi, format='png', close_after_save=vis_options.close_after_save)
    ...
    optic_meas.plot_orthorectified_slope(vis_options.slope_map_resolution, type_='y',
                                         quiver_density=vis_options.slope_map_quiver_density,
                                         clim=vis_options.slope_clim, axis=fig_rec.axis)
    if vis_options.to_save:
        fig_rec.save(vis_options.output_dir, dpi=vis_options.save_dpi, format='png', close_after_save=vis_options.close_after_save)
    ...
    optic_meas.plot_orthorectified_curvature(vis_options.slope_map_resolution, type_='x',
                                             clim=vis_options.curvature_clim, axis=fig_rec.axis)
    if vis_options.to_save:
        fig_rec.save(vis_options.output_dir, dpi=vis_options.save_dpi, format='png', close_after_save=vis_options.close_after_save)

    fig_rec = fm.setup_figure(fig_control, axis_control, name='Curvature Y')
    optic_meas.plot_orthorectified_curvature(vis_options.slope_map_resolution, type_='y',
                                             clim=vis_options.curvature_clim, axis=fig_rec.axis)
    if vis_options.to_save:
        fig_rec.save(vis_options.output_dir, dpi=vis_options.save_dpi, format='png', close_after_save=vis_options.close_after_save)
    ...
    if plot_reference:
        ...
        optic_meas.plot_orthorectified_slope_error(optic_ref, vis_options.slope_map_resolution, type_='magnitude',
                                                   quiver_density=vis_options.slope_map_quiver_density,
                                                   clim=vis_options.slope_error_clim, axis=fig_rec.axis)
        if vis_options.to_save:
            fig_rec.save(vis_options.output_dir, dpi=vis_options.save_dpi, format='png', close_after_save=vis_options.close_after_save)

        fig_rec = fm.setup_figure(fig_control, axis_control, name="X Slope Error")
        optic_meas.plot_orthorectified_slope_error(optic_ref, vis_options.slope_map_resolution, type_='x',
                                                   quiver_density=vis_options.slope_map_quiver_density,
                                                   clim=vis_options.slope_error_clim, axis=fig_rec.axis)
        if vis_options.to_save:
            fig_rec.save(vis_options.output_dir, dpi=vis_options.save_dpi, format='png', close_after_save=vis_options.close_after_save)
        ...
        optic_meas.plot_orthorectified_slope_error(optic_ref, vis_options.slope_map_resolution, type_='y',
                                                   quiver_density=vis_options.slope_map_quiver_density,
                                                   clim=vis_options.slope_error_clim, axis=fig_rec.axis)
        if vis_options.to_save:
            fig_rec.save(vis_options.output_dir, dpi=vis_options.save_dpi, format='png', close_after_save=vis_options.close_after_save)
