...
class Heliostat(RayTraceable):
    ...
    def __init__(self,
                 ...
                 use_center_facet_for_aiming: bool = False):  # maybe make this default to true
        ...
        self.facet_dict = {f.name: f_i for f_i, f in enumerate(facets)}  # TODO tjlarki: check with RCB that this is what we want
    ...
    def compute_tracking_configuration(self,
        ...
        if self.use_center_facet_for_aiming:
            ...
            for _ in range(10):
                n = Vxyz(st.tracking_surface_normal_xyz((h + n * d).data.T.flatten(), aimpoint_xyz, location_lon_lat, when_ymdhmsz)).normalize()
    ...
    @strict_types
    def compute_tracking_configuration_from_sun_vector(self,
                                                       aimpoint_xyz: list | np.ndarray,
                                                       sun_vector: Vxyz,) -> hc.HeliostatConfiguration:
        """ !!! DOES NOT WORK !!!
        ...
        # take into account the offset to the center facet
        for _ in range(10):
            n = Vxyz(st.tracking_surface_normal_xyz_given_sun_vector((h + n * d).data.T.flatten(), aimpoint_xyz, sun_vector)).normalize()
    ...
    def set_off_axis_canting(self, long_lat: tuple, aimpoint: Pxyz, time_ymdhmsz: tuple):
        ...
        for facet in self.facets:
            # facet_aiming_direction = up_vector.rotate(facet.composite_rotation)
            vector_the_normal_should_be = Vxyz(st.tracking_surface_normal_xyz(facet.origin,
                                                                              aimpoint_xyz,
                                                                              long_lat,
                                                                              time_ymdhmsz))
            R1_prime = heliostat_aiming_direction.align_to(vector_the_normal_should_be)
    ...
    def rodrigues_vector_from_partial_derivatives(self, dfdx_n: float, dfdy_n: float) -> np.ndarray:
        ...
        if ((dfdx_n == 0) and (dfdy_n == 0)):
            ...
            return np.array([0., 0., 0.])  # Zero magnitude implies zero rotation; direction doesn't matter.

        else:
            # There is a rotation.
            x_vec = [1., 0., float(dfdx_n)]  # direction of x partial derivative
            y_vec = [0., 1., float(dfdy_n)]  # direction of y partial derivative
            ...
            v_rot = np.cross(u, np.array([0., 0., 1.]))  # axis of rotation
            ...
            theta_rot = -mt.robust_arccos(np.dot(u, np.array([0., 0., 1.])))  # angle of rotation
