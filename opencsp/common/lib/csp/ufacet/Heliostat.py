...
class Heliostat(RayTraceable):
    ...
    def __init__(
        ...
        self.bottom_left_facet: Facet = self.facets[
            self.facet_dict[str(num_facets - num_cols + 1)]
        ]
        ...
        self.top_left_corner_offset = [
            x + y
            for x, y in zip(
                self.top_left_facet.centroid_offset,
                self.top_left_facet.top_left_corner_offset,
            )
        ]
        self.top_right_corner_offset = [
            x + y
            for x, y in zip(
                self.top_right_facet.centroid_offset,
                self.top_right_facet.top_right_corner_offset,
            )
        ]
        self.bottom_right_corner_offset = [
            x + y
            for x, y in zip(
                self.bottom_right_facet.centroid_offset,
                self.bottom_right_facet.bottom_right_corner_offset,
            )
        ]
        self.bottom_left_corner_offset = [
            x + y
            for x, y in zip(
                self.bottom_left_facet.centroid_offset,
                self.bottom_left_facet.bottom_left_corner_offset,
            )
        ]
        ...
        )  # Origin is at torque tube center.

        self.az = np.deg2rad(
            180
        )  # (az,el) = (180,90) degrees corresponds to pointing straight up,
        self.el = np.deg2rad(
            90
        )  # as if transitioned by tilting up from face south orientation.
    ...
    def aimpoint_xyz(self):
        if self._aimpoint_xyz == None:
            print(
                'ERROR: In Heliostat.aimpoint_xyz(), attempt to fetch unset _aimpoint_xyz.'
            )
            assert False
    ...
    def when_ymdhmsz(self):
        if self._when_ymdhmsz == None:
            print(
                'ERROR: In Heliostat.when_ymdhmsz(), attempt to fetch unset _when_ymdhmsz.'
            )
            assert False
    ...
    def compute_tracking_configuration(
        self,
        aimpoint_xyz: list | np.ndarray,
        location_lon_lat: tuple | list,
        when_ymdhmsz: tuple,
    ):
        if self.use_center_facet_for_aiming:
            if self.center_facet == None:
                raise AttributeError(
                    f"Helisotat (Name: {self.name}) does not have a center facet defined"
                )
            ...
            n = Vxyz(
                st.tracking_surface_normal_xyz(
                    h_tube, aimpoint_xyz, location_lon_lat, when_ymdhmsz
                )
            ).normalize()
            ...
            return hc.heliostat_configuration_given_surface_normal_xyz(
                n.data.T.flatten()
            )
        else:
            # Heliostat centroid coordinates.
            ...
            # Compute heliostat surface normal which tracks the sun to the aimpoint.
            n_xyz = st.tracking_surface_normal_xyz(
                h, aimpoint_xyz, location_lon_lat, when_ymdhmsz
            )
    ...
    def compute_tracking_configuration_from_sun_vector(
        ...
        # Compute heliostat surface normal which tracks the sun to the aimpoint.
        n = Vxyz(
            st.tracking_surface_normal_xyz_given_sun_vector(
                h_tube, aimpoint_xyz, sun_vector
            )
        ).normalize()
        ...
        for _ in range(10):
            n = Vxyz(
                st.tracking_surface_normal_xyz_given_sun_vector(
                    (h + n * d).data.T.flatten(), aimpoint_xyz, sun_vector
                )
            ).normalize()
    ...
    def compute_stow_configuration(self):
        ...
        if NSTTF:
            azimuth = np.deg2rad(
                270.0
            )  # ?? SCAFFOLDING RCB -- FIND OUT THE CORRECT NUMBER FOR THIS.
    ...
    def set_canting_from_equation(
        self, func: Callable[[float, float], float], move_centriods: bool = False
    ) -> None:
        """
    ...
    def set_off_axis_canting(
        self, long_lat: tuple, aimpoint: Pxyz, time_ymdhmsz: tuple
    ):
        # we need the Rotation R1 (canting rotation)
        # we will call the heliostat roation R2
        # R2 * R1 = R (total rotation)
        ...
        for facet in self.facets:
            ...
            vector_the_normal_should_be = Vxyz(
                st.tracking_surface_normal_xyz(
                    facet.origin, aimpoint_xyz, long_lat, time_ymdhmsz
                )
            )
            R1_prime = heliostat_aiming_direction.align_to(vector_the_normal_should_be)
            facet.canting = copy.deepcopy(R2_inv * R1_prime * R2)
    ...
    def rodrigues_vector_from_partial_derivatives(
        self, dfdx_n: float, dfdy_n: float
    ) -> np.ndarray:
        """
        ...
        if (dfdx_n == 0) and (dfdy_n == 0):
            ...
            return np.array(
                [0.0, 0.0, 0.0]
            )  # Zero magnitude implies zero rotation; direction doesn't matter.
            ...
            if v_norm == 0.0:
                ...
                print(
                    'ERROR: In Heliostat.set_canting_from_equation(), encountered unexpected zero v_norm.'
                )
                print('         x_vec = ', x_vec)
                print('         y_vec = ', y_vec)
                assert False
            ...
            if v_rot_norm == 0.0:
                # TODO RCB: REPLACE THIS WITH LOG/EXCEPTION THROW.
                print(
                    'ERROR: In Heliostat.set_canting_from_equation(), encountered unexpected zero v_rot_norm.'
                )
                print('         u = ', u)
                print('         v_rot = ', v_rot)
                assert False
            ...
            theta_rot = -mt.robust_arccos(
                np.dot(u, np.array([0.0, 0.0, 1.0]))
            )  # angle of rotation
    ...
    def set_tracking(self, aimpoint_xyz, location_lon_lat, when_ymdhmsz):
        ...
        if len(location_lon_lat) != 2:
            raise ValueError(
                f'{location_lon_lat} must be of length 2. (Lattitude, Longitude)'
            )
        # Save tracking command.
        self._aimpoint_xyz = aimpoint_xyz
        self._when_ymdhmsz = when_ymdhmsz
        # Set tracking configuration.
        h_config = self.compute_tracking_configuration(
            aimpoint_xyz, location_lon_lat, when_ymdhmsz
        )
    ...
    def set_configuration(
        self, h_config: hc.HeliostatConfiguration, clear_tracking=True
    ):
        ...
        origin = (
            np.array(self.origin) + vector_offset
        )  # Origin is at torque tube center.

        self.surface_normal = hel_rotation.apply(
            [0, 0, 1]
        )  # Before rotation, heliostat is face up.
    ...
    def update_position_in_space(
        self,
    ):  # TODO tjlarki: should this be in the RayTraceable object?
    ...
    def draw(
        self,
        view: View3d,
        heliostat_styles: RenderControlHeliostat | RenderControlEnsemble = None,
    ):
        # Assumes that heliostat configuration has already been set.
        if heliostat_styles == None:
            heliostat_styles = RenderControlEnsemble(
                default_style=rch.mirror_surfaces()
            )
        ...
        if heliostat_style.draw_surface_normal:
            ...
            surface_normal_ray = self.surface_normal_ray(
                self.origin, heliostat_style.surface_normal_length
            )
            # Draw ray and its base.
            view.draw_xyz(self.origin, style=heliostat_style.surface_normal_base_style)
            view.draw_xyz_list(
                surface_normal_ray, style=heliostat_style.surface_normal_style
            )
        ...
        if heliostat_style.draw_surface_normal_at_corners:
            ...
            top_left_ray = self.surface_normal_ray(
                self.top_left_corner, heliostat_style.corner_normal_length
            )
            top_right_ray = self.surface_normal_ray(
                self.top_right_corner, heliostat_style.corner_normal_length
            )
            bottom_left_ray = self.surface_normal_ray(
                self.bottom_left_corner, heliostat_style.corner_normal_length
            )
            bottom_right_ray = self.surface_normal_ray(
                self.bottom_right_corner, heliostat_style.corner_normal_length
            )
    ...
    def survey_of_points(
        ...
        normals = Vxyz.empty()
        for facet in self.facets:
            additional_points, additional_normals = facet.survey_of_points(
...
def h_from_facet_centroids(
    ...
    # Facets
    facets, _ = facets_read_file(
        file_centroids_offsets, facet_height, facet_width, default_mirror_shape
    )

    return Heliostat(
        name, origin, num_facets, num_rows, num_cols, facets, pivot_height, pivot_offset
    )
