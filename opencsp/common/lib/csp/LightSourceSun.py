...
class LightSourceSun(LightSource):
    ...
    def set_incident_rays(self, loc: tuple[float, float], time: tuple, resolution: int, sun_dia: float = 0.009308, verbose=False) -> None:
        """
        ...
        warn('LightSourceSun.set_incident_rays is deprecated. Use initialize_from_solar_position instead.', DeprecationWarning, stacklevel=2)
        ...
        if resolution >= 3:
            xs = ys = np.linspace(-sun_radius, sun_radius, resolution)  # defines a square of points, corners cut off later
        elif resolution == 2:
            xs = ys = np.array([-sun_radius / 3, sun_radius / 3])
        elif resolution == 1:
            xs = ys = np.zeros(1)
