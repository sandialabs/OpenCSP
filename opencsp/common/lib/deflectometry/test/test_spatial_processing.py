...
class TestSpatialProcessing(unittest.TestCase):
    ...
    def test_t_from_distance(self):
        ...
        # Perform calculation
        v_cam_optic_cam_exp = sp.t_from_distance(Vxy(data['v_mask_centroid_image']), measurement.optic_screen_dist,
                                                 self.camera, self.display.v_cam_screen_cam).data.squeeze()
    ...
    def test_r_from_position(self):
        ...
        # Perform calculation
        r_optic_cam_exp = sp.r_from_position(Vxyz(data['v_cam_optic_cam_exp']), self.display.v_cam_screen_cam).inv().as_rotvec()
    ...
    def test_calc_rt_from_img_pts(self):
        ...
        # Perform calculation
        r_optic_cam, v_cam_optic_cam_refine = sp.calc_rt_from_img_pts(Vxy(data['loop_facet_image_refine']),
                                                                      Vxyz(data['v_facet_corners']),
                                                                      self.camera)

        # Test
        np.testing.assert_allclose(data['r_optic_cam_refine_1'], r_optic_cam.as_rotvec(), atol=1e-5, rtol=0)
        np.testing.assert_allclose(data['v_cam_optic_cam_refine_1'], v_cam_optic_cam_refine.data.squeeze(), atol=1e-5, rtol=0)
    ...
    def test_distance_error(self):
        ...
        # Perform calculation
        error_optic_screen_dist_2 = sp.distance_error(self.display.v_cam_screen_cam,
    ...
    def test_reprojection_error(self):
        ...
        # Perform calculation
        error_reproj_init = sp.reprojection_error(self.camera,
                                                  Vxyz(data['v_facet_corners']),
    ...
    def test_refine_v_distance(self):
        ...
        v_cam_optic_cam_refine_2 = sp.refine_v_distance(Vxyz(data['v_cam_optic_cam_refine_1']),
                                                        measurement.optic_screen_dist,
                                                        self.display.v_cam_screen_cam,
                                                        v_meas_pt_optic_cam).data.squeeze()
