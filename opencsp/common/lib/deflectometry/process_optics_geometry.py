...
def process_singlefacet_geometry(facet_data: FacetData,
                                 mask_raw: ndarray,
                                 ...
                                 ) -> tuple[cdc.CalculationDataGeometryGeneral,
    ...
    v_cam_optic_centroid_cam_exp = sp.t_from_distance(v_mask_centroid_image,
                                                      optic_screen_dist,
                                                      camera,
    ...
    if debug.debug_active:
        ...
        debug.figures.append(fig)
        plt.imshow(mask_raw)
        plt.scatter(*camera.project(v_cam_optic_centroid_cam_exp, Rotation.identity(), Vxyz((0, 0, 0))).data, marker='.')
    ...
    r_optic_cam_refine_1, v_cam_optic_cam_refine_1 = sp.calc_rt_from_img_pts(loop_facet_image_refine.vertices,
                                                                             v_facet_corners,
                                                                             camera)
    ...
    v_cam_optic_cam_refine_2 = sp.refine_v_distance(v_cam_optic_cam_refine_1,
                                                    optic_screen_dist,
                                                    ori.v_cam_screen_cam,
    ...
    error_optic_screen_dist_1 = sp.distance_error(ori.v_cam_screen_cam,
    ...
    error_reprojection_1 = sp.reprojection_error(camera,
                                                 v_facet_corners,
    ...
    error_optic_screen_dist_2 = sp.distance_error(ori.v_cam_screen_cam,
    ...
    error_reprojection_2 = sp.reprojection_error(camera,
                                                 v_facet_corners,
                                                 loop_facet_image_refine.vertices,
                                                 r_optic_cam_refine_1,
...
def process_undefined_geometry(mask_raw: ndarray,
    ...
    """
    if debug.debug_active:
        print('Image processing debug on, but is not yet supported for undefined mirrors.')
    ...
    v_cam_optic_cam = sp.t_from_distance(v_mask_centroid_image,
                                         optic_screen_dist,
                                         camera,
...
def process_multifacet_geometry(
    ...
    # Get facet data
    v_facet_corners_facet: list = [f.v_facet_corners for f in facet_data]  # Location of facet corners in facet coordinates
    v_facet_centroid_facet: list = [f.v_facet_centroid for f in facet_data]  # Location of facet centroids in facet coordinates

    # Get ensemble data
    v_facet_locs_ensemble = ensemble_data.v_facet_locations  # Locations of facet origins relative to ensemble origin in ensemble coordinates
    ...
    data_image_processing_facet = [cdc.CalculationsImageProcessingFacet() for _ in range(num_facets)]
    data_error = cdc.CalculationError()

    # Convert facet corners to ensemble coordinates
    v_ensemble_facet_corns = []
    for idx in range(num_facets):
        v_ensemble_facet_corns.append(v_facet_locs_ensemble[idx] + v_facet_corners_facet[idx].rotate(r_facet_ensemble[idx]))
    ...
    for r_facet_ensemble_cur, (idx_facet, idx_corn) in zip(r_facet_ensemble, ensemble_corns_indices):
        v_ensemble_corns_ensemble.append((v_facet_locs_ensemble[idx_facet] + v_facet_corners_facet[idx_facet][idx_corn].rotate(r_facet_ensemble_cur)).data)
    ...
    v_cam_ensemble_cent_cam_exp = sp.t_from_distance(v_mask_centroid_image,
                                                     optic_screen_dist,
                                                     camera,
                                                     orientation.v_cam_screen_cam)
    ...
    v_ensemble_corners_exp_image = camera.project(v_ensemble_corns_ensemble, r_cam_ensemble_exp.inv(), v_cam_ensemble_cam_exp)
    ...
    r_ensemble_cam_refine_1, v_cam_ensemble_cam_refine_1 = sp.calc_rt_from_img_pts(loop_ensemble_image_refine.vertices,
                                                                                   v_ensemble_corns_ensemble,
                                                                                   camera)
    ...
    v_facet_corners_image_exp = [camera.project(P, r_ensemble_cam_refine_1, v_cam_ensemble_cam_refine_1) for P in v_ensemble_facet_corns]
    ...
    r_ensemble_cam_refine_2, v_cam_ensemble_cam_refine_2 = sp.calc_rt_from_img_pts(v_facet_corners_all_image_refine,
    ...
    error_optic_screen_dist_1 = sp.distance_error(orientation.v_cam_screen_cam,
                                                  v_cam_ensemble_cam_refine_1 + v_meas_pt_ensemble_cam_refine_1,
                                                  optic_screen_dist)
    data_error.error_optic_screen_dist_1 = error_optic_screen_dist_1
    error_reprojection_1 = sp.reprojection_error(camera, v_ensemble_corns_ensemble, loop_ensemble_image_refine.vertices,
                                                 r_ensemble_cam_refine_1, v_cam_ensemble_cam_refine_1)
    data_error.error_reprojection_1 = error_reprojection_1

    # Calculate error 2 (R/T calculated using all facet corners)
    error_optic_screen_dist_2 = sp.distance_error(orientation.v_cam_screen_cam,
                                                  v_cam_ensemble_cam_refine_2 + v_meas_pt_ensemble_cam_refine_2,
                                                  optic_screen_dist)
    data_error.error_optic_screen_dist_2 = error_optic_screen_dist_2
    error_reprojection_2 = sp.reprojection_error(camera, v_ensemble_facet_corns_all, v_facet_corners_all_image_refine,
                                                 r_ensemble_cam_refine_2, v_cam_ensemble_cam_refine_2)
    data_error.error_reprojection_2 = error_reprojection_2
    ...
    error_reprojection_3 = sp.reprojection_error(camera, v_ensemble_facet_corns_all, v_facet_corners_all_image_refine,
                                                 r_cam_ensemble_refine_2, v_cam_ensemble_cam_refine_3)
