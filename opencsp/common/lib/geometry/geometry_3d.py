...
def direction_uxyz_given_azimuth_elevation(azimuth:float, elevation:float):  # Both radians.
    warn('geometry_3d.direction_uxyz_given_azimuth_elevation is deprecated. This function should be migrated to another library.', DeprecationWarning, stacklevel=2)
...
def distance_between_xyz_points(xyz_1, xyz_2):
    warn('geometry_3d.distance_between_xyz_points is deprecated. Use Vxyz subtraction/magnitude instead.', DeprecationWarning, stacklevel=2)
    x1 = xyz_1[0]
    y1 = xyz_1[1]
    z1 = xyz_1[2]
    x2 = xyz_2[0]
...
def vector_3d_cross_product(vxyz_1, vxyz_2):
    warn('geometry_3d.vector_3d_cross_product is deprecated. Use Vxyz.cross() instead.', DeprecationWarning, stacklevel=2)
    return list(np.cross(np.array(vxyz_1), np.array(vxyz_2)))  # ?? SCAFFOLDING RCB -- THESE ARRAY-LIST CONVERSIONS ARE YET ANOTHER ARGUMENT FOR USING NUMPY ARRAYS INSTEAD OF LISTS.  OR BETTER TO MAKE A VECTOR CLASS?
...
def normalize_vector_3d(vxyz):
    warn('geometry_3d.normalize_vector_3d is deprecated. Use Vxyz.normalize() instead.', DeprecationWarning, stacklevel=2)
...
def best_fit_plane_B(xs, ys, zs):
    warn('geometry_3d.best_fit_plane_B is deprecated. Should be migrated to another library.', DeprecationWarning, stacklevel=2)
...
def best_fit_plane(xyz_list):
    warn('geometry_3d.best_fit_plane is deprecated. Should be migrated to another library.', DeprecationWarning, stacklevel=2)
...
def flip_homogeneous_plane(plane):
    warn('geometry_3d.flip_homogeneous_plane is deprecated. Should be migrated to another library.', DeprecationWarning, stacklevel=2)
    # Reverse the sense of the homogeneous plane.
    return [-x for x in plane]


def homogeneous_plane_signed_distance_to_xyz(xyz, plane):
    warn('geometry_3d.homogeneous_plane_signed_distance_to_xyz is deprecated. Should be migrated to another library.', DeprecationWarning, stacklevel=2)
    x = xyz[0]
    y = xyz[1]
    z = xyz[2]
    A = plane[0]
...
def construct_line_3d_given_two_points(xyz_1, xyz_2, tolerance=0.0):  # ?? SCAFFOLDING RCB -- THE 3-D LINE SHOULD BE A CLASS.
    """
    A line_3d is an infinite line.
    It is not bounded by its defining points; that would be a line segment.
    """
    warn('geometry_3d.construct_line_3d_given_two_points is deprecated.  Should be migrated to another library.', DeprecationWarning, stacklevel=2)
    ...
    line_3d['theta']     = theta      # Angle the line points, after projecting onto the xy plane, measured ccw about the z axis.
...
def closest_point_on_line_3d(xyz, line_3d):  # ?? SCAFFOLDING RCB -- THE 3-D LINE SHOULD BE A CLASS.
    ...
    """
    warn('geometry_3d.closest_point_on_line_3d is deprecated. Should be migrated to another library.', DeprecationWarning, stacklevel=2)
    # Fetch coordinates.
    x = xyz[0]
    y = xyz[1]
    z = xyz[2]
...
def distance_to_line_3d(xyz, line_3d):  # ?? SCAFFOLDING RCB -- THE 3-D LINE SHOULD BE A CLASS.
    ...
    """
    warn('geometry_3d.distance_to_line_3d is deprecated. Should be migrated to another library.', DeprecationWarning, stacklevel=2)
    # Fetch coordinates.
    x = xyz[0]
    y = xyz[1]
    z = xyz[2]
