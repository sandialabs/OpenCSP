...
def matlab_color_bar():
    ...
    color_bar = [ (0,0,143),
                  ...
                  (159,0,0),
                  (143,0,0),
                  (128,0,0) ]  # ?? SCAFFOLDING RCB -- BUG: THIS LAST ENTRY IS AN ACTIVE CELL, NOT A BOUNDARY AT THE END OF THE CELL LIST.
    return color_bar
...
def corner_tour_closed_color_bar():
    ...
    color_bar = [ (   0.00,    0.00,  255.00),
                  ...
                  ( 255.00,    0.00,  255.00),
                  ( 127.50,    0.00,  255.00),
                  (   0.00,    0.00,  255.00) ]    # End boundary of the color bar, not the color of the last cell in the color bar.
    return color_bar
...
def nikon_D3300_monitor_equal_step_color_bar():
    ...
    color_bar = [ (   0.00,   14.00,  254.00),
                  ...
                  ( 107.00,    0.00,  255.00),
                  (  76.00,    0.00,  255.00),
                  (   0.00,   14.00,  254.00) ]  # End boundary of the color bar, not the color of the last cell in the color bar.
    return color_bar
...
def color_given_value(val, val_min, val_max, color_below_min, color_bar, color_above_max, discrete_or_continuous):
    ...
    if discrete_or_continuous == 'discrete':
        ...
        if idx < 0:
            ...
            # Return.
            return [ (color_0[0] + (frac * d_red)), 
                     (color_0[1] + (frac * d_green)), 
                     (color_0[2] + (frac * d_blue)) ]
    else:
        print('ERROR: In color_given_value(), encountered unexpected discrete_or_continuous value:', discrete_or_continuous)
        assert False
...
def color_bar_segment_spanned_angle(idx, color_bar):
    ...
    if idx >= (len(color_bar)-1):  # Below we fetch color_bar(idx+1), so compare against (len(color-bar)-1).
        print('ERROR: In angle_between_color_vectors(), idx = ', str(idx), ' is greater than or equal to color bar length:', len(color_bar))
        assert False  # ?? SCAFFOLDING RCB -- REPLACE WITH RAISING AN EXCEPTION?  (THROUGHOUT)
    rgb_1 = color_bar[idx]
