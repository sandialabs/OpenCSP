...
class TargetColor(TargetAbstract):
    ...
    def __init__(self,
        ...
        initial_rgb = self.initial_color.rgb_255()  # ?? SCAFFOLDING RCB -- I GOT TRIPPED UP BY CONFUSION RE: IMAGES IN [0,1.0] AND IMAGES IN [0,255].  HOW BEST RESOLVE/PREVENT?
    ...
    def rows_cols(self):  # ?? SCAFFOLDING RCB -- MODIFY TO USE BASE CLASS ROWS_COLS_BANDS() ACCESSOR, AND THEN APPLY COLOR-SPECIFIC TEST.
    ...
    def set_image_to_linear_color_bar_x(self, color_below_min:Color, color_bar, color_above_max:Color, discrete_or_continuous:str) -> None:
        n_rows, n_cols = self.rows_cols()
        for row in range(0,n_rows):
            for col in range(0,n_cols):
                ...
                val_min = 0
                val_max = n_cols
                color = tcc.color_given_value(val, val_min, val_max, color_below_min, color_bar, color_above_max, discrete_or_continuous)
                # Set pixel color
                # ?? SCAFFOLDING RCB -- FIXUP ALL THIS CONFUSION REGARDING WHETHER COLORS ARE OVER [0,1] OR [0,255].
                self.image[row,col,0] = color[0]#/255.0  # ?? SCAFFOLDING RCB -- CONVERT COLOR BAR TO INTERVAL [0,1]
                self.image[row,col,1] = color[1]#/255.0
                self.image[row,col,2] = color[2]#/255.0

    # Linear color bar, y direction
    def set_image_to_linear_color_bar_y(self, 
        ...
        for row in range(0,n_rows):
            for col in range(0,n_cols):
                ...
                color = tcc.color_given_value(val, val_min, val_max, color_below_min, color_bar, color_above_max, discrete_or_continuous)  # ?? SCAFFOLDING -- USE "SPLIT" CONTROL PARAMETER.
                ...
                if lateral_gradient_type == None:
                    ...
                    print('ERROR: In TargetColor.set_image_to_linear_color_bar_y(), encountered unexpected lateral_gradient_type = ' + str(lateral_gradient_type))
    ...
    # Polar color bar
    def set_image_to_polar_color_bar(self, 
        ...
        for row in range(0,n_rows):
            ...
            for col in range(0,n_cols):
                ...
                color = tcc.color_given_value(this_angle, -math.pi, math.pi, color_below_min, color_bar, color_above_max, discrete_or_continuous)
                ...
                if pattern_boundary == 'circle':
                    ...
                    if math.sin(this_angle) == 0:
                        ...
                        radius_for_this_angle = min(abs(half_width/math.cos(this_angle)), abs(half_height/math.sin(this_angle)))
                else:
                    print('ERROR:  In TargetColor.set_image_to_polar_color_bar(), unexpected pattern_boundary = "' + str(pattern_boundary))
                ...
                if radial_gradient_type == "saturated_center_to_white":
                    ...
                    saturation_range = light_center_to_saturated_saturation_max - light_center_to_saturated_saturation_min
                    ...
                    print('ERROR: In TargetColor.set_image_to_polar_color_bar(), encountered unexpected radial_gradient_type = ' + str(radial_gradient_type))
                    assert False  # ?? SCAFFOLDING RCB -- USE EXCEPTION

                # Set pixel color
                # ?? SCAFFOLDING RCB -- FIXUP ALL THIS CONFUSION REGARDING WHETHER COLORS ARE OVER [0,1] OR [0,255].
                self.image[row,col,0] = this_red   #/255.0  # ?? SCAFFOLDING RCB -- CONVERT COLOR BAR TO INTERVAL [0,1]?
    ...
    def set_center_fiducial(self, center_fiducial_width_pix, center_fiducial_color):
        ...
        if center_fiducial_half_margin_pix == 0:
            ...
            for this_row in range(center_row-center_fiducial_half_margin_pix, (center_row+center_fiducial_half_margin_pix)+1):
                for this_col in range(center_col-center_fiducial_half_margin_pix, (center_col+center_fiducial_half_margin_pix)+1):
            ...
            print('ERROR: In TargetColor.set_center_fiducial(), unexpected negative center_fiducial_half_margin_pix = ' + str(center_fiducial_half_margin_pix))
            assert False  # ?? SCAFFOLDING RCB -- CONVERT TO EXCEPTION.
        ...
        for tick_idx_x in range(n_ticks_x):
            ...
            for tick_idx_y in range(tick_length_pix):
                ...
                if tick_half_margin_pix == 0:
                    ...
                    print('ERROR: In TargetColor.set_ticks_along_top_and_bottom_edges(), unexpected negative tick_half_margin_pix = ' + str(tick_half_margin_pix))
                    assert False  # ?? SCAFFOLDING RCB -- CONVERT TO EXCEPTION.
        ...
        for tick_idx_y in range(n_ticks_y):
            ...
            for tick_idx_x in range(tick_length_pix):
                ...
                if tick_half_margin_pix == 0:
                    ...
                    print('ERROR: In TargetColor.set_ticks_along_left_and_right_edges(), unexpected negative tick_half_margin_pix = ' + str(tick_half_margin_pix))
                    assert False  # ?? SCAFFOLDING RCB -- CONVERT TO EXCEPTION.
    ...
    def adjust_color_saturation(self, saturation_fraction:float) -> None:
        ...
        for row in range(0,n_rows):
            for col in range(0,n_cols):
                ...
                new_red, new_green, new_blue = self.adjust_rgb_color_saturation(original_rgb, saturation_fraction, max_rgb)
                # Set pixel color.
...
def construct_target_linear_color_bar(image_width:float,          # Meter
    ...
    # Set colors.
    if x_or_y == 'x':
        target.set_image_to_linear_color_bar_x(color_below_min, color_bar, color_above_max, discrete_or_continuous)  # ?? SCAFFOLDING RCB -- UPDATE COLOR BAR X TO MATCH Y.
    elif x_or_y == 'y':
        target.set_image_to_linear_color_bar_y(color_below_min, color_bar, color_above_max, discrete_or_continuous,
    ...
    if include_above_below_in_pattern_name:
        color_pattern_name = color_below_min.short_name + '.' + color_bar_name + '_linear.' + color_above_max.short_name  # ?? SCAFFOLDING RCB -- REPLACE "color_bar_name" WITH CLASS FETCH
    else:
...
def construct_target_polar_color_bar(# Target dimensions.
    ...
    if discrete_or_continuous == 'discrete':
        ...
        print('ERROR: In TargetColor.construct_target_polar_color_bar(), encountered unexpected discrete_or_continuous = ' + str(discrete_or_continuous))
        assert False  # ?? SCAFFOLDING RCB -- USE EXCEPTION
    # Radial gradient.
    if radial_gradient_type == "saturated_center_to_white":
        pattern_description += '_exp' + '{0:.2f}'.format(saturated_center_to_white_exponent)
    elif radial_gradient_type == "light_center_to_saturated":
        pattern_description += '_sat' + '{0:.2f}'.format(light_center_to_saturated_saturation_min) + 'to' + '{0:.2f}'.format(light_center_to_saturated_saturation_max)
    else:
        print('ERROR: In TargetColor.construct_target_polar_color_bar(), encountered unexpected radial_gradient_type = ' + str(radial_gradient_type))
        assert False  # ?? SCAFFOLDING RCB -- USE EXCEPTION
    # Radial boundary.
    if pattern_boundary == "circle":
        ...
        print('ERROR: In TargetColor.construct_target_polar_color_bar(), encountered unexpected pattern_boundary = ' + str(pattern_boundary))
        assert False  # ?? SCAFFOLDING RCB -- USE EXCEPTION
    # Fiducials.
    if draw_center_fiducial:
...
def construct_target_blue_under_red_cross_green(image_width:float,   # Meter
                                                image_height:float,  # Meter
                                                dpm:float,           # Dots per meter
                                                ) -> TargetColor:
    # Blank target.
    target = tc.TargetColor(image_width, image_height, dpm, Color.black())  # We will cover the entire image with new colors.
    # Set colors.
...
def construct_target_rgb_cube_inscribed_square(image_width:float,   # Meter
                                               image_height:float,  # Meter
                                               dpm:float,           # Dots per meter
                                               project_to_cube:bool) -> TargetColor:
    # Blank target.
    target = tc.TargetColor(image_width, image_height, dpm, Color.black())  # We will cover the entire image with new colors.
    # Set colors.
...
def extend_target_left(target:TargetColor,        # Target to extend.
    ...
    dpm = target.dpm   # ?? SCAFFOLDING RCB -- SHOULD THESE BE ACCESSOR FUNCTIONS?  WHAT IS OPENCSP POLICY/PATTERN ON THIS?
    ...
    if new_n_rows != check_n_rows:
        print("ERROR: In extend_target_left(), unequal new_n_rows="+str(new_n_rows)+" and check_n_rows="+str(check_n_rows))
        assert False   # ?? SCAFFOLDING RCB -- CONVERT TO EXCEPTION
    if new_n_cols != check_n_cols:
        print("ERROR: In extend_target_left(), unequal new_n_cols="+str(new_n_cols)+" and check_n_cols="+str(check_n_cols))
        assert False   # ?? SCAFFOLDING RCB -- CONVERT TO EXCEPTION
    if new_n_bands != check_n_bands:
        print("ERROR: In extend_target_left(), unequal new_n_bands="+str(new_n_bands)+" and check_n_bands="+str(check_n_bands))
        assert False   # ?? SCAFFOLDING RCB -- CONVERT TO EXCEPTION
    ...
    if new_target_name == None:
        new_target.set_pattern_description('l' + str(new_pixels) + new_color.short_name + 'px_' + target.pattern_description)
    else:
...
def extend_target_right(target:TargetColor,        # Target to extend.
                        new_pixels:int,            # Pixels.  Number of additional pixels to extend.
                        new_color:Color.Color,     # Color to fill new pixels.
                        new_target_name:str=None,  # If none, new target name will extend input target name.
                        ) -> TargetColor:  
    ...
    check_n_rows, check_n_cols, check_n_bands = new_target.rows_cols_bands()
    if new_n_rows != check_n_rows:
        print("ERROR: In extend_target_right(), unequal new_n_rows="+str(new_n_rows)+" and check_n_rows="+str(check_n_rows))
        assert False   # ?? SCAFFOLDING RCB -- CONVERT TO EXCEPTION
    if new_n_cols != check_n_cols:
        print("ERROR: In extend_target_right(), unequal new_n_cols="+str(new_n_cols)+" and check_n_cols="+str(check_n_cols))
        assert False   # ?? SCAFFOLDING RCB -- CONVERT TO EXCEPTION
    if new_n_bands != check_n_bands:
        print("ERROR: In extend_target_right(), unequal new_n_bands="+str(new_n_bands)+" and check_n_bands="+str(check_n_bands))
        assert False   # ?? SCAFFOLDING RCB -- CONVERT TO EXCEPTION
    ...
    # Set description.
    if new_target_name == None:
        new_target.set_pattern_description(target.pattern_description + '_r' + str(new_pixels) + new_color.short_name + 'px')
    else:
        new_target.set_pattern_description(new_target_name)
...
def extend_target_top(target:TargetColor,        # Target to extend.
                      new_pixels:int,            # Pixels.  Number of additional pixels to extend.
                      new_color:Color.Color,     # Color to fill new pixels.
                      new_target_name:str=None,  # If none, new target name will extend input target name.
                      ) -> TargetColor:  
    ...
    check_n_rows, check_n_cols, check_n_bands = new_target.rows_cols_bands()
    if new_n_rows != check_n_rows:
        print("ERROR: In extend_target_top(), unequal new_n_rows="+str(new_n_rows)+" and check_n_rows="+str(check_n_rows))
        assert False   # ?? SCAFFOLDING RCB -- CONVERT TO EXCEPTION
    if new_n_cols != check_n_cols:
        print("ERROR: In extend_target_top(), unequal new_n_cols="+str(new_n_cols)+" and check_n_cols="+str(check_n_cols))
        assert False   # ?? SCAFFOLDING RCB -- CONVERT TO EXCEPTION
    if new_n_bands != check_n_bands:
        print("ERROR: In extend_target_top(), unequal new_n_bands="+str(new_n_bands)+" and check_n_bands="+str(check_n_bands))
        assert False   # ?? SCAFFOLDING RCB -- CONVERT TO EXCEPTION
    ...
    # Set description.
    if new_target_name == None:
        new_target.set_pattern_description('t' + str(new_pixels) + new_color.short_name + 'px_' + target.pattern_description)
    else:
        new_target.set_pattern_description(new_target_name)
...
def extend_target_bottom(target:TargetColor,        # Target to extend.
                         new_pixels:int,            # Pixels.  Number of additional pixels to extend.
                         new_color:Color.Color,     # Color to fill new pixels.
                         new_target_name:str=None,  # If none, new target name will extend input target name.
                         ) -> TargetColor:  
    ...
    check_n_rows, check_n_cols, check_n_bands = new_target.rows_cols_bands()
    if new_n_rows != check_n_rows:
        print("ERROR: In extend_target_bottom(), unequal new_n_rows="+str(new_n_rows)+" and check_n_rows="+str(check_n_rows))
        assert False   # ?? SCAFFOLDING RCB -- CONVERT TO EXCEPTION
    if new_n_cols != check_n_cols:
        print("ERROR: In extend_target_bottom(), unequal new_n_cols="+str(new_n_cols)+" and check_n_cols="+str(check_n_cols))
        assert False   # ?? SCAFFOLDING RCB -- CONVERT TO EXCEPTION
    if new_n_bands != check_n_bands:
        print("ERROR: In extend_target_bottom(), unequal new_n_bands="+str(new_n_bands)+" and check_n_bands="+str(check_n_bands))
        assert False   # ?? SCAFFOLDING RCB -- CONVERT TO EXCEPTION
    ...
    # Set description.
    if new_target_name == None:
        new_target.set_pattern_description(target.pattern_description + '_b' + str(new_pixels) + new_color.short_name + 'px')
    else:
        new_target.set_pattern_description(new_target_name)
...
def extend_target_all(target:TargetColor,        # Target to extend.
                      new_pixels:int,            # Pixels.  Number of additional pixels to extend.
                      new_color:Color.Color,     # Color to fill new pixels.
                      new_target_name:str=None,  # If none, new target name will extend input target name.
                      ) -> TargetColor:  
    ...
    if new_target_name == None:
        ...
        new_target.set_pattern_description('bord' + str(new_pixels) + new_color.short_name + 'px_' + target.pattern_description)
    else:
        new_target.set_pattern_description(new_target_name)
...
def extend_target_for_splice_left_right(target:TargetColor, n_extend:int, fill_color:Color.Color, auto_expand:str) -> TargetColor:
    if auto_expand == 'fill_top':
        ...
        if (n_extend_top == 0) and (n_extend_bottom == 0):
            print('ERROR: In extend_target_for_splice_left_right(), encountered unexpected situation where n_extend_top and n_extend_bottom are both zero.')
            assert False  # ?? SCAFFOLDING -- CONVERT TO EXCEPTION.
        if n_extend_top > 0:
            new_target_top = extend_target_top(target, n_extend_top, fill_color, new_target_name=target.pattern_description)
        else:
            new_target_top = target
        if n_extend_bottom > 0:
            new_target_top_bottom = extend_target_bottom(new_target_top, n_extend_bottom, fill_color, new_target_name=target.pattern_description)
        else:
            new_target_top_bottom = new_target_top
        return new_target_top_bottom
    else:
        print('ERROR: In extend_target_for_splice_left_right(), encountered unexpected auto_expand = "' + str(auto_expand) + '".')
        assert False  # ?? SCAFFOLDING -- CONVERT TO EXCEPTION.
...
def splice_targets_left_right(left_target:TargetColor,     # Target to place at left of new target. 
    ...
    left_dpm  = left_target.dpm   # ?? SCAFFOLDING RCB -- SHOULD THESE BE ACCESSOR FUNCTIONS?  WHAT IS OPENCSP POLICY/PATTERN ON THIS?
    right_dpm = right_target.dpm  # ?? SCAFFOLDING RCB -- SHOULD THESE BE ACCESSOR FUNCTIONS?  WHAT IS OPENCSP POLICY/PATTERN ON THIS?
    ...
    # Check input.
    if left_n_rows != right_n_rows:
        print("ERROR: In splice_targets_left_right(), unequal left_n_rows="+str(left_n_rows)+" and right_n_rows="+str(right_n_rows))
        assert False   # ?? SCAFFOLDING RCB -- CONVERT TO EXCEPTION
    if left_n_bands != right_n_bands:
        print("ERROR: In splice_targets_left_right(), unequal left_n_bands="+str(left_n_bands)+" and right_n_bands="+str(right_n_bands))
        assert False   # ?? SCAFFOLDING RCB -- CONVERT TO EXCEPTION
    if left_n_bands != 3:
        print("ERROR: In splice_targets_left_right(), left_n_bands="+str(left_n_bands)+" is not equal to 3.")
        assert False   # ?? SCAFFOLDING RCB -- CONVERT TO EXCEPTION
    if left_dpm != right_dpm:
        print("ERROR: In splice_targets_left_right(), unequal left_dpm="+str(left_dpm)+" and right_dpm="+str(right_dpm))
        assert False   # ?? SCAFFOLDING RCB -- CONVERT TO EXCEPTION
    ...
    check_n_rows, check_n_cols, check_n_bands = new_target.rows_cols_bands()
    if new_n_rows != check_n_rows:
        print("ERROR: In splice_targets_left_right(), unequal new_n_rows="+str(new_n_rows)+" and check_n_rows="+str(check_n_rows))
        assert False   # ?? SCAFFOLDING RCB -- CONVERT TO EXCEPTION
    if new_n_cols != check_n_cols:
        print("ERROR: In splice_targets_left_right(), unequal new_n_cols="+str(new_n_cols)+" and check_n_cols="+str(check_n_cols))
        assert False   # ?? SCAFFOLDING RCB -- CONVERT TO EXCEPTION
    if new_n_bands != check_n_bands:
        print("ERROR: In splice_targets_left_right(), unequal new_n_bands="+str(new_n_bands)+" and check_n_bands="+str(check_n_bands))
        assert False   # ?? SCAFFOLDING RCB -- CONVERT TO EXCEPTION
    ...
    # Set description.
    if new_target_name == None:
        new_target.set_pattern_description(left_target.pattern_description + "__left__" + right_target.pattern_description)
    else:
        new_target.set_pattern_description(new_target_name)
...
def splice_targets_above_below(above_target:TargetColor,   # Target to place at top of new target. 
    ...
    above_dpm = above_target.dpm  # ?? SCAFFOLDING RCB -- SHOULD THESE BE ACCESSOR FUNCTIONS?  WHAT IS OPENCSP POLICY/PATTERN ON THIS?
    below_dpm = below_target.dpm  # ?? SCAFFOLDING RCB -- SHOULD THESE BE ACCESSOR FUNCTIONS?  WHAT IS OPENCSP POLICY/PATTERN ON THIS?

    # Check input.
    if above_n_cols != below_n_cols:  # ?? SCAFFOLDING RCB -- EXTEND ROUTINE TO ALLOW UNEQUAL COLUMNS?
        print("ERROR: In splice_targets_above_below(), unequal above_n_cols="+str(above_n_cols)+" and below_n_cols="+str(below_n_cols))
        assert False   # ?? SCAFFOLDING RCB -- CONVERT TO EXCEPTION
    if above_n_bands != below_n_bands:
        print("ERROR: In splice_targets_above_below(), unequal above_n_bands="+str(above_n_bands)+" and below_n_bands="+str(below_n_bands))
        assert False   # ?? SCAFFOLDING RCB -- CONVERT TO EXCEPTION
    if above_n_bands != 3:
        print("ERROR: In splice_targets_above_below(), above_n_bands="+str(above_n_bands)+" is not equal to 3.")
        assert False   # ?? SCAFFOLDING RCB -- CONVERT TO EXCEPTION
    if above_dpm != below_dpm:
        print("ERROR: In splice_targets_above_below(), unequal above_dpm="+str(above_dpm)+" and below_dpm="+str(below_dpm))
        assert False   # ?? SCAFFOLDING RCB -- CONVERT TO EXCEPTION
    ...
    check_n_rows, check_n_cols, check_n_bands = new_target.rows_cols_bands()
    if new_n_rows != check_n_rows:
        print("ERROR: In splice_targets_above_below(), unequal new_n_rows="+str(new_n_rows)+" and check_n_rows="+str(check_n_rows))
        assert False   # ?? SCAFFOLDING RCB -- CONVERT TO EXCEPTION
    if new_n_cols != check_n_cols:
        print("ERROR: In splice_targets_above_below(), unequal new_n_cols="+str(new_n_cols)+" and check_n_cols="+str(check_n_cols))
        assert False   # ?? SCAFFOLDING RCB -- CONVERT TO EXCEPTION
    if new_n_bands != check_n_bands:
        print("ERROR: In splice_targets_above_below(), unequal new_n_bands="+str(new_n_bands)+" and check_n_bands="+str(check_n_bands))
        assert False   # ?? SCAFFOLDING RCB -- CONVERT TO EXCEPTION
    ...
    # Set description.
    if new_target_name == None:
        new_target.set_pattern_description(above_target.pattern_description + "__above__" + below_target.pattern_description)
    else:
        new_target.set_pattern_description(new_target_name)
...
def construct_stacked_linear_color_bar(n_stack:int,
                                       color_bar_width, 
                                       color_total_height, 
                                       composite_dpm,
                                       color_below_min,
    ...
    if n_stack != len(discrete_or_continuous_list):
        print('ERROR: In stack_linear_color_bar(), encountered mismatched n_stack=' + str(n_stack) + ' and len(discrete_or_continuous_list)=' + len(discrete_or_continuous_list))
        assert False  # ?? SCAFFOLDING RCB -- USE EXCEPTION
    if n_stack != len(saturation_spec_list):
        print('ERROR: In stack_linear_color_bar(), encountered mismatched n_stack=' + str(n_stack) + ' and len(saturation_spec_list)=' + len(saturation_spec_list))
        assert False  # ?? SCAFFOLDING RCB -- USE EXCEPTION
    ...
    single_bar_target_discrete   = tc.construct_target_linear_color_bar(color_bar_width, color_total_height/n_stack, composite_dpm,
                                                                        color_below_min, color_bar, color_bar_name, color_above_max, x_or_y, 'discrete',
    ...
    single_bar_target_continuous = tc.construct_target_linear_color_bar(color_bar_width, color_total_height/n_stack, composite_dpm,
                                                                        color_below_min, color_bar, color_bar_name, color_above_max, x_or_y, 'continuous',
                                                                        lateral_gradient_type=first_lateral_gradient_type,
                                                                        saturated_to_white_exponent=first_saturated_to_white_exponent,
                                                                        light_to_saturated_min=first_light_to_saturated_min,
                                                                        light_to_saturated_max=first_light_to_saturated_max)
    ...
    if discrete_or_continuous_list[0] == 'discrete':
        ...
        print('ERROR: In construct_stacked_linear_color_bar(1), encountered unexpected discrete_or_continuous_list[0] = "' + str(discrete_or_continuous_list[0]) + '".')
        assert False  # ?? SCAFFOLDING RCB -- USE EXCEPTION
    ...
    if n_stack == 1:
        ...
        for idx in range(1, n_stack):
            ...
            if discrete_or_continuous_list[idx] == 'discrete':
                this_bar_target_discrete   = tc.construct_target_linear_color_bar(color_bar_width, color_total_height/n_stack, composite_dpm,
                                                                                  color_below_min, color_bar, color_bar_name, color_above_max, x_or_y, 'discrete',
                ...
                stacked_target = tc.splice_targets_above_below(this_bar_target_discrete, stacked_target, gap, initial_color=gap_color, new_target_name=target_name)
            elif discrete_or_continuous_list[idx] == 'continuous':
                this_bar_target_continuous = tc.construct_target_linear_color_bar(color_bar_width, color_total_height/n_stack, composite_dpm,
                                                                                  color_below_min, color_bar, color_bar_name, color_above_max, x_or_y, 'continuous',
                                                                                  lateral_gradient_type=this_lateral_gradient_type,
                                                                                  saturated_to_white_exponent=this_saturated_to_white_exponent,
                                                                                  light_to_saturated_min=this_light_to_saturated_min,
                                                                                  light_to_saturated_max=this_light_to_saturated_max)
                stacked_target = tc.splice_targets_above_below(this_bar_target_continuous, stacked_target, gap, initial_color=gap_color, new_target_name=target_name)
            else:
                print('ERROR: In construct_stacked_linear_color_bar(2), encountered unexpected discrete_or_continuous_list['+str(idx)+'] = "' + str(discrete_or_continuous_list[idx]) + '".')
                assert False  # ?? SCAFFOLDING RCB -- USE EXCEPTION
...
def construct_linear_color_bar_cascade(# Dimensions.
    ...
    if len(list_of_discrete_or_continuous_lists) != len(stack_sequence):
        print('ERROR: In construct_linear_color_bar_cascade(), mismatched len(stack_sequence)=' + str(len(stack_sequence)) + ' and len(list_of_discrete_or_continuous_lists)=' + str(len(list_of_discrete_or_continuous_lists)))
        assert False  # ?? SCAFFOLDING RCB -- USE EXCEPTION
    ...
    ref_target = tc.construct_target_linear_color_bar(color_bar_width, color_total_height, composite_dpm,
                                                      ref_color_below_min, ref_color_bar, ref_color_bar_name, ref_color_above_max, 
                                                      x_or_y, 'discrete')
    ...
    main_color_target = tc.construct_target_linear_color_bar(color_bar_width, color_total_height, composite_dpm,
                                                             color_below_min, color_bar, color_bar_name, color_above_max, 
                                                             x_or_y, 'discrete')
    cascade_target = tc.splice_targets_left_right(cascade_target, main_color_target, gap=ref_gap_pix, initial_color=gap_color, new_target_name=cascade_target_name)

    # Generate cascade.
    for n_bars_in_stack, discrete_or_continuous_list, saturation_spec_list in zip(stack_sequence, list_of_discrete_or_continuous_lists, list_of_saturation_spec_lists):
        # Status update.
        ...
        stacked_color_target = tc.construct_stacked_linear_color_bar(n_bars_in_stack, color_bar_width, color_total_height, composite_dpm, 
                                                                     color_below_min, color_bar, color_bar_name, color_above_max, x_or_y, 
                                                                     discrete_or_continuous_list, saturation_spec_list, gap_color)
        ...
        if include_grey_neighbors:
            ...
            grey_rgb_list  = [(255*x, 255*x, 255*x) for x in grey_scale_list]  # ?? SCAFFOLDING RCB -- IF COLOR_BAR BECOMES A CLASS, UPDATE THIS.
            grey_below_min = Color.black()
            grey_above_max = Color.white()
            grey_bar_name = 'grey_' + str(len(grey_scale_list))
            grey_target   = tc.construct_target_linear_color_bar(grey_bar_width, color_total_height, composite_dpm,
                                                                 grey_below_min, grey_rgb_list, grey_bar_name, grey_above_max, x_or_y, 'discrete') # Always discrete.
            stacked_target = tc.splice_targets_left_right(grey_target, stacked_color_target, gap=0, initial_color=gap_color, new_target_name=cascade_target_name)
            stacked_target = tc.splice_targets_left_right(stacked_target, grey_target, gap=0, initial_color=gap_color, new_target_name=cascade_target_name)
        else:
            stacked_target = stacked_color_target

        # Update cascade.
        cascade_target = tc.splice_targets_left_right(cascade_target, stacked_target, gap_between_bars_pix, gap_color, new_target_name=cascade_target_name)
