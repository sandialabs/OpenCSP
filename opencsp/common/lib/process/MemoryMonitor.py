...
class MemoryMonitor():
    def __init__(self, server_index=0, cpu_index=0, max_lifetime_hours=2.0, print_threshold=0.95, print_on_new_min=True, always_print=False):
    ...
    def _run(self):
        ...
        self._proc = multiprocessing.Process(target=_monitor_sys_memory, 
...
def _monitor_sys_memory(max_lifetime_secs: int,
    ...
    while (elapsed := time.time() - start) < max_lifetime_secs:
        ...
        if do_print_min or do_print_threshold or always_print:
            lt.info("MM: %s %s %s %s" % \
                    (f"{int(elapsed):>3d}", f"{sys_tot:0.1f}".rjust(5), f"{sys_used:0.1f}".rjust(5), f"{sys_free:0.1f}".rjust(5))
            )
