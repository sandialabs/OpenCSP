...
class ParallelPartitioner():
    ...
    def _get_portion_range(self, count: int, nworkers: int, worker_idx: int, partitions_per_worker: int, npartitions: int = -1):
        if npartitions < 0:
    ...
    def _get_portion(self, data: list[T], nworkers: int, worker_idx: int, partitions_per_worker: int, npartitions: int = -1):
    ...
    def get_my_range(self, data: list[T], desc: str=None):
        ...
        rstart_serv, rend_serv = self._get_portion_range(len(data), self.nservers, self.server_idx, npartitions_per_server)
