...
def parallel_frames_to_videos(partitioner: ppart.ParallelPartitioner, frames_path: str, frames_names: list[str], videos_path: str, render_control: rcv.RenderControlVideo=None, overwrite=False):
    ...
    lt.info(f"partition info: [nservers: {partitioner.nservers}, server_idx: {partitioner.server_idx}, ncpus: {partitioner.ncpus}, cpu_idx: {partitioner.cpu_idx}]")
    ...
    video_handler: vh.VideoHandler = vh.VideoHandler.VideoCreator(frames_path, video_out_path_file_ext, render_control, frame_control)
    video_name_ext = video_handler.frames_to_video(my_frames, overwrite=overwrite)

    return video_name_ext


def parallel_video_to_frames(num_servers: int, server_index: int, video_handler: vh.VideoHandler, server_synchronizer: ss.ServerSynchronizer):
    ...
    video_handler = vh.VideoHandler.VideoExtractor(src_video_dir_name_ext, dst_frames_dir_serv, dst_example_frames_dir_serv, frame_control)
    ...
    lt.info(f"In parallel_video_to_frames(), server @{server_index} (0-{num_servers-1}) extracting frames from {rstart:0.1f}s to {rend:0.1f}s")
    ...
    lt.info(f"In parallel_video_to_frames(), server extracted {len(non_duplicate_frame_files)} frames and {len(duplicate_frame_files)} duplicates")
    ...
    if server_index == 0:
        for dst_dir, fnf in [(dst_frames_dir, frame_name_format), (dst_example_frames_dir, frame_name_format_example)]:
            ...
            for si in range(num_servers):
                ...
                if num_residual != 0:
                    lt.warn(f"Warning: in parallel_video_to_frames(), server's extraction directory isn't empty! (found {num_residual} files)")
                else:
