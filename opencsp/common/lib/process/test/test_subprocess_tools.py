...
class TestSubprocess(unittest.TestCase):
    ...
    def test_get_mixed_stdout_stderr(self):
        if os.name == "nt":
            ...
            return
        output = subt.run(f"{sys.executable} {helper.__file__} --mixed_stdout_stderr", stdout="collect", stderr="collect")
    ...
    def test_timeout_completes(self):
        ...
        output = subt.run(f"{sys.executable} {helper.__file__} --simple_stdout --delay_before=0.5", stdout="collect", stderr="collect", timeout=5.0)
    ...
    def test_timeout_timesout(self):
        ...
        with self.assertRaises((subprocess.CalledProcessError,subprocess.TimeoutExpired)):
            start_time = time.time()
            output = subt.run(f"{sys.executable} {helper.__file__} --simple_stdout --simple_stderr --delay_before=0.5 --delay_after=0.4", stdout="collect", stderr="collect", timeout=0.4)
