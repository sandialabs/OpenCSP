...
class TestSubprocess(unittest.TestCase):
    ...
    def test_single_process_retval(self):
        ...
        self.assertEqual(
            len(results), 1, "Should get one result back from the single process"
        )
    ...
    def test_many_process_retval(self):
        ...
        self.assertEqual(
            len(results), 20, "Should get 20 results back from the 20 processes"
        )
        self.assertEqual(
            results, outputs, "Subprocess should have added 2 to each value"
        )
    ...
    def test_many_process_retval_randsleep(self):
        ...
        self.assertEqual(
            sorted(results), outputs, "Subprocess should have added 2 to each value"
        )
        self.assertEqual(
            results,
            outputs,
            "Output order from pool.starmap() should be the same as input order",
        )
