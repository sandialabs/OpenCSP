...
class ServerSynchronizer():
    ...
    def __init__(self, num_servers: int, server_index: int, propagate_errors=True, timeout: int=1000, do_initial_wait=True):
    ...
    def _wait_on_files(self, wait_file_path_name_exts: list[str], stop_file_path_name_exts: list[str]=None, msg: str=None):
    ...
    def _wait(self, check_for_stopped_servers=True):
        ...
        if not ft.file_exists(my_file_path_name_ext):
            ...
            lt.warn(f"Warning: in ServerSynchronizer.wait(), current synchronization file {my_file_path_name_ext} " + \
        ...
        try:
            ...
            for i in range(self.num_servers):
                ...
                if ft.file_exists(next_file_path_name_ext):
                    lt.warn(f"Warning: in ServerSynchronizer.wait(), next synchronization file {next_file_path_name_ext} " + \
                            "should not exist yet! This probably indicates a bug in the flow of your synchronized server code!")
            ...
            self._wait_on_files(wait_file_path_name_exts, stop_file_path_name_exts, msg=f"step {self._synchronization_index}")
    ...
    def gather(self, value: str):
        ...
        if ft.file_exists(my_file_path_name_ext):
            lt.warn(f"Warning: in ServerSynchronizer.gather(), value file {my_file_path_name_ext} " + \
                    "should not exist yet! This probably indicates a bug in the flow of your synchronized server code!")
            ft.delete_file(my_file_path_name_ext)
    ...
    def stop(self, error_to_propagate: Exception=None):
        ...
        if not ft.file_exists(my_file_path_name_ext):
            ...
            lt.warn(f"Warning: in ServerSynchronizer.stop(), stop synchronization file {my_file_path_name_ext} " + \
                    "should not exist yet! This probably indicates a bug in the flow of your synchronized server code!")
        ...
        if error_to_propagate != None:
            if not isinstance(error_to_propagate, sse.ServerSynchronizerError):
                ...
                if not ft.file_exists(my_file_path_name_ext):
                    ...
                    lt.warn(f"Warning: in ServerSynchronizer.stop(), error synchronization file {my_file_path_name_ext} " + \
                            "should not exist yet! This probably indicates a bug in the flow of your synchronized server code!")
        ...
        if err_msg != None and self.propagate_errors:
            if error_to_propagate != None and \
                    isinstance(error_to_propagate, sse.ServerSynchronizerError) and \
                    str(error_to_propagate).replace("wait()","stop()") == err_msg:
