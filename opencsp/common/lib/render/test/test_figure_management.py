...
class test_figure_management(unittest.TestCase):
    ...
    def test_save_all_figures_two_lines(self):
        ...
        for i in range(2):
            figure_control = rcfg.RenderControlFigure(tile_array=(1,1), tile_square=True)
            fig_record = fm.setup_figure(figure_control, name=names[i], code_tag=f"{__file__}.test_save_all_figures_two_lines()")
            view = fig_record.view
            line = lines[i]
            view.draw_p_list(line)
    ...
    def test_save_all_figures_timeout_0(self):
        ...
        figure_control = rcfg.RenderControlFigure(tile_array=(1,1), tile_square=True)
        fig_record = fm.setup_figure(figure_control, name=name, code_tag=f"{__file__}.test_save_all_figures_timeout_0()")
        view = fig_record.view
        line = list(range(100))
        view.draw_p_list(line)

        with self.assertRaisesRegex((RuntimeError,subprocess.TimeoutExpired), r".*failed to save.*", msg="Failed to time out in 0 seconds while trying to save figure"):
    ...
    def _figure_manager_timeout_1(self):
        ...
        fig_record = rcfr_is.RenderControlFigureRecordInfSave(name=fig_old.name, title=fig_old.title, caption=fig_old.caption, figure_num=fig_old.figure_num, figure=fig_old.figure)
    ...
    def test_save_all_figures_fail_and_raise(self):
        ...
        if os.name == "nt":
            ...
            stderr = st.run(f"{sys.executable} {__file__} --funcname save_all_figures_fail_and_raise_executor", None, ignore_return_code=True, timeout=10.0)
            found = False
            for line in stderr:
                line = line.val.lower()
    ...
    def test_save_all_figures_fail_no_raise(self):
        ...
        if os.name == "nt":
            ...
            stdout = st.run(f"{sys.executable} {__file__} --funcname save_all_figures_fail_no_raise_executor", ignore_return_code=True, timeout=10.0)
    ...
    def test_save_all_figures_notimeout_100(self):
        ...
        figure_control = rcfg.RenderControlFigure(tile_array=(1,1), tile_square=True)
        fig_record = fm.setup_figure(figure_control, name=name, code_tag=f"{__file__}.test_save_all_figures_notimeout_100()")
        view = fig_record.view
        line = list(range(100))
        view.draw_p_list(line)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        ...
        description = 'Testing figure management')
    parser.add_argument('--funcname', help="Calls the given function")
    args = parser.parse_args()
    func_name = args.funcname

    if func_name != None and func_name in [ "save_all_figures_fail_and_raise_executor", "save_all_figures_fail_no_raise_executor" ]:
