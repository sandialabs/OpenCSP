...
class test_VideoHandler(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.frame_control = rcvf.RenderControlVideoFrames(inframe_format="png", outframe_name="-%05d", outframe_format="png")
    ...
    def test_extract_example_frames(self):
        ...
        self.assertTrue(nframes == 1 or nframes == 2, f"Unexpected number of example frames. Expected either 1 or 2 from a 1s video clip. Found {nframes} in {example_dir}.")
    

    def test_construct_video(self):
        dst_video_dir_name_ext = os.path.join(self.dir_out, "test_construct_video3.mp4")
        handler = vh.VideoHandler.VideoCreator(self.dir_in, dst_video_dir_name_ext, self.video_control, self.frame_control)
        self.set_output_dirs(handler)
