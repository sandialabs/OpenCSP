...
class PlotAnnotation():
    ...
    def __init__(self,
                 ...
                 style):   # Appropriate for type, e.g, RenderControlText or RenderControlPointSeq.
                ...
                if len(pt0) == 2:
                    ...
                    if (len(cropped_pt_list) < len(self.pt_list)) and \
                       ((isinstance(self.style, type(rcps.default()))) and (self.style.linestyle != 'None')):
                        print('WARNING: In PlotAnnotation.plot(), current implementation of point cropping does not ensure proper drawing of lines connecting points.')
                    plt.plot([pt[0] for pt in cropped_pt_list],
                    ...
                    plt.plot([pt[0] for pt in self.pt_list],
                    ...
                    print('ERROR: In PlotAnnotation.plot(), when plotting a point sequence, unexpected point length len(pt0)="'+str(len(pt0))+'" encountered.')
                    assert False
        ...
        # Text.
            ...
            if (self.pt_list != None) and (len(self.pt_list) > 0):
                ...
                if len(pt) == 2:
                    ...
                    print('ERROR: In PlotAnnotation.plot(), when plotting text, unexpected point length len(pt)="'+str(len(pt))+'" encountered.')
                    assert False
    ...
    def image_draw(self, 
                   ...
                   crop_box=None):  # OpenCV drawing routines automatically crop to image boundaries.
        ...
        if self.type == 'point_seq':
            if (self.pt_list != None) and (len(self.pt_list) > 0):
                pt0 = self.pt_list[0]  # Sample for length check.
                if False:  #len(pt0) != 2:  # ?? SCAFFOLDING RCB -- THERE IS A BUG IN 180_heliostat3d.py, __init()__, WHERE THE VARIABLE list_of_frame_id_xy_lists_to_process contains elements [-1.0, -1, 0] FOR MISSING POINTS.  I DON'T HAVE TIME RIGHT NOW TO FIND THIS AND FIX IT, SO IGNORE TEMPORARILY.
                    print('ERROR: In PlotAnnotation.image_draw(), when drawing a point sequence, unexpected point length len(pt0)="'+str(len(pt0))+'" encountered.')
                    assert False
                else:
                    ...
                    if (len(cropped_pt_list) < len(self.pt_list)) and \
                       ((isinstance(self.style, type(rcps.default()))) and (self.style.linestyle != 'None')):
                        print('WARNING: In PlotAnnotation.image_draw(), current implementation of point cropping does not ensure proper drawing of lines connecting points.')
                    # Connecting lines.
                    if (len(cropped_pt_list) > 1) and (self.style.linestyle != 'None'):
                        if ((self.style.linestyle != '-') and (self.style.linestyle != 'solid')):
                            print('WARNING: In PlotAnnotation.image_draw(), dashed or dotted lines are not implemented yet.  Drawing a solid line instead.')
                    ...
                    if (self.style.marker != 'None'):
                        for int_or_float_pt in cropped_pt_list:
                            ...
                            radius = int(self.style.markersize/2)  # ?? SCAFFOLDING RCB -- LATER, REPLACE WITH "image_marker_size"
                            if radius == 0:
                                radius = 1
                            marker = self.style.marker
                            if marker == '.':
                                ...
                                thickness = int(self.style.markeredgewidth)  # ?? SCAFFOLDING RCB -- LATER, REPLACE WITH "image_marker_edge_width"
                            else:
                                print('WARNING: In PlotAnnotation.image_draw(), marker type "' + str(marker) + '" is not implemented yet.  Drawing filled circle instead.')
                                plot_color = self.style.markerfacecolor
                                thickness = -1  # Filled
                    ...
                    if ((self.text is not None) and (len(self.text) > 0)):
                        print('WARNING: In PlotAnnotation.image_draw(), drawing legend labels for points is not implemented yet.')
        ...
        # Text.
            ...
            if (self.pt_list != None) and (len(self.pt_list) > 0):
                ...
                if len(int_or_float_pt0) != 2:
                    print('ERROR: In PlotAnnotation.image_draw(), when drawing a text string, unexpected point length len(int_or_float_pt0)="'+str(len(int_or_float_pt0))+'" encountered.')
                    assert False
                else:
                    ...
                    if self.point_is_in_crop_box(int_pt0, crop_box):
                        ...
                        origin = self.opencv_origin(self.text, int_pt0, font, font_scale, font_thickness, 
                                                    self.style.horizontalalignment, self.style.verticalalignment)
                        # Draw text.
    ...
    def opencv_color(self, plot_color):
        ...
        if (plot_color == 'k') or (plot_color == 'black'):
            ...
            print('WARNING: In PlotAnnotation.opencv_color(), plot_color "' + str(plot_color) + '" is not implemented yet.  Using medium grey instead.')
            return (127, 127, 127)
    ...
    def opencv_font(self, plot_fontstyle):
        ...
        if plot_fontstyle == 'normal':
            ...
            print('ERROR: In PlotAnnotation.opencv_font(), unexpected plot_fontstyle="' + str(plot_fontstyle) + '" encountered.')
            assert False
        return font
    ...
    def opencv_font_scale(self, plot_fontsize):
        ...
        if mt.is_number(plot_fontsize):
            ...
            print('ERROR: In PlotAnnotation.opencv_font_scale(), unexpected plot_fontsize="' + str(plot_fontsize) + '" encountered.')
            assert False
        return scale
    ...
    def opencv_font_thickness(self, plot_fontweight):
        ...
        if plot_fontweight == 'light':
            ...
            print('ERROR: In PlotAnnotation.opencv_font_thickness(), unexpected plot_fontweight="' + str(plot_fontweight) + '" encountered.')
            assert False
        return thickness


    def opencv_origin(self, text, int_pt, font, font_scale, font_thickness, plot_horizontalalignment, plot_verticalalignment):
        ...
        if (plot_horizontalalignment == 'left'):
            ...
            print('ERROR: In PlotAnnotation.opencv_origin(), unexpected plot_horizontalalignment="' + str(plot_horizontalalignment) + '" encountered.')
            assert False
        # Compute origin y.
        if (plot_verticalalignment == 'bottom'):
            ...
            print('ERROR: In PlotAnnotation.opencv_origin(), unexpected plot_verticalalignment="' + str(plot_verticalalignment) + '" encountered(1).')
            assert False
        # Return.
...
def text_annotation(point, 
    ...
    A text annotation.
    """
    return PlotAnnotation('text', 
                          [point],
                          text_str,
                          rctxt.RenderControlText(fontsize=fontsize, 
                                                  color=color,
