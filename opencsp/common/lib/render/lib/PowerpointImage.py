...
class PowerpointImage(pps.PowerpointShape):
    ...
    def set_val(self, image: str|np.ndarray|Image.Image|rcfr.RenderControlFigureRecord):
        ...
        if isinstance(self._val, str):
            ...
            if not ft.file_exists(self._val, error_if_exists_as_dir=False):
                lt.warn(f"Warning: PowerpointImage.__init__: reference type value \"{self._val}\" should be a path to an image file, but no such file exists!")
    ...
    def _test_saved_path(self):
        ...
        if ft.path_to_cmd_line(self.get_saved_path()) == ft.path_to_cmd_line(str(self._val)):
            if "tmp" in str(self._val):
                ...
                lt.warn(f"Image val and save path are the same:\n\tval: {self._val}\n\tsave path: {self.get_saved_path()}")
    ...
    @staticmethod
    def _image_dims_relative_to_cell(cell_dims: tuple[float,float,float,float], image_width: int, image_height: int, stretch=False):
    ...
    def _save(self, path_name_ext: str):
        ...
        if isinstance(self._val, Image.Image):
            ...
            lt.error_and_raise(ValueError, f"Error: in PowerpointImage.save: unrecognized type for image " +
    ...
    def _to_txt_file(self):
        ...
        with open(path_name_ext, "w") as fout:
            ...
            fout.write("v1\n")
            for v in [self.has_val(), image_name_ext, self._dims_to_str(self.dims), self._dims_to_str(self.cell_dims),
                      self.caption_is_above, self.caption, self.caption==None, self.stretch]:
                fout.write(f"{v}\n")
    ...
    def from_txt_file(cls, path_name_ext: str):
        ...
        if file_type != "PowerpointImage":
            lt.error_and_raise(RuntimeError, f"Error: in PowerpointImage.from_txt_file(), bad file type {file_type} in {path_name_ext}, expected type PowerpointImage")
        version = slines[1]
        if version != "v1":
            lt.error_and_raise(RuntimeError, f"Error: in PowerpointImage.from_txt_file(), bad version {version} in {path_name_ext}, expected version v1")
    ...
    def save(self):
        ...
        if not found_unused_name:
            lt.error_and_raise(RuntimeError, "Failed to find an empty spot to save this image to. Try using PowerpointImage.clear_tmp_save_all() to make more room.")
        ...
        if saved_path != self._tmp_save_path:
            lt.error_and_raise(RuntimeError, f"Error: in PowerpointImage.save(): programmer error, " +
