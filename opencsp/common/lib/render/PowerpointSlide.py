...
class PowerpointSlide():
    def __init__(self, slide_control: RenderControlPowerpointSlide,
    ...
    @staticmethod
    def _get_cell_dims(table_dims: tuple[float,float,float,float], inter_cell_buffer_hor: float, inter_cell_buffer_vert: float, nrows: int, ncols: int, row_idx: int, col_idx: int):
        if row_idx >= nrows or col_idx >= ncols:
            lt.error_and_raise(ValueError, f"Error: in PowerpointSlide._get_cell_dims: row_idx ({row_idx}) or col_idx ({col_idx}) outside row/col bounds ({nrows}/{ncols})!")
        x, y, w, h = table_dims
    ...
    @classmethod
    def template_content_grid(cls, nrows=2, ncols=2, slide_control: RenderControlPowerpointSlide=None) -> "PowerpointSlide":
        ...
        get_cell_dims = lambda row_idx, col_idx: cls._get_cell_dims(content_dims, inter_cell_buffer, inter_cell_buffer+caption_size, nrows, ncols, row_idx, col_idx)
    ...
    def _next_empty_cell_index(self, which_shapes='any'):
        if which_shapes == 'any':
            ...
            lt.error_and_raise(ValueError, f"Error: in PowerpointSlide._next_empty_cell_index(), programmer error invalid value '{which_shapes}' for which_shapes argument")
    ...
    def add_image(self,
        ...
        if isinstance(image, PowerpointImage):
            ...
            lt.error_and_raise(ValueError, "Error: in PowerpointSlide.add_image(), can't add a 'None' type image to a slide.")
        else:
        ...
        for pps_text in texts:
            if pps_text.has_val():
                if not pps_text.has_dims():
                    lt.warn(f"Warning: text \"{pps_text.get_val()}\" does not have a location set!\n\tOrigin: {pps_text.code_location}")
    ...
    def from_txt_file(cls, file_path_name_ext: str, slide_control: RenderControlPowerpointSlide=None):
        ...
        if len(lines) < 2:
            lt.error_and_raise(RuntimeError, f"Error: in PowerpointSlide.from_txt_file(), not enough lines in file {file_path_name_ext}")
        
        file_type = lines[0]
        if file_type != "PowerpointSlide":
            lt.error_and_raise(RuntimeError, f"Error: in PowerpointSlide.from_txt_file(), bad file type {file_type} in {file_path_name_ext}, expected type PowerpointSlide")
        version = lines[1]
        if version != "v1":
            lt.error_and_raise(RuntimeError, f"Error: in PowerpointSlide.from_txt_file(), bad version {version} in {file_path_name_ext}, expected version v1")
        nimages = int(lines[2])
        ntexts = int(lines[3])
