...
class VideoHandler():
    _video_extensions = ["mp4", "avi", "wmv", "ogv", "4xm", "amv", "mtv", "m4v", "flv", "f4v", "f4p", "f4a", "f4b", "vlb", "m4v", "mpg", "mp2", "mpeg", "mpe", "mpv", "m4v", "webm", "mov", "qt", "rm", "rmvb", ]
    """ List of video formats that ffmpeg supports (probably not comprehensive, maybe some errors) """
    
    
    def __init__(self, src_video_dir_name_ext: str=None, dst_video_dir_name_ext: str=None,
                 src_frames_dir: str=None, dst_frames_dir: str=None, dst_example_frames_dir: str=None,
                 video_control: rcv.RenderControlVideo=None, frame_control: rcvf.RenderControlVideoFrames=None):
        ...
        self.dst_frames_dir = dst_frames_dir if dst_frames_dir != None else os.path.join(ft.default_output_path(), "output_frames")
        self.dst_example_frames_dir = dst_example_frames_dir if dst_example_frames_dir != None else os.path.join(ft.default_output_path(), "example_frames")
    ...
    @classmethod
    def VideoCreator(cls, src_frames_dir: str, dst_video_dir_name_ext: str, video_control: rcv.RenderControlVideo, frame_control: rcvf.RenderControlVideoFrames):
        return cls(dst_video_dir_name_ext=dst_video_dir_name_ext, src_frames_dir=src_frames_dir, video_control=video_control, frame_control=frame_control)

    @classmethod
    def VideoMerger(cls, src_videos_path, src_videos_ext, dst_video_dir_name_ext):
        return cls(src_video_dir_name_ext=os.path.join(src_videos_path, f"NA.{src_videos_ext}"), dst_video_dir_name_ext=dst_video_dir_name_ext)

    @classmethod
    def VideoTransformer(cls, src_video_dir_name_ext: str, dst_video_dir_name_ext: str, video_control: rcv.RenderControlVideo):
        return cls(src_video_dir_name_ext=src_video_dir_name_ext, dst_video_dir_name_ext=dst_video_dir_name_ext, video_control=video_control)

    @classmethod
    def VideoExtractor(cls, src_video_dir_name_ext: str, dst_frames_dir: str, dst_example_frames_dir: str, frame_control: rcvf.RenderControlVideoFrames):
        return cls(src_video_dir_name_ext=src_video_dir_name_ext, dst_frames_dir=dst_frames_dir, dst_example_frames_dir=dst_example_frames_dir, frame_control=frame_control)
    ...
    def _build_ffmpeg_cmd(self, args_str: str, paths: dict[str,str]=None):
        ...
        for path_key in paths.keys():
            ...
            if match in args_str:
                ...
                lt.debug(f"VideoHandler::_build_ffmpeg_cmd: could not find matches to path {match}:{path} in {args_str}")
    ...
    def extract_frames(self, start_time: float=None, end_time: float=None):
        ...
        if not ft.file_exists(self.src_video_dir_name_ext):
            lt.error_and_raise(FileNotFoundError, 'ERROR: In VideoHandler.extract_frames(), src_video_dir_name_ext does not exist: "' + str(self.src_video_dir_name_ext) + '"')
        if (start_time != None and end_time != None) and (end_time < start_time):
            lt.error_and_raise(ValueError, f"Error: in VideoHandler.extract_frames(), end_time < start_time ({end_time} < {start_time})")
        ...
        for run in ["example", "output"]:
            ...
            ffmpeg_args, paths = self.frame_control.get_ffmpeg_args(frame_dir, self.src_video_dir_name_ext, is_example_frames)
            ...
            if run == "output":
                ...
                n_frames = ft.count_items_in_directory(frame_dir,
                                                       name_prefix=input_video_body,
                                                       name_suffix=frame_ext)
    ...
    def identify_duplicate_frames(self, tolerance_image_size: int, tolerance_image_pixel: int):
        ...
        if not ft.directory_exists(self.src_frames_dir):
            lt.error_and_raise(RuntimeError, 'ERROR: In identify_duplicate_frames(), src_frames_dir does not exist: "' + str(self.src_frames_dir) + '"')
        ...
        if n_input_frames < 2:
            ...
            for this_frame_file_size_pair in input_frame_file_size_pair_list[1:]:
                if self._this_frame_is_a_duplicate_of_previous(previous_frame_file_size_pair, this_frame_file_size_pair, 
                                                               tolerance_image_size, tolerance_image_pixel):
    ...
    def _this_frame_is_a_duplicate_of_previous(self,
    ...
    def _frames_are_identical(self, previous_frame_file: str, this_frame_file: str, tolerance_image_pixel: int):
        ...
        """
        # Load frames.
        previous_dir_body_ext = os.path.join(self.src_frames_dir, previous_frame_file)  # Already includes the extension.
    ...
    def _files_list_to_video(self, src_dir: str, src_names_exts: list[str], dst_video_dir_name_ext: str, tmp_dir: str=None, overwrite: bool=False, widthheight_vidorimg_dir_name_ext=""):
    ...
    def frames_to_video(self, frame_names: list[str], tmp_dir: str=None, overwrite: bool=False):
        ...
        try:
            ...
            # build the video
            self._files_list_to_video(self.src_frames_dir, frame_names, self.dst_video_dir_name_ext, tmp_dir, overwrite, img0_dir_path_ext)

            return self.dst_video_dir_name_ext
        except Exception:
            lt.error(f"Error: in VideoHandler.frames_to_video: failed to create video \"{self.dst_video_dir_name_ext}\"{frame_names_msg}")
            raise
    ...
    def merge_videos(self, src_video_names: list[str]=None, tmp_dir: str=None, overwrite: bool=False):
        ...
        self._files_list_to_video(src_video_dir, src_video_names, self.dst_video_dir_name_ext, tmp_dir, overwrite, src_video0_dir_name_ext)
    ...
    def transform_powerpoint(cls, src_video_dir_name_ext: str, dst_dir: str=None, overwrite: bool=False):
        ...
        handler = cls(src_video_dir_name_ext=src_video_dir_name_ext, dst_video_dir_name_ext=dst_video_dir_name_ext, video_control=video_control)
    ...
    def get_width_height(self, input_or_output="input"):
        ...
        lines = subt.run(f"ffprobe -v error -select_streams v:0 -show_entries stream=width,height -of csv=s=x:p=0 {path}")
        line = lines[0].val
        parts = line.split("x")
    ...
    def get_duration(self, input_or_output="input"):
        ...
        path = self.src_video_dir_name_ext if input_or_output == "input" else self.dst_video_dir_name_ext
        path = ft.path_to_cmd_line(path)
        lines = subt.run(f"ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 {path}")
        line = lines[0].val
    ...
    def get_num_frames(self, input_or_output="input"):
        ...
        path = self.src_video_dir_name_ext if input_or_output == "input" else self.dst_video_dir_name_ext
        path = ft.path_to_cmd_line(path)
        lines = subt.run(f"ffprobe -v error -select_streams v:0 -count_packets -show_entries stream=nb_read_packets -of csv=p=0 {path}")
        line = lines[0].val
