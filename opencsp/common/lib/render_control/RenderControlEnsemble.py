...
class RenderControlEnsemble():
    ...
    def add_special_name(self, input_name: str|float, style: any):
        ...
        for entry in self.special_style_entries:
            if name in entry[0]:
                print('In RenderControlEnsemble.add_special_name(), name="' + str(name) + '" already has a special style defined.')
                assert False
    ...
    def add_special_names(self, input_names: Iterable[str|float], style: any):
        ...
        for entry in self.special_style_entries:
            for name in names:
                if name in entry[0]:
                    print('In RenderControlEnsemble.add_special_names(), name="' + str(name) + '" already has a special style defined.')
                    assert False
