...
class RenderControlPowerpointPresentation():
    def __init__(self, new_slides: list[pps.PowerpointSlide]=None, existing_presentation_path_name_ext: str=None):
        ...
        if existing_presentation_path_name_ext != None:
            lt.error_and_raise(NotImplementedError, "Error: in RenderControlPowerpointPresentation, support for use of existing presentations as a template not yet tested")
    ...
    def save(self, dest_path_name_ext: str, overwrite=False):
        ...
        if ft.file_exists(dest_path_name_ext):
            if not overwrite:
                lt.error_and_raise(FileExistsError, f"Error: in RenderControlPowerpointPresentation.save: trying to save presentation to \"{dest_path_name_ext}\" but the file already exists")
        
        dest_path, _, _ = ft.path_components(dest_path_name_ext)
        # check if the directory exists
        if not ft.directory_exists(dest_path):
            lt.error_and_raise(FileNotFoundError, f"Error: in RenderControlPowerpointPresentation.save: destination directory \"{dest_path}\" doesn't exist")

        # setup
        tmp_dir = os.path.join(orp.opencsp_temporary_dir(), "powerpoint_presentations", tdt.current_date_time_string_forfile())
        ...
        for slide_list_idx, pps_slide in enumerate(self.new_slides):
            ...
            if control.slide_index < 0 or control.slide_index == slide_list_idx:
                ...
                raise NotImplementedError("Insertion of slides at a specific index hasn't been implemented in python-pptx (as of 06/28/2023)")
        ...
        if ft.file_exists(dest_path_name_ext):
            ...
            if not overwrite:
                lt.error_and_raise(RuntimeError, f"Reached unreachable code!!! (file {dest_path_name_ext} already exists)")
