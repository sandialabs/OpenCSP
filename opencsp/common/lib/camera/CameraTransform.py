...
class CameraTransform():
    ...
    def construct_rotation_matrix(self):
        ...
        xf = self.transform
        return np.array([ [xf[0][0], xf[0][1], xf[0][2]],
    ...
    def pq_or_none(self, camera, xyz):
        ...
        if distance < 1e-6:  # Fuzzy tolerance for on or behind plane.
            ...
            obj_points = np.float64([xyz])  # float 64 required by OpenCV.  Can also "obj_points = obj_points.astype('float64')"
            open_cv_img_points, jacobian_project = cv.projectPoints(obj_points, 
                                                                    self.rvec, self.tvec,
            ...
            if len(open_cv_img_points) != 1:
                print('ERROR: In CameraTransform.pq_or_none(), open_cv_img_pts = "' + str(open_cv_img_points) + '" was not of length 1.')
                assert False
            nested_img_pt = open_cv_img_points[0]
            if len(nested_img_pt) != 1:
                print('ERROR: In CameraTransform.pq_or_none(), nested_img_pt = "' + str(nested_img_pt) + '" was not of length 1.')
                assert False
