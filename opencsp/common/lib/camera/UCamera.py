...
class Camera(csvi.CsvInterface):
    ...
    def __init__(self,
        ...
        self.focal_length_min = focal_lengths_mm[0] / 1000.0  # m.  Minimum focal length.  For fixed focal length min == max.
        self.focal_length_max = focal_lengths_mm[1] / 1000.0  # m.  Maximum focal length.  For fixed focal length min == max.
        # Dependent parameters.
        self.fov_vertical_min   = 2.0 * math.atan((self.sensor_y/2.0) / self.focal_length_max)  # Angular field of view of the camera, in the vertical direction.
        self.fov_vertical_max   = 2.0 * math.atan((self.sensor_y/2.0) / self.focal_length_min)  #
        self.fov_horizontal_min = 2.0 * math.atan((self.sensor_x/2.0) / self.focal_length_max)  # Angular field of view of the camera, in the horizontal direction.
    ...
    def csv_header(delimeter=","):
        return delimeter.join(["name","sensor_w","sensor_h","pixels_x","pixels_y","focal_length_min","focal_length_max","fov_vert_min","fov_vert_max","fov_hor_min","fov_hor_max"])
    ...
    def to_csv_line(self, delimeter=","):
        return delimeter.join([str(v) for v in [
...
def sony_alpha_20mm_landscape():
    return Camera(name='Sony Alpha, 20mm',
                   sensor_x_mm=35.9,          # mm.
                   sensor_y_mm=24.0,          # mm.
...
def ultra_wide_angle():
    return Camera(name='Ultra Wide Angle',
                   sensor_x_mm=24.0,        # mm.
                   sensor_y_mm=35.9,        # mm.
                   pixels_x=4864,           # For still images.
                   pixels_y=8760,           # For still images.
...
class RealCamera(csvi.CsvInterface):
    ...
    We will probably need to extend this model going forward.
    """
    ...
    @staticmethod
    def csv_header(delimeter=","):
        return delimeter.join(["name","pix_x","pix_y","focal_length_x","focal_length_y","center_pix_x","center_pix_y","radial_1","radial_2","tangential_1","tangential_2"])
...
def ideal_camera_wide_angle():
    return RealCamera(name='Ideal Camera, Wide Angle',
                       # Image size.
                       n_x=4,       # Pixels.
                       n_y=3,       # Pixels.
                       # Focal length.
...
def ideal_camera_normal():
    ...
    #
    return RealCamera(name='Ideal Camera, Normal',
                       # Image size.
                       n_x=4,       # Pixels.
                       n_y=3,       # Pixels.
                       # Focal length.
...
def ideal_camera_telephoto():
    return RealCamera(name='Ideal Camera, Telephoto',
                       # Image size.
                       n_x=4,       # Pixels.
                       n_y=3,       # Pixels.
                       # Focal length.
...
def real_mavic_zoom():
    ...
    return RealCamera(name='Real Mavic Zoom',
                       # Image size.
                       n_x=3840,           # Pixels.
                       n_y=2160,           # Pixels.
                       # Focal length.
