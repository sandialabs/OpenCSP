...
def opencsp_doc_dir():
    ...
    return os.path.join(opencsp_code_dir(), '..', 'doc')  # TODO BGB: will this always live next to the 'opencsp' directory?
...
def opencsp_scratch_dir(project_dir=None) -> str:
    ...
    if os.name == "nt":
        ...
        actual_scratch_dir = os.path.join(opencsp_code_dir(), '..', opencsp_settings["opencsp_root_path"]["scratch_name"])
        actual_scratch_dir = actual_scratch_dir if project_dir == None else os.path.join(actual_scratch_dir, project_dir)
        ...
        actual_scratch_dir = os.path.join(os.path.expandvars("%LOCALAPPDATA%"), "opencsp", opencsp_settings["opencsp_root_path"]["scratch_name"])
        return actual_scratch_dir if project_dir == None else os.path.join(actual_scratch_dir, project_dir)
    else:
