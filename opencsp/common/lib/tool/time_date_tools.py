...
def tz(name_or_offset: str|float|timedelta):
    ...
    if len(partial_matches) == 1:
        ...
        lt.error_and_raise(RuntimeError, f"Error: in time_date_tools.tz(), ambiguous timezone name {name} has multiple possible matches: {partial_matches}")
...
def add_seconds_to_ymdhmsz(ymdhmsz:list[int,int,int,int,int,float,int], time_sec:float, ignore_legacy=None) -> list[int,int,int,int,int,float,int]:
    ignore_legacy = tdt_ignore_legacy if (ignore_legacy == None) else ignore_legacy
    if ignore_legacy == False:
        print("subtract_seconds_from_ymdhmsz is a legacy function. Consider using \"to_datetime() + datetime.timedelta(0,s)\" instead")
...
def subtract_seconds_from_ymdhmsz(ymdhmsz:list[int,int,int,int,int,float,int], time_sec:float, ignore_legacy=None) -> list[int,int,int,int,int,float,int]:
    ignore_legacy = tdt_ignore_legacy if (ignore_legacy == None) else ignore_legacy
    if ignore_legacy == False:
        print("subtract_seconds_from_ymdhmsz is a legacy function. Consider using \"to_datetime() - datetime.timedelta(0,s)\" instead")
    # Parse the ymdhmsz object.
        ...
        if minute >= 1:
            ...
            if hour >= 1:
                ...
                if day >= 1:
                    ...
                    print('ERROR: In subtract_seconds_from_ymdhms(), rolling over a month boundary not implemented yet.')
                    assert False
    # Return.
    return [year, month, day, hour, minute, second, zone]