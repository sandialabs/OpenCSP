...
class TestLogTools(unittest.TestCase):
    ...
    def proc_exec(self, func_name1: str, func_name2: str=None):
        with subprocess.Popen([sys.executable, __file__, "--funcname", func_name1, "--logname", self.log_dir_body_ext]):
            if func_name2 != None:
                with subprocess.Popen([sys.executable, __file__, "--funcname", func_name2, "--logname", self.log_dir_body_ext]):
                    pass
    ...
    def test_single_process_dont_delete(self):
        ...
        log_contents = self.get_log_contents()
        self.assertTrue("Hello, world!" in log_contents, f"Can't find hello log in log contents:\n\t\"{log_contents}\"")
        self.assertTrue("Goodbye, world!" in log_contents, f"Can't find goodbye log in log contents:\n\t\"{log_contents}\"")
    ...
    def test_log_level_screening(self):
        ...
        log_contents = self.get_log_contents()
        self.assertTrue("Hello, world!" in log_contents, f"Can't find hello log in log contents:\n\t\"{log_contents}\"")
        self.assertFalse("Goodbye, world!" in log_contents, "Found goodbye log in log contents when it shouldn't be there")
    ...
    def test_error_and_raise(self):
        ...
        self.assertTrue("RuntimeError encountered" in log_contents, f"Can't find evidence of RuntimeError in log contents:\n\t\"{log_contents}\"")
