...
// Test Crescent Dunes configuration
TEST(PresetSites, CrescentDunes_Configuration) {
    preset_site_config config;
    std::string aimFile;

    bool result = get_preset_site_config(site_crescentDunes, aim_point, &config, &aimFile);

    EXPECT_TRUE(result);

    // Crescent Dunes has ~10,400 heliostats
    EXPECT_TRUE(config.nHelios >= 10000);

    // Nevada location (around 38.2N, 117.4W)
    EXPECT_TRUE(config.coords.x > 37.0f);
    EXPECT_TRUE(config.coords.x < 40.0f);
    EXPECT_TRUE(config.coords.y < -116.0f);
    EXPECT_TRUE(config.coords.y > -118.0f);
