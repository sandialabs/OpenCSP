...
bool get_preset_site_config(int siteId, const int aimId, preset_site_config* config, std::string* aimFile)
{
    ...
    *aimFile = "";
    ...
    else if (site == site_crescentDunesSmall)
    {
        // Small 1 km annular slice
        std::cout << fmt::format("Using CRESCENT DUNES (1 km) field layout\n");
        config->coords.x = 38.2389f;
        config->coords.y = -117.36346f;
        config->timezone = -8;
        ...
        config->facetFile = "data/Crescent_Dunes_Facet_Centroids.csv";
        config->helioFile = "data/Crescent_Dunes_Heliostats_Small.csv";
        config->nHelios = 80 * 80;
        config->radius = 1000;
    }
    else if (site == site_crescentDunes)
    {
        std::cout << fmt::format("Using CRESCENT DUNES field layout\n");
        config->coords.x = 38.2389f;
        config->coords.y = -117.36346f;
        config->timezone = -8;
        config->towerHeight = 100.0f;
        ...
        config->facetDims.y = 1.8f;
        config->facetFile = "data/Crescent_Dunes_Facet_Centroids.csv";
        config->helioFile = "data/Crescent_Dunes_Heliostats.csv";
        config->nHelios = 10348;
        config->radius = 1600;
    }
