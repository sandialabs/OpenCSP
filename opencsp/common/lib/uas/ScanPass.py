...
class ScanPass():
    ...
    def set_waypoints_with_margin(self, scan_parameters):
        ...
        localeN = wptN.locale
        if locale0 != localeN:
            print('In ScanPass.set_waypoints_with_margin(), mismatched locale0=' + str(locale0)+' and localeN=' + str(localeN)+'.')
            assert False
        # Fetch theta.
        theta0 = wpt0.theta
        thetaN = wptN.theta
        if theta0 != thetaN:
            print('In ScanPass.set_waypoints_with_margin(), mismatched theta0=' + str(np.rad2deg(theta0))+' and thetaN=' + str(np.rad2deg(thetaN))+'.')
            assert False
        # Fetch speed.
        speed0 = wpt0.speed
        speedN = wptN.speed
        if speed0 != speedN:
            print('In ScanPass.set_waypoints_with_margin(), mismatched speed0=' + str(speed0)+' and speedN=' + str(speedN)+'.')
            assert False
        ...
        if len(self._core_waypoint_list) == 2:
            waypoint_list = [start_wpt, end_wpt]  # If there are only two waypoints, keep this property to avoid potential stutters.
        else:
...
def construct_scan_pass_given_segment_of_interest(segment_xyz,
                                                  fly_backward,
                                                  scan_parameters):
    scan_pass = ScanPass()
...
def construct_scan_pass_given_UFACET_scan_pass(ufacet_scan_pass,  # UfacetScanPass object.
                                               fly_backward,
                                               scan_parameters):
    # Notify progress.
    print('Constructing UFACET scan pass '+ufacet_scan_pass.heliostat_name_list[0]+'-'+ufacet_scan_pass.heliostat_name_list[-1]+'...')
