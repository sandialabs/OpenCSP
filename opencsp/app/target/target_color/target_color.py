...
if __name__ == "__main__":
    ...
    output_dir = os.path.join('app', 'target', 'target_color', 'test', 'data', 'output', source_file_body) # ?? SCAFFOLDING RCB -- ADD CODE TO CREATE DIRECTORY IF NECESSARY.
    output_ext = '.png'
...
#    dpi = 100
    ...
    target_color_1d_gradient.linear_color_bar('discrete', color_bar, color_below_min, color_above_max, img, dpi, output_dir, output_ext)

    # Continuous linear color bar
    target_color_1d_gradient.linear_color_bar('continuous', color_bar, color_below_min, color_above_max, img, dpi, output_dir, output_ext)
...
#    frame_width_mm  = 10.0  # millimeters.
    ...
    target_diameter_m = 8.0 * target_design_focal_length_m * target_design_err_max_rad  # Required target diameter is d=8*f*err_max.
...
#    board_height_in = 90.0  # inch.  Current plascore is cut to 90" length.
    ...
    up_over_board_in = board_half_height_in     \
                       + top_roller_radius_in   \
                       + top_roller_half_arc_in \
                       + top_roller_radius_in   \
                       + board_half_height_in # Distance to go from board center up over the roller, back to board center.
    ...
    wrap_distance_when_unfurled_in = num_turns_when_unfurled * (np.pi * spool_diameter_in)  # Length around spool when target installed.
    leader_margin_in = up_over_board_in - (target_img_height_in / 2.0)  # Leader required to go up over board and back to board center.
    ...
    # Generate image #1.
    alignment_line_start_x, \
    dummy = bullseye_color_bar('discrete', cx_offset_1_pix, y_offset_pix,
                               target_design_focal_length_m, target_design_err_min_mrad, target_design_err_max_mrad, 
                               color_below_min, color_above_max, color_frame, frame_width_pix, 
                               True, cx_offset_2_pix,
                               False, -999, -999)  # Dummy value, ignored if not printing trim line.

    # Generate image #2.
    dummy, \
    trim_line_start_x = bullseye_color_bar('discrete', cx_offset_2_pix, y_offset_pix,
                                           target_design_focal_length_m, target_design_err_min_mrad, target_design_err_max_mrad, 
                                           color_below_min, color_above_max, color_frame, frame_width_pix, 
                                           False, -999,  # Dummy value, ignored if not printing alignment line.
                                           True, cx_offset_1_pix, target_img_width_pix)
