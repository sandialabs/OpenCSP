...
def bullseye_color_bar(discrete_or_continuous, cx_offset_pix, y_offset_pix, 
                       focal_length_meter, r_min_mrad, r_max_mrad, 
                       color_below_min, color_above_max, color_frame, frame_width_pix, 
                       draw_alignment_line, cx_offset_alignment_pix,
                       draw_trim_line, cx_offset_trim_pix, target_img_width_pix):
    # Bullseye.
    for row in range(0,n_rows):
        for col in range(0,n_cols):
            r_mrad = bullseye_error.radius_in_mrad_given_row_col(row, col, x_max, cx_offset_pix, y_offset_pix, focal_length_meter)
            color = color_convert.color_given_value(r_mrad, r_min_mrad, r_max_mrad, color_bar, discrete_or_continuous, color_below_min, color_above_max)
            # Set pixel color
            img[row,col,0] = color[0]/255.0
            img[row,col,1] = color[1]/255.0
            img[row,col,2] = color[2]/255.0
