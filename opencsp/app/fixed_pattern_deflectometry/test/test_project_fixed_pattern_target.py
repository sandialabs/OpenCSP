...
def test_project_fixed_pattern_target():
    ...
    file_image_projection = os.path.join(os.path.dirname(opencsp.__file__), "test/data/sofast_measurements/general/Image_Projection_test.h5")

    # Load ImageProjection
    im_proj = ImageProjection.load_from_hdf_and_display(file_image_projection)

    fixed_pattern = FixedPatternScreenProjection(im_proj.size_x, im_proj.size_y, width_pattern=3, spacing_pattern=6)
