...
def visualize_setup(display: Display,
                    camera: Camera,
    ...
    lx1 = max(np.nanmax(np.concatenate((v_screen_cam_screen.x, v_cam_fov_screen.x, p_screen_outline.x, obj_x))), min_axis_length_screen)
    ly1 = max(np.nanmax(np.concatenate((v_screen_cam_screen.y, v_cam_fov_screen.y, p_screen_outline.y, obj_y))), min_axis_length_screen)
    lz1 = max(np.nanmax(np.concatenate((v_screen_cam_screen.z, v_cam_fov_screen.z, p_screen_outline.z, obj_z))), min_axis_length_screen)
    # Define negative xyz screen axes extent
    lx2 = min(np.nanmin(np.concatenate((v_screen_cam_screen.x, v_cam_fov_screen.x, p_screen_outline.x, obj_x))), -min_axis_length_screen)
    ly2 = min(np.nanmin(np.concatenate((v_screen_cam_screen.y, v_cam_fov_screen.y, p_screen_outline.y, obj_y))), -min_axis_length_screen)
    lz2 = min(np.nanmin(np.concatenate((v_screen_cam_screen.z, v_cam_fov_screen.z, p_screen_outline.z, obj_z))), -min_axis_length_screen)
    # Add screen axes
    x = p_screen_cent.x[0]
    y = p_screen_cent.y[0]
    z = p_screen_cent.z[0]
