...
class ExampleRayTraceOutput(to.TestOutput):
    ...
    def setUpClass(
        ...
        figure_prefix_root: str = 'trto',  # setUpClass() with no arguments.
        interactive: bool = False,
        verify: bool = True,
    ):
        # Generic setup.
        super(ExampleRayTraceOutput, self).setUpClass(
    ...
    def example_mirror_trace(self) -> None:
        ...
        scene2.add_object(m2)

        trans2 = TransformXYZ.from_R(rot_45_deg)
        scene2.set_position_in_space(m2, trans2)

        trace2 = rt.trace_scene(scene2, Resolution.pixelX(8))
        ...
        m2.draw(view2_yz, mirror_control)
        ...
        ls.incident_rays = LightPath.many_rays_from_many_vectors(None, test_vecs)

        scene3 = Scene()
        scene3.add_light_source(ls)
        scene3.add_object(m3)

        trans3 = TransformXYZ.from_R(rot_id)
        scene3.set_position_in_space(m3, trans3)

        trace3 = rt.trace_scene(scene3, Resolution.pixelX(8))
        ...
        scene4.add_object(m4)

        trans4 = TransformXYZ.from_R(rot_45_deg)
        scene4.set_position_in_space(m4, trans4)

        trace4 = rt.trace_scene(scene4, Resolution.pixelX(8))
        ...
        m4.draw(view4_yz, mirror_control)
    ...
    def example_heliostat_trace(self) -> None:
        ...
        m_flat = MirrorParametricRectangular(flat_func, (1.2192, 1.2192))
        ...
        h_flat, h_flat_loc = HeliostatAzEl.from_csv_files(
            "5W1", dpft.sandia_nsttf_test_heliostats_origin_file(), dpft.sandia_nsttf_test_facet_centroidsfile(), m_flat
        )
        sf_flat = sf.SolarField([h_flat], loc, "mini Nsttf with 5W1")
        sf_flat.set_heliostat_positions([h_flat_loc])
        ...
        h_flat.set_orientation_from_az_el(0, np.pi / 2)
        ...
        trace0 = rt.trace_scene(scene0, obj_resolution=Resolution.pixelX(10))
    ...
    def example_changing_time_of_day(self) -> None:
        ...
        def _heliostat_at_moment(name: str, aimpoint_xyz: Pxyz, when_ymdhmsz: tuple, i: int) -> None:
            ...
            trace_control = RenderControlRayTrace(light_path_control=path_control)
            trace = rt.trace_scene(scene, obj_resolution=Resolution.pixelX(5))
        ...
        # _heliostat_at_moment("5w1 at 12:02", Pxyz([60.0,8.8,28.9]), (2021,5,13,12,2,0,-6), 18)
        _heliostat_at_moment("5w1 at 13:02", Pxyz([60.0, 8.8, 28.9]), (2021, 5, 13, 13, 2, 0, -6), 20)
        # _heliostat_at_moment("5w1 at 14:02", Pxyz([60.0,8.8,28.9]), (2021,5,13,14,2,0,-6), 22)
