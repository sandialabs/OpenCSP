...
def run() -> None:
    ...
    fixed_pattern = SystemSofastFixed(
        image_projection.size_x, image_projection.size_y, width_pattern=3, spacing_pattern=6
    )
    ...
    fixed_pattern_dot_locs = DotLocationsFixedPattern.load_from_hdf(file_fixed_pattern_dot)

    # Define facet measurement setup
    facet_data = DefinitionFacet.load_from_json(file_facet_data)
    surface_data = dict(
