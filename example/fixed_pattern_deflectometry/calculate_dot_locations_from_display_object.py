...
def example_calculate_dot_locs_from_display():
    ...
    dir_base = join(dirname(opencsp.__file__), '../../sample_data/deflectometry/sandia_lab/calibration_files')

    # Define files
    file_display = join(dir_base, 'display_distorted_3d.h5')
    file_image_projection = join(dir_base, 'Image_Projection_optics_lab_landscape_square.h5')
    ...
    projection = FixedPatternScreenProjection(im_proj_params['size_x'], im_proj_params['size_y'], width_dot, spacing_dot)
    fixed_pattern_dot_locs = FixedPatternDotLocations.from_projection_and_display(projection, display)

    # Calculate spatial orientation
    orientation = SpatialOrientation(display.r_cam_screen, display.v_cam_screen_cam)

    # Save data sets
    fixed_pattern_dot_locs.save_to_hdf(join(dir_save, f'fixed_pattern_display_w{width_dot:d}_s{spacing_dot:d}.h5'))
