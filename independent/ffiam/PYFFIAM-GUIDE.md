...
- `ffiam_types.py` exposes `CspSite` (Custom, NSTTF, RadialSmall, Radial, SampleV1/V2/V3), `AimType` (Point, Ring, SplitRing, Vector, CsvData, FixedNormal, Null), and `Direction`.
...
- `presets/`: NSTTF, Radial, RadialSmall, SampleV1/V2/V3
...
Beyond the fixed ~4 GB of pools, `pyffiam` copies the voxel results into NumPy arrays whose size scales with the grid. Full-size sites (hundreds of millions of voxels) can need 32 GB+ of RAM during post-processing, so prefer a smaller site (e.g. `RadialSmall`) on memory-constrained machines.
