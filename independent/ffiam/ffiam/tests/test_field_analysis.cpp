...
TEST(FieldAnalysis, AimCsvRowMismatchReturnsEarly)
{
    ...
    aim.type = aim_data_csv;
    aim.params = {0, 0, 0};
    std::snprintf(aim.file, sizeof(aim.file), "%s", csv.str().c_str());
...
TEST(FieldAnalysis, MissingFacetFileReturnsEarly)
{
    FieldHarness h(4);
    aim_strategy aim = MakePointAim();
    std::snprintf(h.design.facetFile, sizeof(h.design.facetFile), "%s", "/nonexistent/no_such_facet_file.csv");
...
TEST(FieldAnalysis, HelioFileImportOverwritesLocations)
{
    ...
    std::snprintf(h.field.helioFile, sizeof(h.field.helioFile), "%s", csv.str().c_str());
    h.field.radius = 100;
    aim_strategy aim = MakePointAim();
