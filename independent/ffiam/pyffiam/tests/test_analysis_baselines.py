...
class AnalysisBaselineTests(unittest.TestCase):
    ...
    # =========================================================================
    # Radial (small) Tests (generated synthetic field, ~1 km)
    # =========================================================================

    def test_radial_small_total_irradiance(self):
        """Test radial (small) total irradiance matches baseline."""
        baseline = self.baselines["radial_small_point"]
        result = self._run_analysis(
            site=self.CspSite.RadialSmall,
            aim_type=self.AimType.Point,
            aim_params=baseline["aim_params"],
            year=baseline["date"]["year"],
            month=baseline["date"]["month"],
    ...
    def test_radial_small_glaring_voxels(self):
        """Test radial (small) glaring voxel count matches baseline."""
        baseline = self.baselines["radial_small_point"]
        result = self._run_analysis(
            site=self.CspSite.RadialSmall,
            aim_type=self.AimType.Point,
...
class BaselineFileTests(unittest.TestCase):
    ...
    def test_baselines_file_valid_json(self):
        ...
        self.assertIn("radial_small_point", data)
