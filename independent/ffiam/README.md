...
The fastest way to try FFIAM is the containerized one-click flow — clone the repo and run:

```bash
./quickstart.sh                 # runs the default NSTTF demo
./quickstart.sh radial_small    # or pick another demo
```

This builds the Docker image (first run only) and runs a demo analysis, writing the Excel summary and PNG plots to `./output/FFIAM/`. It uses an NVIDIA GPU automatically when one is available (via the [NVIDIA Container Toolkit](#prerequisites)) and otherwise falls back to the CPU backend, so it works on any machine with Docker. Available demos: `nsttf`, `radial_small`, `sample_v2`, and `radial` (full-size, GPU required).

For native (non-Docker) installation and the full Docker reference, see the sections below.
...
- `ffiam_lib_cpu` — CPU-only fallback. No CUDA dependency at link or run
  time. Build the `ffiam_lib_cpu` target: `cmake --build build --target ffiam_lib_cpu`
  (Linux) or `cmake --build build_vs --config Release --target ffiam_lib_cpu` (Windows).
...
Each analysis allocates two host-side memory pools — a 2 GB main pool (heliostat
and aim data) and a 2 GB voxel pool — so every run needs **~4 GB of free RAM**
regardless of site, plus the GPU's own working set. These pools are a fixed size
(pre-sized to hold the largest supported grid), not scaled per field, and are
freed when the analysis completes.

`pyffiam` then copies the voxel results into NumPy arrays for plotting and export,
and this post-processing scales with the voxel grid. Small and medium sites
(NSTTF, SampleV1, `RadialSmall` ≈ 1 km) run in a few GB; full-size grids — the
V3.0-scale ≈ 1.6 km field reaches hundreds of millions of voxels — can require
**32 GB+** of system RAM during post-processing. On smaller machines, use a
smaller site (e.g. `RadialSmall`) or reduce the field radius, airspace height, or
voxel resolution. The standalone C++ executable computes the large grids on the
GPU without the Python post-processing overhead.

CUDA additionally allocates voxel storage in VRAM that scales with the grid, so
larger fields need a GPU with more memory.
...
**Radial site (1.6 km), ~10,000 heliostats:**
...
result = analysis(site=CspSite.Radial,
                  aim_strat=AimType.Point,
...
**Radial site, Vector aim (all heliostats aim straight up, simulating stow):**
...
| `CspSite.RadialSmall` | 6,400 | 1,000 m | Generated radial field (~1 km) |
| `CspSite.Radial` | 10,000 | 1,600 m | Generated radial field (~1.6 km) |
...
| `SiteType` | `Site_Nsttf`, `Site_Radial`, `Site_RadialSmall`, `Site_Custom`, etc. |
...
The **Visualization** workflow uses Unreal Engine 5.7 to render CSP fields and irradiance results in an interactive 3D environment. Users can navigate the field, adjust analysis parameters in real-time, and observe how different aim strategies affect the irradiance distribution.
...
The Unreal Engine visualization project is maintained in a separate repository due to its size and different build requirements. The project integrates with the FFIAM C++ library via shared-library (`.so` / `.dll`) linking.

To use the visualization workflow:
1. Build the FFIAM C++ library as described above
2. Clone the companion repository: [ffiam-ue](https://gitlab-ex.sandia.gov/casims/ffiam-ue)
3. Open `Irradiance.uproject` in Unreal Engine 5.7
4. The project links against the FFIAM library via the `FFIAM_PATH` environment variable
...
Both the C++ library and pyffiam include test suites, covering the CUDA and CPU
(OpenMP) backends — including CPU-vs-CUDA parity checks. CUDA-specific tests are
skipped automatically on machines without a CUDA runtime.

### C++ Tests

Build the `ffiam_tests` target, then run the binary.

Linux:

    cmake --build build --target ffiam_tests
    ./build/bin/ffiam_tests

Windows:

    cmake --build build_vs --config Release --target ffiam_tests
    build_vs\bin\Release\ffiam_tests.exe
...
`tests/test_docker_parity.py` runs the same analyses natively and inside the Docker container, then compares peak irradiance, total irradiance, glaring voxel counts, and heliostat/facet counts. Five cases are covered: NSTTF (Point, Ring), SampleV1 (Point), and radial (Point, stow Vector). A 0.5% relative tolerance accommodates cross-platform numeric drift (MSVC/arch52 vs GCC/arch75+).

Prerequisites:
1. Native build complete (`ffiam_lib.dll` / `libffiam_lib.so` present in `pyffiam/src/pyffiam/external/`)
...
- Full radial site (~10,000 heliostats) added as a preset
...
- New presets: 1 km generic site (6,084 heliostats) and a 1 km radial field (6,400 heliostats)
