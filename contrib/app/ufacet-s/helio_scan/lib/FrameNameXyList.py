...
class FrameNameXyList(csvi.CsvInterface):
    ...
    def add_list_of_name_xy_lists(self, frame_id:int, input_list_of_name_xy_lists:NXL):
        ...
        if frame_id in self.dictionary:
            print('ERROR: In FrameNameXyList.add_list_of_name_xy_lists(), attempt to add frame_id='+str(frame_id)+', which is already present.')
            assert False
    ...
    def merge_list_of_name_xy_lists(self, frame_id:int, input_list_of_name_xy_lists:NXL, warn_if_common_name=True, skip_if_common_name=True, error_if_common_name=False):
        """
        Add a list of [name, xy_list] pairs to the dictionary, under the given frame_id key.
            ...
            for input_name_xy_list in input_list_of_name_xy_lists:
                ...
                if input_name_copy not in existing_name_list:
                    ...
                    if error_if_common_name:
                        print('ERROR: In FrameNameXyList.merge_list_of_name_xy_lists(), attempt to add xy_list for frame_id='+str(frame_id)+' and name="'+str(input_name_copy)+'", both of which are already present.')
                        assert False
                    if warn_if_common_name:
                        if skip_if_common_name:
                            ...
                            suffix = ''
                        print('WARNING: In FrameNameXyList.merge_list_of_name_xy_lists(), attempt to add xy_list for frame_id='+str(frame_id)+' and name="'+str(input_name_copy)+'", both of which are already present.' + suffix)
                    # Proceed.
                    if not skip_if_common_name:
                        if warn_if_common_name:
                            print('WARNING: In FrameNameXyList.merge_list_of_name_xy_lists(), adding to already-existing (frame_id='+str(frame_id)+', name="'+str(input_name_copy)+'") pair.')
    ...
    def parse_row_list_of_name_xylists_aux(self, input_row_list_of_name_xylists:list[str], n_hel:int, return_remainder=False) -> NXL|tuple[NXL,list[str]]:
        ...
        if n_hel > 1:
            further_name_xylists = self.parse_row_list_of_name_xylists_aux(input_row_list_of_name_xylists[last_idx+1:], n_hel-1)
        else:
    ...
    def to_csv_line(self, delimeter=",", frame_id=None):
        ...
        if frame_id == None:
            if self.number_of_frames() != 1:
                raise RuntimeError(f"Expected 1 frame in this {self.__class__.__name__} but found {self.number_of_frames()}!")
    ...
    def draw_frames(self,
                    ...
                    crop):                 # Boolean indicating whether to suppress annotations outside image frame.
        """
    ...
    def draw_frame(self, frame_id:int):
        ...
        for hel_name_xy_list in name_xy_list:
            ...
            if close_xy_list:
                if len(active_xy_list_2) < 3:
                    print('ERROR: In FrameNameXyList.draw_frame(), attempting to close an xy_list with too few vertices; len(xy_list) =', len(active_xy_list_2))
                    assert False
        ...
        ip.plot_image_figure(frame_img,
                             ...
                             output_body=(key_frame_body_ext + fig_suffix),  # Add the suffix on top of the existing extension.  plot_image_figure() will add ".png"
                             dpi=dpi,
