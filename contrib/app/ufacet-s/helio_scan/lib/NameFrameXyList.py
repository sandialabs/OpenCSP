...
class NameFrameXyList():
    ...
    def add_list_of_frame_xy_lists(self, hel_name, input_list_of_frame_xy_lists):
        ...
        if hel_name in self.dictionary:
            print('ERROR: In NameFrameXyList.add_list_of_frame_xy_lists(), attempt to add hel_name='+str(hel_name)+', which is already present.')
            assert False
        self.dictionary[hel_name] = input_list_of_frame_xy_lists


    def merge_list_of_frame_xy_lists(self, hel_name, input_list_of_frame_xy_lists, warn_if_common_frame=True, skip_if_common_frame=True, error_if_common_frame=False):
        """
        Add a list of [frame, xy_list] pairs to the dictionary, under the given hel_name key.
            ...
            for input_frame_xy_list in input_list_of_frame_xy_lists:
                ...
                if input_frame_copy not in existing_frame_list:
                    ...
                    if error_if_common_frame:
                        print('ERROR: In NameFrameXyList.merge_list_of_frame_xy_lists(), attempt to add xy_list for hel_name='+str(hel_name)+' and frame="'+str(input_frame_copy)+'", both of which are already present.')
                        assert False
                    if warn_if_common_frame:
                        if skip_if_common_frame:
                            ...
                            suffix = ''
                        print('WARNING: In NameFrameXyList.merge_list_of_frame_xy_lists(), attempt to add xy_list for hel_name='+str(hel_name)+' and frame="'+str(input_frame_copy)+'", both of which are already present.' + suffix)
                    # Proceed.
                    if not skip_if_common_frame:
                        if warn_if_common_frame:
                            print('WARNING: In NameFrameXyList.merge_list_of_frame_xy_lists(), adding to already-existing (hel_name='+str(hel_name)+', frame="'+str(input_frame_copy)+'") pair.')
    ...
    def merge_frame_xy_list(self, hel_name, input_frame_xy_list, warn_if_common_frame=True, skip_if_common_frame=True, error_if_common_frame=False):
        """
        ...
        if hel_name not in self.dictionary:
            ...
            if input_frame_copy not in existing_frame_list:
                ...
                if error_if_common_frame:
                    print('ERROR: In NameFrameXyList.merge_frame_xy_lists(), attempt to add xy_list for hel_name='+str(hel_name)+' and frame="'+str(input_frame_copy)+'", both of which are already present.')
                    assert False
                if warn_if_common_frame:
                    if skip_if_common_frame:
                        suffix = '  Skipping.'
                        ...
                        suffix = ''
                    print('WARNING: In NameFrameXyList.merge_frame_xy_lists(), attempt to add xy_list for hel_name='+str(hel_name)+' and frame="'+str(input_frame_copy)+'", both of which are already present.' + suffix)
                # Proceed.
                if not skip_if_common_frame:
                    if warn_if_common_frame:
                        print('WARNING: In NameFrameXyList.merge_frame_xy_lists(), adding to already-existing (hel_name='+str(hel_name)+', frame="'+str(input_frame_copy)+'") pair.')
                    for existing_frame_xy_list in existing_list_of_frame_xy_lists:
                        existing_frame    = existing_frame_xy_list[0]
                        existing_xy_list = existing_frame_xy_list[1]
                        if existing_frame == input_frame_copy:
    ...
    def add_FrameNameXyList(self, input_fnxl, warn_if_common_frame=False, skip_if_common_frame=False, error_if_common_frame=True):
        ...
        for frame_id in input_fnxl.sorted_frame_id_list():
            ...
            for name_xy_list in list_of_name_xy_lists:
                ...
                self.merge_frame_xy_list(hel_name, new_frame_xy_list, warn_if_common_frame, skip_if_common_frame, error_if_common_frame)
    ...
    def parse_row_list_of_frame_xylists_aux(self, input_row_list_of_frame_xylists, n_frames):
        ...
        if n_frames > 1:
            further_frame_xylists = self.parse_row_list_of_frame_xylists_aux(input_row_list_of_frame_xylists[last_idx+1:], n_frames-1)
        else:
