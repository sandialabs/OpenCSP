...
class Specifications():
    ...
    def __init__(self,
        ...
        self.facets_corners           = sr.centers3d_to_corners3d(self.facets_centroids, 
                                                                  self.facet_width,
                                                                  self.facet_height)
    ...
    def read_heliostat_locations_dict(self, heliostat_locations_file):  # ?? SCAFFOLDING RCB -- ADAPTED FROM ROUTINE "heliostats_read_file" IN SolarField.py; INTEGRATE WITH THAT
    ...
    def heliostat_corner_xyz_list_given_heliostat_spec(self, heliostat_spec):
        ...
        for facet_spec in heliostat_spec.values():
            ...
            R = R_y.dot(R_x.dot(R_z))  # First Rz, then Rx, then Ry  # ?? SCAFFOLDING RCB -- USE FASTER AND MORE PRECISE ROTx, ROTy, ROTz FUNCTIONS.  See https://learnopencv.com/rotation-matrix-to-euler-angles/
            ...
            heliostat_corner_xyz_list += [list(final_ul_xyz), list(final_ur_xyz), list(final_lr_xyz), list(final_ll_xyz)]
        # Return.
...
def heliostat_corner_xyz_list_given_focal_length_faceted(flat_corner_xyz_list, focal_length):
    ...
    smooth_xyz_list = [[xyz[0], xyz[1], xyz[2]+smooth_z(xyz[0], xyz[1], focal_length)] for xyz in flat_corner_xyz_list]
...
#        facets_centroids_file    = '<home_dir>/temporary_ufacet_input/csv_files/nsttf_centroids.csv',
        facets_centroids_file    = experiment_dir() + '2020-12-03_FastScan1/3_Post/Answers/20201203/1544_NS_U/010_HeliostatModel/data/NSTTF_Facet_Centroids.csv',  # ?? SCAFFOLDING RCB -- RENAME TO "facet_centroids_file"  ALSO, THIS SHOULD BE PASSED IN.
        heliostat_locations_file = experiment_dir() + '2020-12-03_FastScan1/3_Post/Answers/20201203/1544_NS_U/020_FieldModel/data/NSTTF_Heliostats_origin_at_torque_tube.csv',  # ?? SCAFFOLDING RCB -- THIS SHOULD BE PASSED IN.
        # Aim point assumed when computing an ideal focal length for a heliostat.
