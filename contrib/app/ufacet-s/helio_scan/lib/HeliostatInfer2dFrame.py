...
class HeliostatInfer2dFrame():
    ...
    def __init__(self,
                 ...
                 flat_corner_xyz_list):    # A "flat heliostat" model, oriented with z parallel to the nominal heliostat optical axis.
        ...
        if len(observed_corner_xy_list) != len(flat_corner_xyz_list):
            msg = 'In HeliostatInfer2dFrame.__init__(), input len(observed_corner_xy_list)=' + str(len(observed_corner_xy_list)) + ' is not equal to input len(flat_corner_xyz_list)=' + str(len(flat_corner_xyz_list))
            print('ERROR: ' + msg)
