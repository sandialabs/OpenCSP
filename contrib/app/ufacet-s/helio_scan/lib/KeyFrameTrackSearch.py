"""
Tracking facet corners previously found in a key frame pair of a UFACET scan video.
...
from . import FrameNameXyList as fnxl
from . import ufacet_pipeline_frame as upf


class KeyFrameTrackSearch():
    """
    Controls forward and backward tracking of heliostat corners in a UFACET scan video.
    """
    def __init__(self,
                 # Execution control.
                 iterations: int,                # iterations for homing in on confirming corners
                 canny_levels: list[str],        # inner loop canny levels used during corner confirmation iteration, eg ['medium', 'light']
                 solvePnPtype: str,              # used with solvePnP, eg "pnp"
                 cam_matrix: any | None,         # passed to opencv cv.projectPoints
                 dist_coeff: any | None,         # passed to opencv cv.projectPoints
                 confirm_type: str,              # "iterative" or ""/"canny"
                 # Input/output sources.
                 input_video_body: str,          # Used for output figure file names. Ex: first part of "DJI_427t_428_429.018581.JPG"
                 key_frame_projected_corners_fnxl: fnxl,  # A FrameNameXyList object containing exactly two frames:  The key frame, and the frame that follows it.  Both with associated found projected corners.
                 key_frame_confirmed_corners_fnxl: fnxl,  # A FrameNameXyList object containing exactly two frames:  The key frame, and the frame that follows it.  Both with associated found confirmed corners.
        ...
        self.minimum_corners_required_inside_frame = math.ceil(MINIMUM_CORNERS_REQUIRED_INSIDE_FRAME * self.specifications.corners_per_heliostat)
        ...
        self.key_frame_1_projected_list_of_name_xy_lists = key_frame_projected_corners_fnxl.list_of_name_xy_lists(self.key_frame_id_1)
        self.key_frame_2_projected_list_of_name_xy_lists = key_frame_projected_corners_fnxl.list_of_name_xy_lists(self.key_frame_id_2)
        self.key_frame_1_confirmed_list_of_name_xy_lists = key_frame_confirmed_corners_fnxl.list_of_name_xy_lists(self.key_frame_id_1)
        self.key_frame_2_confirmed_list_of_name_xy_lists = key_frame_confirmed_corners_fnxl.list_of_name_xy_lists(self.key_frame_id_2)
        ...
        self.output_frame_projected_corners_dir = os.path.join(self.output_projected_corners_dir, self.key_frame_id_str_1)
        print('In KeyFrameTracksSearch.__init__(), self.output_frame_projected_corners_dir =', self.output_frame_projected_corners_dir)
        ...
        self.output_frame_confirmed_corners_dir = os.path.join(self.output_confirmed_corners_dir, self.key_frame_id_str_1)
        print('In KeyFrameTracksSearch.__init__(), self.output_frame_confirmed_corners_dir =', self.output_frame_confirmed_corners_dir)
        if self.draw_track_images:
    ...
    def PredictConfirm(self, tracking_direction): 
        ...
        all_frame_id_str_list = [upf.frame_id_str_given_frame_file_body_ext(body_ext) for body_ext in self.all_frame_body_ext_list]
        if tracking_direction == 'forward':
            # Velocity is from 1 --> 2.
            velocity, latest_projected_corners = self.velocity_and_initial_corners(self.key_frame_1_projected_list_of_name_xy_lists, self.key_frame_2_projected_list_of_name_xy_lists)
            ...
            velocity, latest_projected_corners = self.velocity_and_initial_corners(self.key_frame_2_projected_list_of_name_xy_lists, self.key_frame_1_projected_list_of_name_xy_lists)
            ...
            raise ValueError('ERROR: In KeyFrameTrackSearch.PredictConfirm(), unexpected tracking_direction="'+str(tracking_direction)+'" encountered(1).  Allowable values are "forward" and "backward" only (2).')
        ...
        if tracking_direction == 'forward':
            # Key frame 1.
            self.key_frame_projected_track_fnxl.add_list_of_name_xy_lists(self.key_frame_id_1, self.key_frame_1_projected_list_of_name_xy_lists)
            self.key_frame_confirmed_track_fnxl.add_list_of_name_xy_lists(self.key_frame_id_1, self.key_frame_1_confirmed_list_of_name_xy_lists)
            self.draw_frame_with_points_if_desired(self.key_frame_id_str_1, self.key_frame_1_projected_list_of_name_xy_lists, 'Projected', point_color='g')
            self.draw_frame_with_points_if_desired(self.key_frame_id_str_1, self.key_frame_1_confirmed_list_of_name_xy_lists, 'Confirmed', point_color='b')
            # Key frame 2.
            self.key_frame_projected_track_fnxl.add_list_of_name_xy_lists(self.key_frame_id_2, self.key_frame_2_projected_list_of_name_xy_lists)
            self.key_frame_confirmed_track_fnxl.add_list_of_name_xy_lists(self.key_frame_id_2, self.key_frame_2_confirmed_list_of_name_xy_lists)
            self.draw_frame_with_points_if_desired(self.key_frame_id_str_2, self.key_frame_2_projected_list_of_name_xy_lists, 'Projected', point_color='orange')
            self.draw_frame_with_points_if_desired(self.key_frame_id_str_2, self.key_frame_2_confirmed_list_of_name_xy_lists, 'Confirmed', point_color='yellow')
        ...
        for frame_id_str in frame_id_str_sequence[2:]:
            ...
            if img is None: # you skip that image
                print('In KeyFrameTrackSearch.PredictConfirm(), Unexpected null image encountered.') # ?? SCAFFOLDING RCB -- WE DON'T KNOW WHY THIS IS HERE.  CAN THIS HAPPEN?  WHY IS IT ALLOWED?
            ...
            for hel_indx in range(0, num_hel):
                ...
                projected_corners,  \
                confirmed_corners,  \
                num_non_None_confirmed_corners = self.confirm_corners(img,
                                                                      predicted_corners, 
                                                                      iterations=self.iterations,
                                                                      canny_levels=self.canny_levels,
                                                                      confirm_type=self.confirm_type)
                confirmed_ratio = num_non_None_confirmed_corners / float(n_inside)  # Don't penalize for not confirming corners outside frame.
                #print('In KeyFrameTrackSearch.PredictConfirm(), confirmed_ratio =', confirmed_ratio, ';  self.minimum_fraction_of_confirmed_corners =', self.minimum_fraction_of_confirmed_corners)
                if confirmed_ratio < self.minimum_fraction_of_confirmed_corners:
                    """ending criterion"""
                    stop_track_flags[hel_indx] = True
                    continue
                # store the tracks
                projected_list_of_name_xy_lists.append([self.heliostat_names[hel_indx], projected_corners])  # For adding to the FrameNameXyList object.
                confirmed_list_of_name_xy_lists.append([self.heliostat_names[hel_indx], confirmed_corners])  # For adding to the FrameNameXyList object.
                # update
                velocity[hel_indx] = self.update_velocity(velocity, 
                                                          predicted_corners,
                                                          projected_corners, 
                                                          hel_indx, skip_flag)
            ...
            self.key_frame_confirmed_track_fnxl.add_list_of_name_xy_lists(upf.frame_id_given_frame_id_str(frame_id_str), confirmed_list_of_name_xy_lists)
            # Draw.
            self.draw_frame_with_points_if_desired_aux(img, frame_id_str, projected_list_of_name_xy_lists, 'Projected', point_color='m')
            self.draw_frame_with_points_if_desired_aux(img, frame_id_str, confirmed_list_of_name_xy_lists, 'Confirmed', point_color='c')
    ...
    def velocity_and_initial_corners(self, key_frame_A_list_of_name_xy_lists, key_frame_B_list_of_name_xy_lists):
        ...
        for name_xy_list_A, name_xy_list_B in zip(key_frame_A_list_of_name_xy_lists, key_frame_B_list_of_name_xy_lists):
            ...
            if (name_A != name_B):
                msg = 'In KeyFrameTrackAndSearch.velocity_and_initial_corners(), encountered mismatched name_A="'+str(name_A)+'" and name_B="'+str(name_B)+'".'
                print('ERROR: ' + msg)
    ...
    def draw_frame_with_points_if_desired(self, frame_id_str, list_of_name_xy_lists, Confirmed_or_Projected_str, point_color):
        if self.draw_track_images:
            frame_body_ext = upf.frame_file_body_ext_given_frame_id_str(self.input_video_body, frame_id_str)
            img = cv.imread(os.path.join(self.input_frame_dir, frame_body_ext))
            self.draw_frame_with_points_if_desired_aux(img, frame_id_str, list_of_name_xy_lists, Confirmed_or_Projected_str, point_color)


    def draw_frame_with_points_if_desired_aux(self, img, frame_id_str, list_of_name_xy_lists, Confirmed_or_Projected_str, point_color):
        if self.draw_track_images:
            plt.figure()
            ...
            plt.title(Confirmed_or_Projected_str + ' Points for Key Frame ' + self.key_frame_id_str_1 + ', Frame: ' + frame_id_str)
            fig_file_body_ext     = self.input_video_body + '_' + frame_id_str + '_' + Confirmed_or_Projected_str.lower() + '.png'
            if Confirmed_or_Projected_str == 'Projected':
                ...
                msg = 'In KeyFrameTrackSearch.draw_frame_with_points(), encountered unexpected Confirmed_or_Projected_str = ' + str(Confirmed_or_Projected_str)
                print(msg)
                raise ValueError(msg)
            fig_file_dir_body_ext = os.path.join(figure_output_dir, fig_file_body_ext)
            #print('In KeyFrameTrackSearch(), saving figure file: ', fig_file_dir_body_ext)
    ...
    def confirm_corners(self, img, predicted_corners, canny_levels, iterations, confirm_type='', tolerance=3, pixels=100):
        ...
        for ite in range(0, iterations):
            ...
            for i in range(0, inner_loop_range): # levels of canny
                if confirm_type == 'iterative':
                    ...
                    edges = CannyImg(img, canny_type=canny_types[i])  # ?? SCAFFOLDING RCB -- CHANGE THIS VARIABLE NAME TO "edge_image"
                ...
                # modifying its arguments as a side effect, which I need to test to determine.  Caveat!
                mtx, dist, rvec, tvec, error  = solvePNP(points3d, points2d, h, w, 
                                                         pnptype=self.solvePnPtype, cam_matrix=self.cam_matrix, dist_coeff=self.dist_coeff)
            ...
            if self.solvePnPtype == 'calib' and ite == 0:
                ...
                self.cam_matrix, self.dist_coeff = mtx, dist # save the current camera model and use that for the rest of the tracking
        ...
        num_non_None_confirmed_corners = len([x for x in previous_confirmed_corners if x is not None])  # Don't count "None" corners.

#print('In KeyFrameTrackSearch.confirm_corners(), upon exit, num_non_None_confirmed_corners =', num_non_None_confirmed_corners)  # ?? SCAFFOLDING RCB -- TEMPORARY
