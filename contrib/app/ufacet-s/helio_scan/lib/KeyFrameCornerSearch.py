...
class KeyFrameCornerSearch():
    ...
    def __init__(self,
                 ...
                 # Render control.
                 render_control:RenderControlKeyCorners): # Flags to control rendering on this run.
        ...
        print('In KeyFrameCornerSearch.__init__(), performing full image analysis...')  # ?? SCAFFOLDING RCB -- TEMPORARY

        # Input polygons
        self.draw_img_polygons()
        # Edge detection
        self.frame['edges'],     \
        self.frame['edges_img']                        = self.canny()
        # Sky detection
        # self.frame['sky'],     \
        # self.frame['sky_img']                          = self.skyhsv()  # ?? SCAFFOLDING RCB -- PREVIOUS VERSION
        self.frame['sky'],       \
        self.frame['sky_img']                          = self.sky()
        # Facet boundaries
        self.frame['boundaries'],    \
        self.frame['boundaries_img']                   = self.facet_boundaries()        
        # Connected_components
        self.frame['components'],    \
        self.frame['components_img']                   = self.connected_components()
        # Filtered connected_components
        self.frame['filt_components'],    \
        self.frame['filt_components_img']              = self.filter_connected_components()
        ...
        self.frame['filtered_facets'],  \
        self.frame['heliostats']                       = self.filter_facets_polygons()  # Initial setting of self.frame['heliostats']
        # Top row            
        self.top_row_facets()  # Updates self.frame['heliostats']
        # Register top row
        self.classify_top_row_facets()  # Updates self.frame['heliostats']
        # Projected Corners
        self.project_and_confirm(iterations=5,                               # Updates: self.frame['heliostats'], self.frame['all_projected_corners']
    ...
    def draw_img_polygons(self):
        ...
        if self.render_control.draw_img_box:
            ...
            for name_polygon in self.list_of_name_polygons:  # ?? SCAFFOLDING RCB -- INTEGRATRE THIS WITH STANDARD FNXL RENDERING.
                ...
                x_list  = [pt[0] for pt in closed_xy_list]  # ?? SCAFFOLDING RCB -- INTEGRATRE THIS WITH STANDARD FNXL RENDERING.
                y_list  = [pt[1] for pt in closed_xy_list]  # ?? SCAFFOLDING RCB -- INTEGRATRE THIS WITH STANDARD FNXL RENDERING.
                plt.plot(x_list, y_list, color=color)  # ?? SCAFFOLDING RCB -- INTEGRATRE THIS WITH STANDARD FNXL RENDERING.
                # Draw the heliostat name.  # ?? SCAFFOLDING RCB -- INTEGRATRE THIS WITH STANDARD FNXL RENDERING.
                if len(polygon) > 0:  # ?? SCAFFOLDING RCB -- MAKE THIS INTEGRATED WITH STANDARD PLOTTING AND RENDER CONTROL ROUTINES.  # ?? SCAFFOLDING RCB -- INTEGRATRE THIS WITH STANDARD FNXL RENDERING.
                    label_xy = g2d.label_point(polygon)  # ?? SCAFFOLDING RCB -- MAKE THIS INTEGRATED WITH STANDARD PLOTTING AND RENDER CONTROL ROUTINES.  # ?? SCAFFOLDING RCB -- INTEGRATRE THIS WITH STANDARD FNXL RENDERING.
                    plt.text(label_xy[0],  # ?? SCAFFOLDING RCB -- MAKE THIS INTEGRATED WITH STANDARD PLOTTING AND RENDER CONTROL ROUTINES.  # ?? SCAFFOLDING RCB -- INTEGRATRE THIS WITH STANDARD FNXL RENDERING.
                            label_xy[1],  # ?? SCAFFOLDING RCB -- MAKE THIS INTEGRATED WITH STANDARD PLOTTING AND RENDER CONTROL ROUTINES.  # ?? SCAFFOLDING RCB -- INTEGRATRE THIS WITH STANDARD FNXL RENDERING.
                            ...
                            fontweight='bold')  # ?? SCAFFOLDING RCB -- MAKE THIS INTEGRATED WITH STANDARD PLOTTING AND RENDER CONTROL ROUTINES.  # ?? SCAFFOLDING RCB -- INTEGRATRE THIS WITH STANDARD FNXL RENDERING.

            plt.savefig(os.path.join(self.frame['output_construction_dir'], self.key_frame_id_str+'_00_img_box.png'), dpi=500)
            plt.close()
    ...
    def canny(self, img=None):
        ...
        if img is None:
            img          = self.frame['key_frame_img']  # ?? SCAFFOLDING RCB -- SEE CANNY() AND CONFIRM() ROUTINES FOR DUPLICATE PLACES WHERE THIS CODE IS PLACED.  MUST KEEP CONSISTENT.
            img          = cv.GaussianBlur(img, (5,5), 0)  # ?? SCAFFOLDING RCB -- SEE CANNY() AND CONFIRM() ROUTINES FOR DUPLICATE PLACES WHERE THIS CODE IS PLACED.  MUST KEEP CONSISTENT.
        
        edges = CannyImg(img=img, canny_type='auto') # ! auto  # ?? SCAFFOLDING RCB -- ORIGINAL CODE was 'light'  # ?? SCAFFOLDING RCB -- SEE CANNY() AND CONFIRM() ROUTINES FOR DUPLICATE PLACES WHERE THIS CODE IS PLACED.  MUST KEEP CONSISTENT.
#edges = CannyImg(img=img, canny_type='light') # ! auto  # ?? SCAFFOLDING RCB -- ORIGINAL CODE
        ...
        if self.render_control.draw_edge_fig:
            save_fig(img=edges, imgname=self.key_frame_id_str+'_01_edge_fig.png', path=self.frame['output_construction_dir'], rgb=True, dpi=1000)  # ?? SCAFFOLDING RCB -- SETTING RGB=TRUE, TO SUPPRESS CV CALL WITHIN SAVE_FIG() TO CONVERT FROM BGR TO RGB.  THIS CRASHES ON THIS INPUT.
        if self.render_control.draw_edge_img:
            save_image(img=edge_img, imgname=self.key_frame_id_str+'_01_edge_img.png', path=self.frame['output_construction_dir'])
        if self.render_control.draw_edge_img_fig:
            save_fig(img=edge_img, imgname=self.key_frame_id_str+'_01_edge_img_fig.png', path=self.frame['output_construction_dir'], rgb=True, dpi=1000)
    ...
    def skyhsv(self):
        ...
        if self.render_control.draw_skyhsv:
            save_image(img=sky, imgname=self.key_frame_id_str+'_02_skyhsv.png', path=self.frame['output_construction_dir'])
        if self.render_control.draw_skyhsv_fig:
            save_fig(img=sky, imgname=self.key_frame_id_str+'_02_skyhsv_fig.png', path=self.frame['output_construction_dir'], rgb=True, dpi=1000)
        if self.render_control.draw_skyhsv_img:
            save_image(img=sky_img, imgname=self.key_frame_id_str+'_02_skyhsv_img.png', path=self.frame['output_construction_dir'])
        if self.render_control.draw_skyhsv_img_fig:
            save_fig(img=sky_img, imgname=self.key_frame_id_str+'_02_skyhsv_img_fig.png', path=self.frame['output_construction_dir'], rgb=True, dpi=1000)
        return sky, sky_img
    ...
    def sky(self):
        ...
        if self.render_control.draw_sky:
            save_image(img=(sky*255), imgname=self.key_frame_id_str+'_02_sky.png', path=self.frame['output_construction_dir'])  # ?? SCAFFOLDING RCB -- MULTIPLIED BY 255 TO MAKE IMAGE VISIBLE IN PREVIEW.
        if self.render_control.draw_sky_fig:
            save_fig(img=sky, imgname=self.key_frame_id_str+'_02_sky_fig.png', path=self.frame['output_construction_dir'], rgb=True, dpi=1000)  # ?? SCAFFOLDING RCB -- SETTING RGB=TRUE, TO SUPPRESS CV CALL WITHIN SAVE_FIG() TO CONVERT FROM BGR TO RGB.  THIS CRASHES ON THIS INPUT.
        if self.render_control.draw_sky_img:
            save_image(img=sky_img, imgname=self.key_frame_id_str+'_02_sky_img.png', path=self.frame['output_construction_dir'])
        if self.render_control.draw_sky_img_fig:
            save_fig(img=sky_img, imgname=self.key_frame_id_str+'_02_sky_img_fig.png', path=self.frame['output_construction_dir'], rgb=True, dpi=1000)
    ...
    def facet_boundaries(self):
        ...
        print('In KeyFrameCornerSearch.facet_boundaries(), number of edge pixels len(row_edges) =', len(row_edges))  # ?? SCAFFOLDING RCB -- TEMPORARY
        ...
        for row, col in zip(row_edges, col_edges):
            ...
            is_boundary = False
            # Left
            if  (self.is_boundary_pixel(row-1,  col, 'left')
        ...
        if self.render_control.draw_boundaries:
            save_image( img=boundaries_img,     imgname=self.key_frame_id_str+'_03_boundaries.png',       path=self.frame['output_construction_dir'])
        if self.render_control.draw_boundaries_fig:
            save_fig(   img=boundaries_img,     imgname=self.key_frame_id_str+'_03_boundaries_fig.png',   path=self.frame['output_construction_dir'], dpi=1000)
    ...
    def is_boundary_pixel(self, row:int, col:int, btype:str, required_sky_width:int=None, ignore_margin:int=None) -> bool:
    ...
    def connected_components(self) -> tuple[list[Component],  np.ndarray]:
        ...
        def construct_component(row:int, col:int, btype:str, color:list[int], img:np.ndarray) -> Component:
            ...
            while len(horizon) > 0:
                ...
                img[r,c, :]     = void_color 
                # add neighbors
                if ( (r-1) >= 0)        and ((c-1) >= 0)        and (img[r-1,c-1,:] == color).all(axis=-1) and [r-1, c-1]  not in horizon:  horizon.append([r-1,c-1])
                if ( (r-1) >= 0)                                and (img[r-1,c,:]   == color).all(axis=-1) and [r-1, c]    not in horizon:  horizon.append([r-1,c])
                if ( (r-1) >= 0)        and ((c+1) < max_col)   and (img[r-1,c+1,:] == color).all(axis=-1) and [r-1, c+1]  not in horizon:  horizon.append([r-1,c+1])
                if                          ((c-1) >= 0)        and (img[r,  c-1,:] == color).all(axis=-1) and [r, c-1]    not in horizon:  horizon.append([r,  c-1])
                if                          ((c+1) < max_col)   and (img[r,  c+1,:] == color).all(axis=-1) and [r, c+1]    not in horizon:  horizon.append([r,  c+1])
                if ((r+1) < max_row)    and ((c-1) >= 0)        and (img[r+1,c-1,:] == color).all(axis=-1) and [r+1, c-1]  not in horizon:  horizon.append([r+1,c-1])
                if ((r+1) < max_row)                            and (img[r+1,c,:]   == color).all(axis=-1) and [r+1, c]    not in horizon:  horizon.append([r+1,c])
                if ((r+1) < max_row)    and ((c+1) < max_col)   and (img[r+1,c+1,:] == color).all(axis=-1) and [r+1, c+1]  not in horizon:  horizon.append([r+1,c+1])
        ...
        if self.render_control.draw_components:
            save_image( img=components_img, imgname=self.key_frame_id_str+'_04_components.png',       path=self.frame['output_construction_dir'])
        if self.render_control.draw_components_fig:
            save_fig(   img=components_img, imgname=self.key_frame_id_str+'_04_components_fig.png',   path=self.frame['output_construction_dir'])
        
        if self.render_control.write_components:
            save_connected_components(components=components, filename='components.csv',path=self.frame['output_construction_dir'])
    ...
    def filter_connected_components(self) -> tuple[list[Component],  np.ndarray]:
        ...
        if self.render_control.draw_filt_components  or  self.render_control.draw_filt_components_fig:
            ...
            if self.render_control.draw_filt_components:
                save_image( img=filt_connected_comp_img, imgname=self.key_frame_id_str+'_05_filt_components.png',     path=self.frame['output_construction_dir'])
            if self.render_control.draw_filt_components_fig:
                save_fig(   img=filt_connected_comp_img, imgname=self.key_frame_id_str+'_05_filt_components_fig.png', path=self.frame['output_construction_dir'])
        
        if self.render_control.write_filt_components:
            save_connected_components(filtered_components, filename='filt_components.csv', path=self.frame['output_construction_dir'])
    ...
    def fitted_lines_connected_components(self, type_fit='regression'):
        ...
        if self.render_control.write_fitted_lines_components:
            save_fitted_lines_connected_components(components=line_components,
    ...
    def fitted_lines_inliers_components(self):
        ...
        def find_inliers_component(component:Component):
            A, B, C :float = component['original_line_hom_coef'] # from Ax + By + C = 0
            ...
            if tolerance > max_tolerance:
                ...
                if btype == 'left' or btype == 'right':
                    ...
                    x1, y1 = y1, x1
                    x2, y2 = y2, x2
                start_point = [x1, y1] # point at first x pixel [first y for left/right], with the y [x] adjusted to lie on the fit line
        ...
        if self.render_control.write_fitted_lines_inliers_components:
            save_fitted_lines_inliers_connected_components(components=inliers_components,
    ...
    def find_corners(self, corners_type=None):
        ...
        corners_types = [corners_type] if (corners_type != None) else ['top_left', 'top_right', 'bottom_right', 'bottom_left']
        ...
        for corners_type in corners_types:
            ...
            for component in components_tomatched:
                ...
                A, B, C               :int = component['inliers_line_hom_coef']
                ...
                distances            = [euclidean_distance(representative_tomatched, candidate) for candidate in representatives_candidates]
                ...
                corner  = intersection_point(component_start_point[0],component_start_point[1],
                                            component_end_point[0], component_end_point[1],
                                            found_candidate_start_point[0], found_candidate_start_point[1],
                                            found_candidate_end_point[0], found_candidate_end_point[1])
                ...
                if corner[1] >= 0 and corner[1] < max_row and corner[0] >= 0 and corner[0] < max_col:
                    ...
                    corner_structure = {
                        ...
                        prefix1 + key3: [component_start_point[0],component_start_point[1], component_end_point[0], component_end_point[1]],
                        prefix2 + key1: found_candidate['inliers_line_hom_coef'],
                        prefix2 + key2: found_candidate['inliers_pixels'],
                        prefix2 + key3: [found_candidate_start_point[0], found_candidate_start_point[1], found_candidate_end_point[0], found_candidate_end_point[1]]
                    }
        ...
        if self.render_control.draw_corners:
            ...
            plt.scatter([x[0] for x in top_left], [x[1] for x in top_left], marker='o', facecolor=PLT_TOP_LEFT_COLOR, s=5)
            plt.scatter([x[0] for x in top_right], [x[1] for x in top_right], marker='o', facecolor=PLT_TOP_RIGHT_COLOR, s=5)
            plt.scatter([x[0] for x in bottom_right], [x[1] for x in bottom_right], marker='o', facecolor=PLT_BOTTOM_RIGHT_COLOR, s=5)
            plt.scatter([x[0] for x in bottom_left], [x[1] for x in bottom_left], marker='o', facecolor=PLT_BOTTOM_LEFT_COLOR, s=5)
            plt.savefig(os.path.join(self.frame['output_construction_dir'], self.key_frame_id_str+'_08_corners.png'), dpi=200)
            plt.close()
        
        if self.render_control.write_top_left_corners:
            save_corners_facets(corners=top_left_corners, filename='top_left_corners.csv',path=self.frame['output_construction_dir'], corners_type='top_left')
        if self.render_control.write_top_right_corners:
            save_corners_facets(corners=top_right_corners, filename='top_right_corners.csv',path=self.frame['output_construction_dir'], corners_type='top_right')
        if self.render_control.write_bottom_right_corners:
            save_corners_facets(corners=bottom_right_corners, filename='bottom_right_corners.csv',path=self.frame['output_construction_dir'], corners_type='bottom_right')
        if self.render_control.write_bottom_left_corners:
            save_corners_facets(corners=bottom_left_corners, filename='bottom_left_corners.csv',path=self.frame['output_construction_dir'], corners_type='bottom_left')
    ...
    def facets(self):
        ...
        bottom_left_corners    = self.frame['corners']
        ...
        for top_left_corner in top_left_corners:
            ...
            top_right_distances = [euclidean_distance(top_left_point, top_right_corner['point']) for top_right_corner in top_right_corners]
            ...
            for indice in indices:
                ...
                interpoint = intersection_point(left_edge_points[0], left_edge_points[1],
                                                left_edge_points[2], left_edge_points[3],
                                                top_edge_points[0], top_edge_points[1],
                                                top_edge_points[2], top_edge_points[3])
                                
                distance = euclidean_distance(top_left_point, interpoint)
                if ((A_left*top_right_point[0] + B_left*top_right_point[1] + C_left < 0) # on the right side
            ...
            bottom_right_distances = [euclidean_distance(top_right_point, bottom_right_corner['point']) for bottom_right_corner in bottom_right_corners]
            ...
            for indice in indices:
                ...
                interpoint = intersection_point(top_edge_points[0],      top_edge_points[1],
                                                top_edge_points[2],     top_edge_points[3],
                                                right_edge_points[0],   right_edge_points[1],
                                                right_edge_points[2],   right_edge_points[3])
                
                distance = euclidean_distance(top_right_point, interpoint)
                if ((A_top*bottom_right_point[0] + B_top*bottom_right_point[1] + C_top < 0) # on the right side
                and (A_left*bottom_right_point[0] + B_left*bottom_right_point[1] + C_left < 0) # on the right side
                and (distance <= INTER_POINT_DISTANCE)): # ?? MAGIC NUMBER 
                    flag = False
                    break
            ...
            bottom_left_distances = [euclidean_distance(bottom_right_point, bottom_left_corner['point']) for bottom_left_corner in bottom_left_corners]
            ...
            for indice in indices:
                ...
                interpoint      = intersection_point(right_edge_points[0],      right_edge_points[1],
                                                     right_edge_points[2],      right_edge_points[3],
                                                     bottom_edge_points[0],     bottom_edge_points[1],
                                                     bottom_edge_points[2],     bottom_edge_points[3])
                # confirm intersection point is top left corner
                interpoint_alt  = intersection_point(left_edge_points[0],       left_edge_points[1],
                                                     left_edge_points[2],       left_edge_points[3],
                                                     bottom_edge_points[0],     bottom_edge_points[1],
                                                     bottom_edge_points[2],     bottom_edge_points[3])
                ...
                if ((A_right*bottom_left_point[0] + B_right*bottom_left_point[1] + C_right < 0) # on the right side
            ...
            center = intersection_point(top_left_point[0], top_left_point[1],
                                        bottom_right_point[0], bottom_right_point[1],
                                        top_right_point[0], top_right_point[1],
                                        bottom_left_point[0], bottom_left_point[1])
            
            # center should satisfy boundary constraints
            hom_coeff = [top_left_corner['left_edge_coeff'],        top_right_corner['top_edge_coeff'],
                         bottom_right_corner['right_edge_coeff'],   bottom_left_corner['left_edge_coeff']]
            ...
            if (self.is_boundary_pixel(r, c, 'left', required_sky_width=required_sky_width) 
        ...
        if self.render_control.draw_facets:
            ...
            plt.savefig(os.path.join(self.frame['output_construction_dir'], self.key_frame_id_str+'_09_facets.png'), dpi=200)
            plt.close()
    ...
    def filter_facets_polygons(self):
        ...
        for facet in all_facets:
            ...
            for name_polygon in self.list_of_name_polygons:
                ...
                y_min = min(y_list)
                y_max = max(y_list)
                if (x_min < center[0]) and (center[0] < x_max) and \
                   (y_min < center[1]) and (center[1] < y_max):
                    in_polygon = True
                    break
            if in_polygon:
        ...
        for name_polygon in self.list_of_name_polygons:
            name    = name_polygon[0]
            polygon = name_polygon[1]
            x_list  = [pt[0] for pt in polygon]  # ?? SCAFFOLDING RCB -- REDUNDANT AND INEFFIICENT; IMPLEMENT POINT IN POLYGON TEST, OR SIMILAR.
            y_list  = [pt[1] for pt in polygon]  # ?? SCAFFOLDING RCB -- REDUNDANT AND INEFFIICENT; IMPLEMENT POINT IN POLYGON TEST, OR SIMILAR.
            x_min = min(x_list)  # ?? SCAFFOLDING RCB -- REDUNDANT AND INEFFIICENT; IMPLEMENT POINT IN POLYGON TEST, OR SIMILAR.
            x_max = max(x_list)  # ?? SCAFFOLDING RCB -- REDUNDANT AND INEFFIICENT; IMPLEMENT POINT IN POLYGON TEST, OR SIMILAR.
            y_min = min(y_list)  # ?? SCAFFOLDING RCB -- REDUNDANT AND INEFFIICENT; IMPLEMENT POINT IN POLYGON TEST, OR SIMILAR.
            y_max = max(y_list)  # ?? SCAFFOLDING RCB -- REDUNDANT AND INEFFIICENT; IMPLEMENT POINT IN POLYGON TEST, OR SIMILAR.
            heliostat = {}
            heliostat['name'] = name
            heliostat['facets'] = []
            for facet in filtered_facets:
                ...
                if (center not in assigned_centers) and \
                   ((x_min < center[0]) and (center[0] < x_max) and \
                    (y_min < center[1]) and (center[1] < y_max)):
        ...
        if self.render_control.draw_filtered_facets:
            ...
            for facet in filtered_facets:
                ...
                plt.scatter(bottom_left_corner[0], bottom_left_corner[1], facecolor=PLT_BOTTOM_LEFT_COLOR,s=1)
                plt.scatter(center[0], center[1], facecolor=PLT_CENTER_COLOR, s=1)
            plt.savefig(os.path.join(self.frame['output_construction_dir'], self.key_frame_id_str+'_10_filtered_facets.png'), dpi=200)
            plt.close()
        ...
        if self.render_control.draw_filtered_heliostats:
            ...
            plt.savefig(os.path.join(self.frame['output_construction_dir'], self.key_frame_id_str+'_11_filtered_heliostats.png'), dpi=200)
            plt.close()
    ...
    def top_row_facets(self):
        ...
        for heliostat in self.frame['heliostats']:
            ...
            keys                    = [[['bottom_right', 'bottom_edge_coeff'], ['bottom_left', 'bottom_edge_coeff']],
        ...
        if self.render_control.draw_top_row_facets:
            ...
            plt.savefig(os.path.join(self.frame['output_construction_dir'], self.key_frame_id_str+'_14_top_row_facets.png'), dpi=200)
            plt.close()
...
#corners3d = centers3d_to_corners3d(centers3d) 
        ...
        if self.render_control.draw_top_row_facets_labels:
            ...
            plt.savefig(os.path.join(self.frame['output_construction_dir'], self.key_frame_id_str+'_15_top_row_facets_labels.png'), dpi=200)
            plt.close()
...
#corners3d           = centers3d_to_corners3d(centers3d) 
        for heliostat in self.frame['heliostats']:
            ...
            if len(points3d) != len(points2d):
                msg = 'In KeyFrameCornerSearch.project_and_confirm(), len(points3d)='+str(len(points3d))+' does not equal len(points3d)='+str(len(points3d))
                print('ERROR: ' + msg)
                raise ValueError(msg)
            if len(points3d) < 4:
                msg = 'In KeyFrameCornerSearch.project_and_confirm(), len(points3d)='+str(len(points3d))+' is not at least 4.  Not expected here.'
                print('ERROR: ' + msg)
                raise ValueError(msg)
            print('In KeyFrameCornerSearch.project_and_confirm(), calling solvePNP(); len(points3d) =', len(points3d), '; len(points2d) =', len(points2d))  # ?? SCAFFOLDING RCB -- TEMPORARY
            mtx, dist,  \
            rvec, tvec, \
            pnp_error        = solvePNP(points3d, points2d, h, w, pnptype=self.solvePnPtype)
            ...
            confirmed_corners, projected_corners = self.confirm(proj_corners, corners3d, canny_levels=canny_levels,iterations=iterations)
            
            if not projected_corners: # no corner was confirmed, discard this heliostat  # ?? SCAFFOLDING RCB -- DOES THIS EVER WORK, GIVEN THAT WE ARE LOOKING AT OUTPUT OF EXPECTED CORNERS ROUTINE?
                continue
            heliostat['confirmed_corners'] = self.convert_None_corners(confirmed_corners)  # Replace None entries with [-1,-1]
        ...
        for heliostat in final_heliostats:
            all_confirmed_corners += heliostat['confirmed_corners']  # ?? SCAFFOLDING RCB -- AFTER THIS REFACTOR IS COMPLETE, MAYBE THESE ARE NO LONGER NEEDED.
            all_projected_corners += heliostat['projected_corners']  # ?? SCAFFOLDING RCB -- AFTER THIS REFACTOR IS COMPLETE, MAYBE THESE ARE NO LONGER NEEDED.
        ...
        if self.render_control.write_all_confirmed_corners:
            save_corners(corners=all_confirmed_corners, filename='all_confirmed_corners.csv', path=self.frame['output_construction_dir'])  # ?? SCAFFOLDING RCB -- AFTER THIS REFACTOR IS COMPLETE, MAYBE THESE ARE NO LONGER NEEDED.
        if self.render_control.write_all_projected_corners:
            save_corners(corners=all_projected_corners, filename='all_projected_corners.csv', path=self.frame['output_construction_dir'])  # ?? SCAFFOLDING RCB -- AFTER THIS REFACTOR IS COMPLETE, MAYBE THESE ARE NO LONGER NEEDED.
        if self.render_control.write_confirmed_fnxl:
            confirmed_fnxl.save(os.path.join(self.frame['output_construction_dir'], 'csv_files', (self.key_frame_id_str + '_confirmed_fnxl.csv')))  # ?? SCAFFOLDING RCB -- INSTEAD OF ADDING "CSV_FILES" HERE, SHOULD BE PASSED IN THAT WAY.  SAVE TO ANSWER DIRECTORY?  -- PROBABLY NOT; INSEAD SAVE FROM CALLER?
        if self.render_control.write_projected_fnxl:
            projected_fnxl.save(os.path.join(self.frame['output_construction_dir'], 'csv_files', (self.key_frame_id_str + '_projected_fnxl.csv')))  # ?? SCAFFOLDING RCB -- INSTEAD OF ADDING "CSV_FILES" HERE, SHOULD BE PASSED IN THAT WAY.  SAVE TO ANSWER DIRECTORY?  -- PROBABLY NOT; INSEAD SAVE FROM CALLER?
        ...
        if self.render_control.draw_confirmed_corners:
            ...
            for final_heliostat in final_heliostats:
                ...
                if len(found_confirmed_corners) > 0:  # ?? SCAFFOLDING RCB -- MAKE THIS INTEGRATED WITH STANDARD PLOTTING AND RENDER CONTROL ROUTINES.
                    label_xy = g2d.label_point(found_confirmed_corners)  # ?? SCAFFOLDING RCB -- MAKE THIS INTEGRATED WITH STANDARD PLOTTING AND RENDER CONTROL ROUTINES.
                    plt.text(label_xy[0],  # ?? SCAFFOLDING RCB -- MAKE THIS INTEGRATED WITH STANDARD PLOTTING AND RENDER CONTROL ROUTINES.
                            label_xy[1],  # ?? SCAFFOLDING RCB -- MAKE THIS INTEGRATED WITH STANDARD PLOTTING AND RENDER CONTROL ROUTINES.
                            final_heliostat['name'],  # ?? SCAFFOLDING RCB -- MAKE THIS INTEGRATED WITH STANDARD PLOTTING AND RENDER CONTROL ROUTINES.
                            ...
                            fontweight='bold')  # ?? SCAFFOLDING RCB -- MAKE THIS INTEGRATED WITH STANDARD PLOTTING AND RENDER CONTROL ROUTINES.
                # Draw confirmed corners.
                for corner in found_confirmed_corners:
                    if corner is not None:
                        plt.scatter(corner[0], corner[1], s=1, facecolor='c')
            # Save the figure.
            plt.savefig(os.path.join(self.frame['output_construction_dir'], self.key_frame_id_str+'_16_confirmed_corners.png'), dpi=200)
            plt.close()
        ...
        if self.render_control.draw_projected_corners:
            ...
            for final_heliostat in final_heliostats:
                ...
                # Draw the heliostat name.
                if len(projected_corners) > 0:  # ?? SCAFFOLDING RCB -- MAKE THIS INTEGRATED WITH STANDARD PLOTTING AND RENDER CONTROL ROUTINES.
                    label_xy = g2d.label_point(projected_corners)  # ?? SCAFFOLDING RCB -- MAKE THIS INTEGRATED WITH STANDARD PLOTTING AND RENDER CONTROL ROUTINES.
                    plt.text(label_xy[0],  # ?? SCAFFOLDING RCB -- MAKE THIS INTEGRATED WITH STANDARD PLOTTING AND RENDER CONTROL ROUTINES.
                            label_xy[1],  # ?? SCAFFOLDING RCB -- MAKE THIS INTEGRATED WITH STANDARD PLOTTING AND RENDER CONTROL ROUTINES.
                            final_heliostat['name'],  # ?? SCAFFOLDING RCB -- MAKE THIS INTEGRATED WITH STANDARD PLOTTING AND RENDER CONTROL ROUTINES.
            ...
            # Save the figure.
            plt.savefig(os.path.join(self.frame['output_construction_dir'], self.key_frame_id_str+'_17_projected_corners.png'), dpi=200)
            plt.close()
            ...
            for final_heliostat in final_heliostats:
                ...
                for corner in found_confirmed_corners:
                    if corner is not None:
                        plt.scatter(corner[0], corner[1], s=1, facecolor='c')
            # Save the figure.
            plt.savefig(os.path.join(self.frame['output_construction_dir'], self.key_frame_id_str+'_18_projected_and_confirmed_corners.png'), dpi=200)
            plt.close()
        ...
        final_heliostats = sorted(final_heliostats, key=lambda x: np.mean(np.array(x['projected_corners']), axis=0)[0])  # ?? SCAFFOLDING RCB -- MAY NOT DO ANYTHING, IN WHICH CASE WE SHOULD DELETE.  WILL BE INCORRECT IF BOXES ARE NOT SORTED LEFT TO RIGHT?


    def confirm(self, expected_corners, corners3d, canny_levels=['tight', 'normal', 'light'], tolerance=3, pixels=100, iterations=5):
        h, w            = self.frame['key_frame_img'].shape[:2]
        max_row         = self.frame['key_frame_img'].shape[0]
        max_col         = self.frame['key_frame_img'].shape[1]
        ...
        def find_corners(confirmed_facets):
            ...
            for facet_indx, facet in confirmed_facets.items():
                ...
                for i, j in zip(range(indx, indx+self.specifications.corners_per_facet), range(0, self.specifications.corners_per_facet)):
        ...
        for i in range(0, iterations):
            ...
            for canny_type in canny_types:
                img   = self.frame['key_frame_img']  # ?? SCAFFOLDING RCB -- SEE CANNY() AND CONFIRM() ROUTINES FOR DUPLICATE PLACES WHERE THIS CODE IS PLACED.  MUST KEEP CONSISTENT.  # ?? SCAFFOLDING RCB -- REDUNDANT IMAGE LOAD, BLUR, AND EDGE FINDING.  COMPUTE ONCE, CACHE AND COMMUNICATE.
                img   = cv.GaussianBlur(img, (5,5), 0)  # ?? SCAFFOLDING RCB -- SEE CANNY() AND CONFIRM() ROUTINES FOR DUPLICATE PLACES WHERE THIS CODE IS PLACED.  MUST KEEP CONSISTENT.  # ?? SCAFFOLDING RCB -- REDUNDANT IMAGE LOAD, BLUR, AND EDGE FINDING.  COMPUTE ONCE, CACHE AND COMMUNICATE.
                edges = CannyImg(img, canny_type=canny_type)  # ?? SCAFFOLDING RCB -- SEE CANNY() AND CONFIRM() ROUTINES FOR DUPLICATE PLACES WHERE THIS CODE IS PLACED.  MUST KEEP CONSISTENT.  # ?? SCAFFOLDING RCB -- REDUNDANT IMAGE LOAD, BLUR, AND EDGE FINDING.  COMPUTE ONCE, CACHE AND COMMUNICATE.
#edges                           = CannyImg(self.frame['sky'], canny_type=canny_type)  # ?? SCAFFOLDING RCB -- ORIGINAL CODE, MULTIPLE FAILURE IMPLICATIONS:  (1) USING SKY, WHEN SKY WAS NOT USERED PREVIOUSLY.  (2) CAUSES OPENCV TO CRASH.  (THANKFULLY; OTHERWISE I WOULDN'T HAVE FOUND THE OTHER BUG.)
                ...
                if len(points3d) != len(points2d):
                    msg = 'In KeyFrameCornerSearch.confirm(), len(points3d)='+str(len(points3d))+' does not equal len(points2d)='+str(len(points2d))
                    print('ERROR: ' + msg)
                    raise ValueError(msg)
