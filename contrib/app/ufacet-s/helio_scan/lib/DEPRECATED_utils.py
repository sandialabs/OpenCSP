...
CameraMatrix    = np.array([[FX,    0,      CX],   # ?? SCAFFOLDING RCB -- USED TO BE SONY ABOVE (NOT [FRAME])
                            [0,     FY,     CY],   # ?? SCAFFOLDING RCB -- USED TO BE SONY ABOVE (NOT [FRAME])
                            [0,     0,      1]]).reshape(3,3)  # ?? SCAFFOLDING RCB -- USED TO BE SONY ABOVE (NOT [FRAME])
...
MINIMUM_CORNERS_REQUIRED_INSIDE_FRAME = 0.7  # This is a measure of sufficiency of information.  This defines the minimum 
...
def save_image(img, imgname, path):
    ...
    if img is None:  # ?? SCAFFOLDING RCB -- TEMPORARY
        print('WARNING: In save_image(), img==None encountered for output:', os.path.join(path, imgname))  # ?? SCAFFOLDING RCB -- TEMPORARY
    if img is not None:
...
def save_fig(img=None, imgname=None, path=None, dpi=500, rgb=False):
    ...
    if img is None:  # ?? SCAFFOLDING RCB -- TEMPORARY
        print('WARNING: In save_fig(), img==None encountered for output:', os.path.join(path, imgname))  # ?? SCAFFOLDING RCB -- TEMPORARY
    if img is not None:
        plt.figure()
        if not rgb:
...
def extract_frames_from_video(video_path=None, video_name=None, saving_path=None, fps=30, starting_time=None, duration=None, backward=False):
    cmd = 'ffmpeg'
    if starting_time is not None and starting_time != '':
        ...
        if duration is not None:
            ...
            cmd += ' -i ' + video_path + video_name + ' -vf fps=' + str(fps) + ' ' + '-start_number ' + str(starting_id) + ' ' + saving_path + '/img%d.png'
        else:
            if int(secs) == 59:
            ...
            cmd += ' -i ' + video_path + video_name + '.mp4' ' -ss ' + starting_time + ' -to ' + next_time_instance + ' -vf fps=' + str(fps) + ' ' + saving_path + 'img%d.png'
    
    subprocess.call(cmd.split())

def extract_frames_nopipe(video_path=None,
                          video_name=None,
    ...
    command = [
        'ffmpeg',
        '-nostats', '-loglevel', '0',
        '-ss', str(starting_frame_id/fps),
        '-i', video_path + video_name + '.mp4',
        '-to', str(ending_frame_id/fps),
        '-vf', str(fps),
        str(saving_path)
        # '-c:v', 'ffv1',
...
def extract_frames_opencv(video=None, start=None, end=None,
                          saving_path=None, every=1, plot=False,
                          store=False):
...
def extract_specific_frames_pipe(video_path=None, video_name=None, 
                                 starting_frame_id=None,
                                 ending_frame_id=None,
                                 fps=30,
                                 height=2160,
...
def fit_line_component(component=None, type_fit='regression', plot_fit=False):
    ...
    if type_fit == 'regression':
        ...
        if plot_fit:
            plot_line(pixels=pixels, points=component['original_line_points'], hom_coef=component['original_line_hom_coef'])
...
def solvePNP(points3d, points2d, h, w, pnptype='pnp', cam_matrix=None, dist_coeff=None):
    ...
    if len(points3d) != len(points2d):
        msg = 'In solvePNP(), len(points3d)='+str(len(points3d))+' does not equal len(points3d)='+str(len(points3d))
        print('ERROR: ' + msg)
    ...
    if pnptype == 'calib':
        ...
        _, mtx, dist, rvecs, tvecs      = cv.calibrateCamera([points3d], [points2d], (w,h), mtx, dist, flags=cv.CALIB_USE_INTRINSIC_GUESS)
