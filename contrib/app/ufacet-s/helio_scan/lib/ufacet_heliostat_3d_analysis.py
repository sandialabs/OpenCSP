...
def translation(heliostat, specifications, confirm=False):
    # heliostat center
    center = facet_center(heliostat[specifications.centered_facet][TOP_LEFT_CORNER_INDX], heliostat[specifications.centered_facet][BOTTOM_RIGHT_CORNER_INDX],
                              heliostat[specifications.centered_facet][TOP_RIGHT_CORNER_INDX], heliostat[specifications.centered_facet][BOTTOM_LEFT_CORNER_INDX])
    ...
    # print new center
    center = facet_center(translated_heliostat[specifications.centered_facet][TOP_LEFT_CORNER_INDX], translated_heliostat[specifications.centered_facet][BOTTOM_RIGHT_CORNER_INDX],
                          translated_heliostat[specifications.centered_facet][TOP_RIGHT_CORNER_INDX], translated_heliostat[specifications.centered_facet][BOTTOM_LEFT_CORNER_INDX])
    if confirm:
...
def find_best_translation(heliostat, heliostat_theoretical_dict=None):
    ...
    for key, corners in sorted(heliostat.items()):
        ...
        mid_point  = [(mid_point1[0] + mid_point2[0])/2, (mid_point1[1] + mid_point2[1])/2, (mid_point1[2] + mid_point2[2])/2]
...
def rotation_around_surface_normal(heliostat, heliostat_theoretical_dict=None):
    ...
    a   = np.array(heliostat[BOTTOM_RIGHT_FACET_INDX][BOTTOM_RIGHT_CORNER_INDX]) - np.array(heliostat[TOP_LEFT_FACET_INDX][TOP_LEFT_CORNER_INDX])
    a   = a / np.linalg.norm(a)
    
    b   = np.array(heliostat[BOTTOM_LEFT_FACET_INDX][BOTTOM_LEFT_CORNER_INDX]) - np.array(heliostat[TOP_RIGHT_FACET_INDX][TOP_RIGHT_CORNER_INDX])
    b   = b / np.linalg.norm(b)

    a_th            = np.array(heliostat_theoretical_dict[BOTTOM_RIGHT_FACET_INDX][BOTTOM_RIGHT_CORNER_INDX]) - np.array(heliostat_theoretical_dict[TOP_LEFT_FACET_INDX][TOP_LEFT_CORNER_INDX])
    a_th            = a_th / np.linalg.norm(a_th)

    b_th            = np.array(heliostat_theoretical_dict[BOTTOM_LEFT_FACET_INDX][BOTTOM_LEFT_CORNER_INDX]) - np.array(heliostat_theoretical_dict[TOP_RIGHT_FACET_INDX][TOP_RIGHT_CORNER_INDX])
...
def scaling(heliostat, specifications, heliostat_theoretical_dict=None):
    ...
    center_above    = facet_center(facet[TOP_LEFT_CORNER_INDX], facet[BOTTOM_RIGHT_CORNER_INDX], facet[TOP_RIGHT_CORNER_INDX], facet[BOTTOM_LEFT_CORNER_INDX])

    facet           = heliostat[specifications.centered_facet+specifications.facets_per_row ]
    center_below    = facet_center(facet[TOP_LEFT_CORNER_INDX], facet[BOTTOM_RIGHT_CORNER_INDX], facet[TOP_RIGHT_CORNER_INDX], facet[BOTTOM_LEFT_CORNER_INDX])
...
def translate_rotate_scale(input_heliostat, option, specifications=None, heliostat_theoretical_dict=None, confirm=False):
...
def plot_heliostat_3d(heliostat, specifications, heliostat_theoretical_dict=None, theoretical_flag=False, saving_path=' ', hel_name=' ', title_prefix=' ', option='normalHel',
                      frame_camera_pose_dict=None, camera_suffix=None):
...
#    ax = plt.axes(projection='3d')  # Perspective projection
    ...
    if theoretical_flag:
        corners = [heliostat_theoretical_dict[TOP_LEFT_FACET_INDX][TOP_LEFT_CORNER_INDX], heliostat_theoretical_dict[TOP_RIGHT_FACET_INDX][TOP_RIGHT_CORNER_INDX],
                    heliostat_theoretical_dict[BOTTOM_RIGHT_FACET_INDX][BOTTOM_RIGHT_CORNER_INDX], heliostat_theoretical_dict[BOTTOM_LEFT_FACET_INDX][BOTTOM_LEFT_CORNER_INDX]]
        ...
        # draw center
        center = facet_center(heliostat_theoretical_dict[specifications.centered_facet][TOP_LEFT_CORNER_INDX], heliostat_theoretical_dict[specifications.centered_facet][BOTTOM_RIGHT_CORNER_INDX],
                                heliostat_theoretical_dict[specifications.centered_facet][TOP_RIGHT_CORNER_INDX], heliostat_theoretical_dict[specifications.centered_facet][BOTTOM_LEFT_CORNER_INDX])
    ...
    # draw top corners
    corners = [heliostat[TOP_LEFT_FACET_INDX][TOP_LEFT_CORNER_INDX], heliostat[TOP_RIGHT_FACET_INDX][TOP_RIGHT_CORNER_INDX],
                heliostat[BOTTOM_RIGHT_FACET_INDX][BOTTOM_RIGHT_CORNER_INDX], heliostat[BOTTOM_LEFT_FACET_INDX][BOTTOM_LEFT_CORNER_INDX]]
    xdata   = [corner[0] for corner in corners]
    ydata   = [corner[1] for corner in corners]
    zdata   = [corner[2] for corner in corners]
    ax.scatter3D(xdata[0], ydata[0], zdata[0], facecolor='tab:blue')
    ...
    if frame_camera_pose_dict != None:
        ...
        idx = 0
        for rvec_tvec in dt.list_of_values_in_sorted_key_order(frame_camera_pose_dict):  # ?? IF WE MAKE FRAME_CAMERA_POSE_DICT A CLASS, THIS WILL NEED TO BE UPDATED.
            idx += 1
            rvec = rvec_tvec[0]
            tvec = rvec_tvec[1]
            t_x = tvec[0][0]
            ...
            ax.scatter3D(-t_x, t_y, t_z, s=0.5, label=str(idx))  # 2021-11-14 4:00 AM.  Discovered that tvec_x coordinate is reversed compared to heliostat x axis.
...
def plot_heliostat_2d(heliostat, specifications, heliostat_theoretical_dict=None, theoretical_flag=False, axes_equal=True, plot_surface_normals=False, saving_path=' ', hel_name=' ', title_prefix=' ', option='normalHel', normal_wrt_average=False):
...
#needle_scale = 150
        ...
        if normal_wrt_average:
            ...
            uy_sum = 0
            u_count = 0
            for _, corners in sorted(heliostat.items()):  # ?? SCAFFOLDING RCB -- THIS IS CRUFTY; REFACTOR TO COMPUTE IN A A SINGLE ANALYTIC PLACE, CONSISTENTLY.
...
def plot_canting_angles(heliostat, specifications, heliostat_theoretical_dict=None, directions_cosines=False, saving_path=' ', hel_name=' ', title_prefix=' ', option='normalHel', plot=True, normal_wrt_average=False):
    anglesx         = []
    anglesy         = []
    anglesz         = []
    canting_angles  = {}
    ...
    if normal_wrt_average:  # ?? SCAFFOLDING RCB -- THIS IS CRUFTY.  THIS SHOULD GO IN AN ANALYSIS PLACE, AND ONLY COMPUTE ONCE.  SEE ALSO SIMILAR CODE IN PLOT NORMALS FUNCTION.
    ...
    if plot:
        ...
        plt.ylim(-y_axis_max, y_axis_max)  # ?? SCAFFOLDING RCB -- THIS MAKES SCALES SIMILAR ACROSS HELIOSTATS, BUT WILL BE TOO SMALL FOR SOME EXTREME ERRORS, AND ALSO DOESN'T ADJUST FOR DIFFERENT SOALR FIELDS.  RECTIFY THIS.
...
def plot_corners_coordinates_differences(heliostat, specifications, heliostat_theoretical_dict=None, xy=True, saving_path=' ', hel_name=' ', title_prefix=' ', option='normalHel'):
    top_left     = []
    top_right    = []
    bottom_right = []
    bottom_left  = []
    ...
    df = pd.DataFrame({'Top Left Corner': top_left,
                       'Top Right Corner': top_right,
                       'Bottom Right Corner': bottom_right,
                       'Bottom Left Corner': bottom_left}, index=[i+1 for i in range(0, specifications.facets_per_heliostat)])
    ax = df.plot.bar(rot=0, color={"Top Left Corner": "blue", "Top Right Corner": "red", "Bottom Right Corner": "green", "Bottom Left Corner":"magenta"})
    if xy:
...
def plot_pose_estimation(heliostat, specifications, heliostat_theoretical_dict=None, best_translation=True, saving_path=' ', hel_name=' ', title_prefix=' ', option='normalHel', plot=True):
    ...
    if plot:
        df = pd.DataFrame({'X offset': xdiff, 'Y offset':ydiff, 'Z offset':zdiff}, index=[i+1 for i in range(0, specifications.facets_per_heliostat)])
        ax = df.plot.bar(rot=0, color={"X offset": "tab:blue", "Y offset":"tab:orange", "Z offset":"cyan"}, figsize=(15,10))
        plt.grid(axis='y')
...
def plot_pose_rotation_estimation(heliostat, specifications, heliostat_theoretical_dict=None, best_translation=True, saving_path=' ', hel_name=' ', title_prefix=' ', option='normalHel', plot=True):
    ...
    if plot:
        df = pd.DataFrame({'Rotation About Z': rot_z_deg_list}, index=[i+1 for i in range(0, specifications.facets_per_heliostat)])
...
def plot_square_sides_quality(heliostat, specifications, heliostat_theoretical_dict=None, best_scaling=True, saving_path=' ', hel_name=' ', title_prefix=' ', option='normalHel', plot=True):
    ...
    for key, corners in sorted(heliostat.items()):
        ...
        top_side.append(top)
        top_error.append(top-specifications.facet_width )  # ?? SCAFFOLDING RCB -- THESE BASIC ERROR DIFFERENCES ARE RECOMPUTED SEVERAL TIMES BELOW.  ELIMINATE REDUNDANT CALCULATION AND OPPORTUNITY FOR ERROR
        ...
        square_sides_errors[key] = [top-specifications.facet_width , right-specifications.facet_height , bottom-specifications.facet_width , left-specifications.facet_height ]

        rmse += (top-specifications.facet_width )**2 + (right-specifications.facet_height )**2 + (bottom-specifications.facet_width )**2 + (left-specifications.facet_height )**2
        mean_error = abs(top-specifications.facet_width ) + abs(right-specifications.facet_height ) + abs(bottom - specifications.facet_width ) + abs(left - specifications.facet_height )
    ...
    if plot:
        ...
        df = pd.DataFrame({'Top Side Error': top_error,'Bottom Side Error': bottom_error, 'Right Side Error': right_error, 'Left Side Error': left_error}, index=[i+1 for i in range(0, specifications.facets_per_heliostat)])
        ax = df.plot.bar(rot=0, color={"Top Side Error": "tab:blue", "Right Side Error": "tab:orange", "Bottom Side Error": "tab:olive", "Left Side Error":"tab:purple"}, figsize=(15,10))
        plt.title(title_prefix)
        plt.xlabel('Facet id')
        plt.ylabel('Meters')
...
def plot_square_diagonals_quality(heliostat, specifications, heliostat_theoretical_dict=None, best_scaling=True, saving_path=' ', hel_name=' ', title_prefix=' ', option='normalHel', plot=True):
    ...
    if plot:
        ...
        df = pd.DataFrame({'BottomLeft-TopRight Error': bltr_error_list, 'BottomRight-TopLeft Error': brtl_error_list}, index=[i+1 for i in range(0, specifications.facets_per_heliostat)])
        ax = df.plot.bar(rot=0, color={"BottomLeft-TopRight Error": "tab:blue", "BottomRight-TopLeft Error": "tab:orange"}, figsize=(15,10))
        plt.title(title_prefix)
        plt.xlabel('Facet id')
        plt.ylabel('Meters')
...
def plot_square_diagonal_offset_quality(heliostat, specifications, heliostat_theoretical_dict=None, saving_path=' ', hel_name=' ', title_prefix=' ', option='normalHel', plot=True):
    ...
    if plot:
        df = pd.DataFrame({'Diagonal Offset Distance':diagonals_distance}, index=[i+1 for i in range(0, specifications.facets_per_heliostat)])
...
def draw_annotated_frame_figure(corner_xy_list, input_video_body, input_frame_dir, input_frame_id_format, frame_id, hel_name, explain, output_dir):
    # Construct annotations.
    annotation_list = []
    # Data: Observed corners in image space.
    ...
    # Draw.
    ip.plot_image_figure(frame_img,
...
def draw_annotated_frame_image(corner_xy_list, input_video_body, input_frame_dir, input_frame_id_format, frame_id, hel_name, note, output_dir):
    # Construct annotations.
    annotation_list = []
    # Data: Observed corners in image space.
    ...
    if (note is not None) and len(note) > 0:
        ...
        annotation_list.append(pa.PlotAnnotation('text', [note_xy], note, rctxt.RenderControlText(fontsize=4,
                                                                                                  color='r', 
...
def add_analysis_annotations(corner_xy_list, text, draw_boundaries, draw_markers, draw_label, color, linewidth, markersize, fontsize, annotation_list):
    if draw_boundaries:
...
def analyze_and_render_heliostat_3d(flat_hel_dir_body_ext, hel_dir_body_ext, explain, specifications, output_dir, 
                                    camera_rvec=None, camera_tvec=None, camera_matrix=None, distortion_coefficients=None,
                                    tracked_frame_camera_pose_dict=None, processed_frame_camera_pose_dict=None):
    ...
    msg += '.'
    print(msg)
    generate_plots(hel_dir_body_ext, output_dir, specifications, flat_heliostat, explain=explain_2,
                   ...
                   tracked_frame_camera_pose_dict=tracked_frame_camera_pose_dict, processed_frame_camera_pose_dict=processed_frame_camera_pose_dict)
...
def generate_plots(filename, output_path, specifications, heliostat_theoretical_dict, explain=None, 
                   camera_rvec=None, camera_tvec=None, camera_matrix=None, distortion_coefficients=None,
                   tracked_frame_camera_pose_dict=None, processed_frame_camera_pose_dict=None):
...
# heliostat_optical       = translate_rotate_scale(heliostat,     specifications=specifications, heliostat_theoretical_dict=heliostat_theoretical_dict, option='normalOptical',   confirm=False)  # ?? SCAFFOLDING RCB -- DISABLE, PROBABLY REMOVE                
    ...
    plot_heliostat_2d(heliostat, specifications=specifications, heliostat_theoretical_dict=heliostat_theoretical_dict, theoretical_flag=True,  plot_surface_normals=False, saving_path=heliostat_path, hel_name=hel_name, title_prefix=title_prefix, option='noRotate')
    ...
    plot_heliostat_2d(heliostat, specifications=specifications, heliostat_theoretical_dict=heliostat_theoretical_dict, theoretical_flag=False,  plot_surface_normals=True, saving_path=heliostat_path, hel_name=hel_name, title_prefix=title_prefix, option='noRotate')
    plot_heliostat_2d(heliostat, specifications=specifications, heliostat_theoretical_dict=heliostat_theoretical_dict, theoretical_flag=False,  plot_surface_normals=True, saving_path=heliostat_path, hel_name=hel_name, title_prefix=title_prefix, option='wrtAvg', normal_wrt_average=True)
    """
    ...
    plot_heliostat_3d(heliostat=heliostat, specifications=specifications, heliostat_theoretical_dict=heliostat_theoretical_dict, theoretical_flag=True, saving_path=heliostat_path, hel_name=hel_name, title_prefix=title_prefix, option='noRotate')
    plot_heliostat_3d(heliostat=heliostat, specifications=specifications, heliostat_theoretical_dict=heliostat_theoretical_dict, theoretical_flag=False, saving_path=heliostat_path, hel_name=hel_name, title_prefix=title_prefix, option='noRotate')
    ...
    if processed_frame_camera_pose_dict != None:
        plot_heliostat_3d(heliostat=heliostat, specifications=specifications, heliostat_theoretical_dict=heliostat_theoretical_dict, theoretical_flag=False, saving_path=heliostat_path, hel_name=hel_name, title_prefix=title_prefix + 'Processed camera locations for', option='noRotate', frame_camera_pose_dict=processed_frame_camera_pose_dict, camera_suffix='process_cameras')
    ...
    plot_canting_angles(heliostat, specifications=specifications, heliostat_theoretical_dict=heliostat_theoretical_dict, saving_path=heliostat_path,     hel_name=hel_name, title_prefix=title_prefix, option='noRotate')
    plot_canting_angles(heliostat, specifications=specifications, heliostat_theoretical_dict=heliostat_theoretical_dict, saving_path=heliostat_path,     hel_name=hel_name, title_prefix=title_prefix, option='wrtAvg', normal_wrt_average=True)
    ...
    plot_pose_estimation(heliostat, specifications=specifications, heliostat_theoretical_dict=heliostat_theoretical_dict, best_translation=False, saving_path=heliostat_path, hel_name=hel_name, title_prefix=title_prefix, option='noRotateNoTranslate')
    """
    ...
    plot_pose_rotation_estimation(heliostat, specifications=specifications, heliostat_theoretical_dict=heliostat_theoretical_dict, best_translation=False, saving_path=heliostat_path, hel_name=hel_name, title_prefix=title_prefix, option='noRotateNoTranslate')
    ...
    plot_square_sides_quality(heliostat, specifications=specifications, heliostat_theoretical_dict=heliostat_theoretical_dict, best_scaling=False, saving_path=heliostat_path, hel_name=hel_name, title_prefix=title_prefix, option='noRotateNoScale')
    ...
    plot_square_diagonals_quality(heliostat, specifications=specifications, heliostat_theoretical_dict=heliostat_theoretical_dict, best_scaling=False, saving_path=heliostat_path, hel_name=hel_name, title_prefix=title_prefix, option='noRotateNoScale')
    ...
    plot_square_diagonal_offset_quality(heliostat, specifications=specifications, heliostat_theoretical_dict=heliostat_theoretical_dict, saving_path=heliostat_path, hel_name=hel_name, title_prefix=title_prefix, option='noRotate')
...
def generate_csv(hel_dir_body_ext, output_path, specifications, heliostat_theoretical_dict, option):
    ...
    hel_name = heliostat_name_given_heliostat_3d_dir_body_ext(hel_dir_body_ext)  # ?? SCAFFOLDING RCB -- CRUFTY.  FIX THIS.
...
#heliostat  = translate_rotate_scale(heliostat, specifications=specifications, heliostat_theoretical_dict=heliostat_theoretical_dict, option=option, confirm=False)  # ?? SCAFFOLDING RCB -- DISABLE, PROBABLY REMOVE                
    ...
    Canting Angles
    """
    canting_angles[hel_name]    = plot_canting_angles(heliostat, specifications=specifications, heliostat_theoretical_dict=heliostat_theoretical_dict, hel_name=hel_name, plot=False, option=option)
    save_csv(canting_angles, output_path=output_path, name=hel_name+'_canglesXYZ_' + option + '.csv')
    wrt_avg_option = 'wrtAvg'
    canting_angles[hel_name]    = plot_canting_angles(heliostat, specifications=specifications, heliostat_theoretical_dict=heliostat_theoretical_dict, hel_name=hel_name, plot=False, option=wrt_avg_option, normal_wrt_average=True)
    save_csv(canting_angles, output_path=output_path, name=hel_name+'_canglesXYZ_' + wrt_avg_option + '.csv')
    """
    Pose Estimation
    """
    pose_estimations[hel_name]  = plot_pose_estimation(heliostat, specifications=specifications, heliostat_theoretical_dict=heliostat_theoretical_dict, hel_name=hel_name, plot=False, option=option)
    save_csv(pose_estimations, output_path=output_path, name=hel_name+'_poseXYZ_' + option + '.csv')
    """
    Pose Rotation Estimation
    """
    pose_rotation_estimations[hel_name]  = plot_pose_rotation_estimation(heliostat, specifications=specifications, heliostat_theoretical_dict=heliostat_theoretical_dict, hel_name=hel_name, plot=False, option=option)
    save_csv(pose_rotation_estimations, output_path=output_path, name=hel_name+'_poseRotZ_' + option + '.csv')
    """
    Square Sides Quality Error
    """
    square_sides_errors[hel_name] = plot_square_sides_quality(heliostat, specifications=specifications, heliostat_theoretical_dict=heliostat_theoretical_dict, hel_name=hel_name, plot=False, option=option)
    save_csv(square_sides_errors, output_path=output_path, name=hel_name+'_serrorTRBL_' + option + '.csv')
    """
    Square Diagonals Quality Error
    """
    square_diagonals_errors[hel_name] = plot_square_diagonals_quality(heliostat, specifications=specifications, heliostat_theoretical_dict=heliostat_theoretical_dict, hel_name=hel_name, plot=False, option=option)
    ...
    """
    square_diagonal_offsets[hel_name] = plot_square_diagonal_offset_quality(heliostat, specifications=specifications, heliostat_theoretical_dict=heliostat_theoretical_dict, hel_name=hel_name, plot=False, option=option)
...
def heliostat_name_given_heliostat_3d_dir_body_ext(heliostat_3d_dir_body_ext):  # ?? SCAFFOLDING RCB -- MAKE THIS GENERAL, CORRECT, ERROR-CHECKING.  (IT'S LATE, AND I'M OUT OF TIME.)
...
def corners_3d_dir_body_ext(input_video_body, hel_name, projected_or_confirmed_str, distorted_or_undistorted_str, corners_3d_dir):
...
def save_heliostat_3d(hel_name, 
                      corner_xyz_list,
                      output_dir,
    ...
    output_heliostat_3d_dir_body_ext = corners_3d_dir_body_ext(input_video_body, hel_name, projected_or_confirmed_str, distorted_or_undistorted_str, output_dir)
...
def plot_heliostat_with_camera_poses(hel_name,
...
#print('In plot_heliostat_with_camera_poses_aux(), t_x, t_y, t_z = ', t_x, t_y, t_z)
        x_list.append(-t_x)  # 2021-11-14 4:00 AM.  Discovered that tvec_x coordinate is reversed compared to heliostat x axis.
        y_list.append(t_y)
        z_list.append(t_z)
    # Plot.
