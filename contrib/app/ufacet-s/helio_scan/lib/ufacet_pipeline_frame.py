...
def frame_id_str_given_prefix_number_KeyWord_body_ext(prefix_number_keyword_body_ext, keyword):
    ...
    if len(components) < 2:
        msg = 'ERROR: In frame_id_str_given_prefix_number_KeyWord_body_ext(), input prefix_number_keyword_body_ext="'+str(prefix_number_keyword_body_ext)+'" did not separate into at least two components when split by "."'
        print(msg)
        raise ValueError(msg)
    ext  = components[-1]
    body = components[-2]
    if ext != 'csv':
        msg = 'ERROR: In frame_id_str_given_prefix_number_KeyWord_body_ext(), input prefix_number_keyword_body_ext="'+str(prefix_number_keyword_body_ext)+'" did not end with extension ".csv"'
        print(msg)
        raise ValueError(msg)
    # Break the filename into components.
    components = body.split('_')
    if len(components) < 4:
        msg = 'ERROR: In frame_id_str_given_prefix_number_KeyWord_body_ext(), input prefix_number_keyword_body_ext="'+str(prefix_number_keyword_body_ext)+'" file body did not separate into at least four components when split by "_"'
        print(msg)
        raise ValueError(msg)
    ...
    if fxnl_str != 'fnxl':
        msg = 'ERROR: In frame_id_str_given_prefix_number_KeyWord_body_ext(), input prefix_number_keyword_body_ext="'+str(prefix_number_keyword_body_ext)+'" body did not end with substring "fnxl"'
        print(msg)
        raise ValueError(msg)
    if keyword_str != keyword:
        msg = 'ERROR: In frame_id_str_given_prefix_number_KeyWord_body_ext(), input prefix_number_keyword_body_ext="'+str(prefix_number_keyword_body_ext)+'" did not include second-to-last substring "'+str(keyword)+'"'
        print(msg)
        raise ValueError(msg)
    # Determine if the "number_str" component is a valid integer.
    if not mt.string_is_integer(number_str):
        msg = 'ERROR: In frame_id_str_given_prefix_number_KeyWord_body_ext(), input prefix_number_keyword_body_ext="'+str(prefix_number_keyword_body_ext)+'" has a third-to-last substring "'+str(number_str)+'" that is not a valid integer.'
        print(msg)
        raise ValueError(msg)
...
def frame_id_str_given_prefix_number_Adjective_KeyWord_body_ext(prefix_number_adjective_keyword_body_ext, adjective, keyword):
    # For multiple items (key corners, key frame tracks, ...), our canonical file names are of the form:
    ...
    if len(components) < 2:
        msg = 'ERROR: In frame_id_str_given_prefix_number_Adjective_KeyWord_body_ext(), input prefix_number_adjective_keyword_body_ext="'+str(prefix_number_adjective_keyword_body_ext)+'" did not separate into at least two components when split by "."'
        print(msg)
        raise ValueError(msg)
    ext  = components[-1]
    body = components[-2]
    if ext != 'csv':
        msg = 'ERROR: In frame_id_str_given_prefix_number_Adjective_KeyWord_body_ext(), input prefix_number_adjective_keyword_body_ext="'+str(prefix_number_adjective_keyword_body_ext)+'" did not end with extension ".csv"'
        print(msg)
        raise ValueError(msg)
    # Break the filename into components.
    components = body.split('_')
    if len(components) < 5:
        msg = 'ERROR: In frame_id_str_given_prefix_number_Adjective_KeyWord_body_ext(), input prefix_number_adjective_keyword_body_ext="'+str(prefix_number_adjective_keyword_body_ext)+'" file body did not separate into at least five components when split by "_"'
        print(msg)
        raise ValueError(msg)
    fxnl_str      = components[-1]
    keyword_str   = components[-2]
    ...
    if fxnl_str != 'fnxl':
        msg = 'ERROR: In frame_id_str_given_prefix_number_Adjective_KeyWord_body_ext(), input prefix_number_adjective_keyword_body_ext="'+str(prefix_number_adjective_keyword_body_ext)+'" body did not end with substring "fnxl"'
        print(msg)
        raise ValueError(msg)
    if keyword_str != keyword:
        msg = 'ERROR: In frame_id_str_given_prefix_number_Adjective_KeyWord_body_ext(), input prefix_number_adjective_keyword_body_ext="'+str(prefix_number_adjective_keyword_body_ext)+'" did not include second-to-last substring "'+str(keyword)+'"'
        print(msg)
        raise ValueError(msg)
    if adjective_str != adjective:
        msg = 'ERROR: In frame_id_str_given_prefix_number_Adjective_KeyWord_body_ext(), input prefix_number_adjective_keyword_body_ext="'+str(prefix_number_adjective_keyword_body_ext)+'" did not include second-to-last substring "'+str(adjective)+'"'
        print(msg)
        raise ValueError(msg)
    # Determine if the "number_str" component is a valid integer.
    if not mt.string_is_integer(number_str):
        msg = 'ERROR: In frame_id_str_given_prefix_number_Adjective_KeyWord_body_ext(), input prefix_number_adjective_keyword_body_ext="'+str(prefix_number_adjective_keyword_body_ext)+'" has a third-to-last substring "'+str(number_str)+'" that is not a valid integer.'
        print(msg)
        raise ValueError(msg)
    # Return the number string, without parsing.
    return number_str
...
def frame_id_str_given_frame_file_body_ext(frame_file_body_ext):
    ...
    if len(components) < 3:
        msg = 'ERROR: In frame_id_str_given_frame_file_body_ext(), input frame_file_body_ext="'+str(frame_file_body_ext)+'" did not separate into at least three components when split by "."'
        print(msg)
        raise ValueError(msg)
    ext        = components[-1]
    ...
    if not mt.string_is_integer(number_str):
        msg = 'ERROR: In frame_id_str_given_frame_file_body_ext(), input frame_file_body_ext="'+str(frame_file_body_ext)+'" has a second-to-last substring "'+str(number_str)+'" that is not a valid integer.'
        print(msg)
        raise ValueError(msg)
    # Return the number string, without parsing.
    return number_str
...
def draw_example_frame(input_full_frame_dir, 
                       input_frame_file, 
                       output_render_dir,
                       render_control,
                       include_figure_idx_in_filename=False):
    # Load frame.
    input_dir_body_ext = os.path.join(input_full_frame_dir, input_frame_file)  # input_frame_file includes the extension.
...
def draw_example_frames(input_full_frame_dir,                   # Directory containing the full list of full-size frames.
                        ...
                        include_figure_idx_in_filename=False):  # Whether to include the figure index in the output figure files.
    """
    ...
    if n_frames > 0:
        ...
        if len(sample_idx_list) > 0:
            ...
            for sample_idx in sample_idx_list:
                draw_example_frame(input_full_frame_dir,
