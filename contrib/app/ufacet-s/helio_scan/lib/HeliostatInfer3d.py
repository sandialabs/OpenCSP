...
class HeliostatInfer3d():
    ...
    def __init__(self,
                 ...
                 # Render control.
                 render_control):                            # Flags to control rendering; e.g., whether to output intermediate construction steps.
        ...
        self.theoretical_flat_heliostat_spec           = self.specifications.construct_flat_heliostat_spec()  # ?? SCAFFOLDING RCB -- THIS IS CONSTRUCTED HERE INSTEAD OF WITH THE OTHER THEORETICAL FLAT ITEMS.  POSSIBLE INCONSISTENCY.  RESOLVE THIS.
        ...
        msg_line = tdt.current_time_string() + ' ' + str(hel_name) + ' constructing ideal heliostat...'  # ?? SCAFFOLDING RCB -- ENCAPSULATE THESE INTO A "LOG" MEMBER FUNCTION.
        ...
        self.design_heliostat_xyz_list = self.specifications.heliostat_corner_xyz_list_given_heliostat_spec(self.design_heliostat_spec)
        self.design_heliostat_spec,          \
        self.design_heliostat_xyz_list,      \
        self.design_heliostat_dir_body_ext,  \
        self.design_heliostat_spec_dir_body_ext = self.save_and_analyze_design_heliostat(self.design_heliostat_spec)

        # Determine which frames to process.
        # (We assemble this list to make adjustment easy during debugging.)
        msg_line = tdt.current_time_string() + ' ' + str(hel_name) + ' analyzing frames...'  # ?? SCAFFOLDING RCB -- ENCAPSULATE THESE INTO A "LOG" MEMBER FUNCTION.
        self.search_log.append(msg_line)  # ?? SCAFFOLDING RCB -- ENCAPSULATE THESE INTO A "LOG" MEMBER FUNCTION.
        print('\n'+msg_line)  # ?? SCAFFOLDING RCB -- ENCAPSULATE THESE INTO A "LOG" MEMBER FUNCTION.
        ...
        uh3a.plot_heliostat_with_camera_poses(hel_name,
                                              self.design_heliostat_dir_body_ext, 
                                              ...
                                              title_prefix=' ',
                                              explain=None,
                                              tracked_style=rcps.RenderControlPointSeq(linestyle='-',
                                                                                       linewidth=0.3,
                                                                                       color='m',
                                                                                       marker='+', #'o',
                                                                                       markersize=3),#0.4),
                                              processed_style=rcps.RenderControlPointSeq(linestyle='-',
                                                                                         linewidth=0.5,
                                                                                         marker='o',
                                                                                         markersize=0.7,
                                                                                         markeredgewidth=0.2,
        ...
        msg_line = tdt.current_time_string() + ' ' + str(hel_name) + ' overall error for ' + str(hel_name) + ' design_heliostat_spec = ' + str(design_error_n_pts[0]) + ',  n_points =' + str(design_error_n_pts[1])  # ?? SCAFFOLDING RCB -- ENCAPSULATE THESE INTO A "LOG" MEMBER FUNCTION.
        self.search_log.append(msg_line)  # ?? SCAFFOLDING RCB -- ENCAPSULATE THESE INTO A "LOG" MEMBER FUNCTION.
        print('\n'+msg_line)  # ?? SCAFFOLDING RCB -- ENCAPSULATE THESE INTO A "LOG" MEMBER FUNCTION.
        ...
        msg_line = tdt.current_time_string() + ' ' + str(hel_name) + ' adjusting design heliostat to minimize reprojection error...'  # ?? SCAFFOLDING RCB -- ENCAPSULATE THESE INTO A "LOG" MEMBER FUNCTION.
        self.search_log.append(msg_line)  # ?? SCAFFOLDING RCB -- ENCAPSULATE THESE INTO A "LOG" MEMBER FUNCTION.
        print('\n'+msg_line)  # ?? SCAFFOLDING RCB -- ENCAPSULATE THESE INTO A "LOG" MEMBER FUNCTION.
        self.search_heliostat_spec = copy.deepcopy(self.design_heliostat_spec)
        self.adjust_heliostat_spec_to_minimize_reprojection_error(self.search_heliostat_spec,  # Changed as a side effect.
        ...
        self.save_and_analyze_final_heliostat_spec(self.search_heliostat_spec, self.search_log, self.search_overall_error_history, self.search_facet_error_history, self.final_projected_points_dict)
    ...
    def select_distortion_model(self):
        if self.distorted_or_undistorted_str == 'distorted':
            ...
            print('ERROR: In Heliostats3dInference.select_distortion_model(), unexpected self.distorted_or_undistorted_str="'+str(self.distorted_or_undistorted_str)+'" encountered.')


    def adjust_heliostat_spec_to_minimize_reprojection_error(self,
                                                             search_heliostat_spec,
                                                             n_planar_iterations, 
                                                             n_canting_iterations, 
                                                             n_steps_one_direction,
                                                             variable_steps):
        ...
        for iteration in range(1,(n_planar_iterations+1)):
            ...
            msg_line = tdt.current_time_string() + ' ' + str(self.hel_name) + ' iteration {iteration:2d} overall error={overall_err:11.8f} n_points={n_points:d}'.format(iteration=iteration, overall_err=current_overall_error, n_points=current_n_points_sum)
            self.search_log.append(msg_line)
            print('\n'+msg_line)
            previous_error = current_overall_error
            for facet_idx in range(0,n_facets):
                if not facet_is_converged[facet_idx]:
                    best_var_name, best_min_error, best_min_error_del_var = self.find_best_variable_xyz_rot_z(search_heliostat_spec, facet_to_adjust_idx=facet_idx, n_steps_one_direction=n_steps_one_direction, variable_steps=variable_steps)
                    ...
                    self.search_facet_error_history.append([iteration,facet_idx, best_var_name, best_min_error, best_min_error_del_var])
                    msg_line = tdt.current_time_string() + ' ' + str(self.hel_name) + ' iteration {iteration:2d} facet {facet_idx:2d} min error={min_err:11.8f} after adjust {var_name:8s} by {del_var:11.8f} reduced={err_reduction:11.7f}'.format(iteration=iteration, facet_idx=facet_idx, min_err=best_min_error, var_name=best_var_name, del_var=best_min_error_del_var, err_reduction=error_reduction)
                    self.search_log.append(msg_line)
                    print(msg_line)
            if not (False in facet_is_converged):
                # Then all facets are converged.
                msg_line = tdt.current_time_string() + ' ' + str(self.hel_name) + ' iteration {iteration:2d}; all facets are converged (x,y,z,rot_z).'.format(iteration=iteration)
                self.search_log.append(msg_line)
                print('\n'+msg_line)
                break
        ...
        for iteration in range( (n_planar_iterations+1), (n_planar_iterations+n_canting_iterations+1)):
            ...
            for facet_idx in range(0,n_facets):
                if not facet_is_converged[facet_idx]:
                    best_var_name, best_min_error, best_min_error_del_var = self.find_best_variable_rot_xy(search_heliostat_spec, facet_to_adjust_idx=facet_idx, n_steps_one_direction=n_steps_one_direction, variable_steps=variable_steps)
                    if best_min_error_del_var == 0:
                        facet_is_converged[facet_idx] = True
                    search_heliostat_spec[facet_idx][best_var_name] += best_min_error_del_var
                    error_reduction = previous_error - best_min_error
                    previous_error = best_min_error
                    self.search_facet_error_history.append([iteration,facet_idx, best_var_name, best_min_error, best_min_error_del_var])
                    msg_line = tdt.current_time_string() + ' ' + str(self.hel_name) + ' iteration {iteration:2d} facet {facet_idx:2d} min error={min_err:11.8f} after adjust {var_name:8s} by {del_var:8.5f} reduced={err_reduction:11.7f}'.format(iteration=iteration, facet_idx=facet_idx, min_err=best_min_error, var_name=best_var_name, del_var=best_min_error_del_var, err_reduction=error_reduction)
                    self.search_log.append(msg_line)
                    print(msg_line)
            if not (False in facet_is_converged):
                # Then all facets are converged.
                msg_line = tdt.current_time_string() + ' ' + str(self.hel_name) + ' iteration {iteration:2d}; all facets are converged (rot_x,rot_y).'.format(iteration=iteration)
                self.search_log.append(msg_line)
                print('\n'+msg_line)
                break
    ...
    def find_best_variable_xyz_rot_z(self, search_heliostat_spec, facet_to_adjust_idx, n_steps_one_direction, variable_steps):  # ?? SCAFFOLDING RCB -- EVALUATE WHETHER WE CAN REDUCE THE NUMBER OF DEEPCOPIES, OR REDUCE THE COMPLEXITY OF EACH DEEPCOPY.
        rot_z_min_err, min_err_del_rot_z = self.find_best_value(copy.deepcopy(search_heliostat_spec), facet_to_adjust_idx=facet_to_adjust_idx, var_name='rot_z',    half_range=0.005, n_steps_one_direction=n_steps_one_direction, variable_steps=variable_steps)
        c_x_min_err,   min_err_del_c_x   = self.find_best_value(copy.deepcopy(search_heliostat_spec), facet_to_adjust_idx=facet_to_adjust_idx, var_name='center_x', half_range=0.005, n_steps_one_direction=n_steps_one_direction, variable_steps=variable_steps)
        c_y_min_err,   min_err_del_c_y   = self.find_best_value(copy.deepcopy(search_heliostat_spec), facet_to_adjust_idx=facet_to_adjust_idx, var_name='center_y', half_range=0.005, n_steps_one_direction=n_steps_one_direction, variable_steps=variable_steps)
    ...
    def find_best_variable_rot_xy(self, search_heliostat_spec, facet_to_adjust_idx, n_steps_one_direction, variable_steps):
        rot_x_min_err, min_err_del_rot_x = self.find_best_value(copy.deepcopy(search_heliostat_spec), facet_to_adjust_idx=facet_to_adjust_idx, var_name='rot_x',    half_range=0.0005, n_steps_one_direction=n_steps_one_direction, variable_steps=variable_steps)
        rot_y_min_err, min_err_del_rot_y = self.find_best_value(copy.deepcopy(search_heliostat_spec), facet_to_adjust_idx=facet_to_adjust_idx, var_name='rot_y',    half_range=0.0005, n_steps_one_direction=n_steps_one_direction, variable_steps=variable_steps)
        # Find most productive error reduction variable.
    ...
    def find_best_value(self, search_heliostat_spec, facet_to_adjust_idx, var_name, half_range, n_steps_one_direction, variable_steps):
        # Common values.
        ...
        zero_change_overall_error, zero_change_n_points_sum = self.find_best_value_aux(search_heliostat_spec, facet_to_adjust_idx, var_name, original_var, zero_change_del_var)
        ...
        for step in variable_steps:
            ...
            overall_error, n_points_sum = self.find_best_value_aux(search_heliostat_spec, facet_to_adjust_idx, var_name, original_var, del_var)
    ...
    def find_best_value_aux(self, search_heliostat_spec, facet_to_adjust_idx, var_name, original_var, del_var):
        ...
        test_error_n_pts_by_facet = self.reprojection_error_single_facet_all_frames_given_heliostat_spec(search_heliostat_spec, facet_to_adjust_idx)
    ...
    def reprojection_error_single_facet_all_frames_given_heliostat_spec(self, heliostat_spec, facet_to_adjust_idx):
        ...
        return self.reprojection_error_single_facet_all_frames_given_corner_xyz_list(heliostat_corner_xyz_list, facet_to_adjust_idx)


    def reprojection_error_single_facet_all_frames_given_corner_xyz_list(self, heliostat_corner_xyz_list, facet_to_adjust_idx):
        # Determine the error for each frame.
        reprojection_error_n_pts_per_frame_list = []
        for frame_id in dt.sorted_keys(self.dict_of_frame_dicts):
            frame_dict = self.dict_of_frame_dicts[frame_id]
            if frame_dict['use_for_metrology']:
                reprojection_error_n_pts = self.reprojection_error_single_facet_one_frame_given_corner_xyz_list(heliostat_corner_xyz_list, facet_to_adjust_idx, frame_id, frame_dict)
    ...
    def reprojection_error_single_facet_one_frame_given_corner_xyz_list(self, current_all_heliostat_xyz_list, facet_to_adjust_idx, frame_id, frame_dict):
        # Fetch data.
    ...
    def projected_points_given_facet_xyz_list(self, frame_dict, facet_xyz_list):
        ...
        proj_pts, jacobian = cv.projectPoints(np.array(facet_xyz_list), camera_rvec, camera_tvec, self.camera_matrix, self.select_distortion_model())  # ?? SCAFFOLDING RCB -- CORRECT?
    ...
    def reprojection_error_all_frames_given_corner_xyz_list(self, heliostat_corner_xyz_list):
        ...
        for frame_id in dt.sorted_keys(self.dict_of_frame_dicts):
            frame_dict = self.dict_of_frame_dicts[frame_id]
            if frame_dict['use_for_metrology']:
                reprojection_error_n_pts_by_facet = self.reprojection_error_one_frame_given_corner_xyz_list(heliostat_corner_xyz_list, frame_id, frame_dict)
    ...
    def reprojection_error_one_frame_given_corner_xyz_list(self, current_all_heliostat_xyz_list, frame_id, frame_dict):
        ...
        for idx in range(0,25):   # ?? SCAFFOLDING RCB -- MAKE GENERAL ACROSS DESIGNS
            ...
            # Update errors.
            n_points, overall_error_sum, facet_error_sum = self.update_corner_error(obs_ul, proj_ul, n_points, overall_error_sum, facet_error_sum)
            n_points, overall_error_sum, facet_error_sum = self.update_corner_error(obs_ur, proj_ur, n_points, overall_error_sum, facet_error_sum)
            n_points, overall_error_sum, facet_error_sum = self.update_corner_error(obs_lr, proj_lr, n_points, overall_error_sum, facet_error_sum)
            n_points, overall_error_sum, facet_error_sum = self.update_corner_error(obs_ll, proj_ll, n_points, overall_error_sum, facet_error_sum)
    ...
    def projected_points_given_corner_xyz_list(self, frame_dict, correspond_heliostat_xyz_list):
        ...
        camera_tvec = frame_dict['single_frame_camera_tvec']
        # Using the camera pose, transform the 3-d points into the camera image space.
        proj_pts, jacobian = cv.projectPoints(np.array(correspond_heliostat_xyz_list), camera_rvec, camera_tvec, self.camera_matrix, self.select_distortion_model())  # ?? SCAFFOLDING RCB -- CORRECT?
        self.calls_to_project_points += 1
        proj_pts_reshaped = proj_pts.reshape(-1, 2)
        # Return.
        return proj_pts_reshaped
    ...
    def compute_camera_pose(self, reference_xyz_list, observed_xy_list):
        ...
        retval, camera_rvec, camera_tvec = cv.solvePnP(np.array(reference_xyz_list), np.array(observed_xy_list), self.camera_matrix, self.select_distortion_model())
    ...
    def construct_frame_camera_pose_dict(self, list_of_frame_id_observed_corner_xy_lists, heliostat_spec):
        ...
        for frame_id_xy_list in list_of_frame_id_observed_corner_xy_lists:
            ...
            observed_corner_xy_list = frame_id_xy_list[1]
            # Find camera position.
            retval, camera_rvec, camera_tvec = cv.solvePnP(np.array(search_xyz_list), np.array(observed_corner_xy_list), self.camera_matrix, self.select_distortion_model())
            self.calls_to_solve_pnp += 1
            # Store.
    ...
    def construct_final_projected_points_dict(self, final_heliostat_spec):
        ...
        final_heliostat_corner_xyz_list = self.specifications.heliostat_corner_xyz_list_given_heliostat_spec(final_heliostat_spec)
    ...
    def save_and_analyze_heliostat_spec(self, hel_name_suffix, heliostat_spec):
        ...
        # Analyze.
        camera_rvec = np.array([-2.68359887, -0.2037837, 0.215282]).reshape(3, 1)     # See 180_Heliostats3d.save_and_analyze_flat_heliostat() for background.
        camera_tvec = np.array([ 5.13947086, -2.22502302, 25.35294025]).reshape(3, 1) #
        uh3a.analyze_and_render_heliostat_3d(self.theoretical_flat_heliostat_dir_body_ext, output_hel_dir_body_ext, None, self.specifications, output_hel_dir,
                                             camera_rvec=camera_rvec, camera_tvec=camera_tvec, camera_matrix=self.camera_matrix, distortion_coefficients=self.zero_distortion_coefficients)
        # Return.
    ...
    def save_and_analyze_design_heliostat(self, design_spec):
        ...
        camera_rvec = np.array([-2.68359887, -0.2037837, 0.215282]).reshape(3, 1)     # See 180_Heliostats3d.save_and_analyze_flat_heliostat() for background.
        camera_tvec = np.array([ 5.13947086, -2.22502302, 25.35294025]).reshape(3, 1) #
        uh3a.analyze_and_render_heliostat_3d(self.theoretical_flat_heliostat_dir_body_ext, design_hel_xyz_dir_body_ext, None, self.specifications, output_design_dir,
                                             camera_rvec=camera_rvec, camera_tvec=camera_tvec, camera_matrix=self.camera_matrix, distortion_coefficients=self.zero_distortion_coefficients)
        # Return.
    ...
    def save_frame_track_image(self, frame_id, observed_corner_xy_list):
        ...
        uh3a.draw_annotated_frame_image(observed_corner_xy_list, self.input_video_body, self.input_frame_dir, self.input_frame_id_format, frame_id, step_hel_name, note, output_frame_tracking_image_dir)


    def save_and_analyze_final_heliostat_spec(self, heliostat_spec, search_log, search_overall_error_history, search_facet_error_history, final_projected_points_dict):
        # Construct (x,y,z) list.
        heliostat_corner_xyz_list = self.specifications.heliostat_corner_xyz_list_given_heliostat_spec(heliostat_spec)
        # Save.
        ...
        output_hel_spec_dir_body_ext        = save_heliostat_spec(heliostat_spec, output_hel_name, output_hel_dir)
        output_frame_dict_dir_body_ext      = save_frame_dict_parameters(output_hel_name, self.dict_of_frame_dicts, output_hel_dir)
        output_log_dir_body_ext             = save_search_log(search_log, output_hel_name, output_hel_dir)
        output_overall_history_dir_body_ext = save_search_overall_error_history(search_overall_error_history, output_hel_name, output_hel_dir)
        output_facet_history_dir_body_ext   = save_search_facet_error_history(search_facet_error_history, output_hel_name, output_hel_dir)
        output_complexity_dir_body_ext      = save_computation_complexity(self.calls_to_solve_pnp, self.calls_to_project_points, output_hel_name, output_hel_dir)
        output_proj_pts_dir_body_ext        = save_projected_points_dict(final_projected_points_dict, output_hel_name, output_hel_dir)
        # Analyze.
        camera_rvec = np.array([-2.68359887, -0.2037837, 0.215282]).reshape(3, 1)     # See 180_Heliostats3d.save_and_analyze_flat_heliostat() for background.
        camera_tvec = np.array([ 5.13947086, -2.22502302, 25.35294025]).reshape(3, 1) #
        uh3a.analyze_and_render_heliostat_3d(self.theoretical_flat_heliostat_dir_body_ext, output_hel_xyz_dir_body_ext, None, self.specifications, output_hel_dir,
                                             camera_rvec=camera_rvec, camera_tvec=camera_tvec, camera_matrix=self.camera_matrix, distortion_coefficients=self.zero_distortion_coefficients)
        # Return.
        return output_hel_xyz_dir_body_ext,          \
    ...
    def print_facets_canting_angles(self, heliostat_corner_xyz_list):  # ?? SCAFFOLDING RCB -- MAKE GENERAL ACROSS HELIOSTAT DESIGNS
    ...
    # ?? SCAFFOLDING RCB -- MOVE SOMEWHERE ELSE.
    def print_facets_canting_angles_aux(self, idx, facet_corner_xyz_list):  # ?? SCAFFOLDING RCB -- MAKE GENERAL ACROSS HELIOSTAT DESIGNS
        ...
        # Diagonal vectors.
        ll_to_ur = np.array([ (facet_ur_xyz[0]-facet_ll_xyz[0]), (facet_ur_xyz[1]-facet_ll_xyz[1]), (facet_ur_xyz[2]-facet_ll_xyz[2]) ])
        lr_to_ul = np.array([ (facet_ul_xyz[0]-facet_lr_xyz[0]), (facet_ul_xyz[1]-facet_lr_xyz[1]), (facet_ul_xyz[2]-facet_lr_xyz[2]) ])
        # Unit diagonals.
        ...
        print('In print_facets_canting_angles_aux(), idx = ', idx, '  cross_product:', cross_product, ' canting_angle_in_x_direction =', canting_angle_in_x_direction, '  canting_angle_in_y_direction =', canting_angle_in_y_direction)


    # ?? SCAFFOLDING RCB -- MOVE SOMEWHERE ELSE.
    def print_facet_canting_angles(self, facet_corner_xyz_list):  # ?? SCAFFOLDING RCB -- MAKE GENERAL ACROSS HELIOSTAT DESIGNS
        facet_ul_xyz = facet_corner_xyz_list[0]
        facet_ur_xyz = facet_corner_xyz_list[1]
        facet_lr_xyz = facet_corner_xyz_list[2]
        facet_ll_xyz = facet_corner_xyz_list[3]
        ...
        canting_angle_in_x_direction = math.atan2(n_x, n_z)
        canting_angle_in_y_direction = math.atan2(n_y, n_z)
        print('In print_facet_canting_angles(), facet ul, ur, lr, ll:', facet_ul_xyz, facet_ur_xyz, facet_lr_xyz, facet_ll_xyz)
    ...
    def step_hel_name(self,
                      frame_id,
                      step_suffix):  # String to add to heliostat name to denote this step result.  Cannot include "_" or "-" characters.
...
def save_heliostat_spec(heliostat_spec, hel_name, output_dir):  # ?? SCAFFOLDING RCB -- HELIOSTAT_SPEC SHOULD BE A CLASS, AND THIS SHOULD BE A CLASS MEMBER FUNCTION
    # Prepare output.
    ...
    for facet_id in dt.sorted_keys(heliostat_spec):
        ...
        data_lines.append('{0:d},{1:.6f},{2:.6f},{3:.6f},{4:.8f},{5:.8f},{6:.8f}'.format(facet_id, center_x, center_y, center_z, rot_x, rot_y, rot_z))
    # Write to disk.
    ...
    output_dir_body_ext = ft.write_csv_file(explain,                       # Explanatory string to include in notification output.  None to skip.
                                            ...
                                            error_if_dir_not_exist=False)  # If the directory does not exist, create it.
    # Return.
...
def save_frame_dict_parameters(hel_name, dict_of_frame_dicts, output_dir):  # ?? SCAFFOLDING RCB -- FRAME_CAMERA_POSE_DICT SHOULD BE A CLASS, AND THIS SHOULD BE A CLASS MEMBER FUNCTION
    # Prepare output.
    ...
    for frame_id in dt.sorted_keys(dict_of_frame_dicts):
        ...
        t_y = camera_tvec[1][0]
        t_z = camera_tvec[2][0]
        data_lines.append('{0:d},{1:.8f},{2:.8f},{3:.8f},{4:.8f},{5:.8f},{6:.8f},{7:d},{8:d}'.format(frame_id, r_x, r_y, r_z, t_x, t_y, t_z, n_missing, use_for_metrology))
    # Write to disk.
...
def save_search_log(search_error_log, hel_name, output_dir):
    ...
    explain = output_file_body  # I don't feel like something fancier.
    output_dir_body_ext = ft.write_text_file(explain,                       # Explanatory string to include in notification output.  None to skip.
                                             output_dir,                    # Directory to write file.  See below if not exist.
                                             output_file_body,              # Body of output filename; extension is ".csv"
...
def save_search_facet_error_history(search_facet_error_history, hel_name, output_dir):
    ...
    for fb in search_facet_error_history:
        ...
        data_lines.append('{0:d},{1:d},{2:s},{3:.10f},{4:.6f}'.format(iteration,facet_idx, best_var_name, best_min_error, best_min_error_del_var))
    # Write to disk.
...
def save_projected_points_dict(projected_pts_dict, hel_name, output_dir):
    ...
    explain = output_file_body  # I don't feel like something fancier.
    print('In save_projected_points_dict(), saving final reprojected points to file:', os.path.join(output_dir, output_file_body))
    output_dir_body_ext = ft.write_csv_file(explain,                       # Explanatory string to include in notification output.  None to skip.
                                            output_dir,                    # Directory to write file.  See below if not exist.
                                            output_file_body,              # Body of output filename; extension is ".csv"
                                            None,                          # First line to write to file. None to skip.
