...
#         self.render_control_confirmed_undistorted          = render_control_confirmed_undistorted
        ...
        self.solar_field = sf.SolarField(name='Sandia NSTTF',
                                         short_name='NSTTF',
                                         ...
                                         facet_centroids_file=self.specifications.facets_centroids_file)  # ?? SCAFFOLDING RCB -- RENAME "_file" TO "_dir_body_ext" THROUGHOUT CODE.
        ...
        self.gps_ymdhmsz_given_flight_log_zero_seconds_dir_body_ext = self.save_gps_ymdhmsz_given_flight_log_zero_seconds()
        ...
        self.output_gps_velocity_xy_change_minima_dir_body_ext = self.save_gps_velocity_xy_change_points(self.gps_velocity_xy_change_minima, 'minima')
        self.output_gps_velocity_xy_change_maxima_dir_body_ext = self.save_gps_velocity_xy_change_points(self.gps_velocity_xy_change_maxima, 'maxima')
        ...
        self.maximum_to_minimum_pass_pair_list_dir_body_ext = self.save_gps_velocity_xy_change_pairs(self.maximum_to_minimum_pass_pair_list, 'maximum_to_minimum')
        self.minimum_to_maximum_pass_pair_list_dir_body_ext = self.save_gps_velocity_xy_change_pairs(self.minimum_to_maximum_pass_pair_list, 'minimum_to_maximum')
        ...
        self.synchronization_slope, \
            self.synchronization_intercept = self.initialize_synchronization_constants()
    ...
    def add_velocity_columns_to_flight_log_df(self):
        ...
        for idx in self.flight_log_df.index:  # ?? SCAFFOLDING RCB -- THIS FOR LOOP IS VERY SLOW.  CAN WE VECTORIZE IT?  REPLACE THE .loc[] CALLS WITH SOMETHING FASTER?
            ...
            z = altitude
            t_sec = t_msec / 1000.0
            self.flight_log_xyz_list.append([x, y, z])  # ?? SCAFFOLDING RCB -- ELIMINATE THIS, IN FAVOR OF THE DATAFRAME.
        ...
        print('In TrajectoryAnalysis.add_velocity_columns_to_flight_log_df(), adding average velociy, velocity change, angle, and angle change columns...')
        empty_column = [np.nan] * len(self.flight_log_df.index)
    ...
    def gps_ymdhmsz_given_flight_log_seconds(self, time_sec):  # Time is in seconds, corresponding to flight log 'time(sec)' column.
        """
    ...
    def compute_gps_ymdhmsz_given_flight_log_zero_seconds(self):
        ...
        flight_log_first_row_gps_date_time_str = self.flight_log_df.loc[0, 'datetime(local)']  # Example: '2020-12-03 15:44:13.802'
    ...
    def flight_log_xyz_given_time(self, input_time):
        for idx in self.flight_log_df.index:
            ...
            if time == input_time:
                ...
                if idx == 0:
                    print('ERROR: In TrajectoryAnalysis.flight_log_xyz_given_time(), input time is before flight log start time.  Input time =', str(input_time), '; Flight log start time =', str(time))
                    assert False
        ...
        print('ERROR: In TrajectoryAnalysis.flight_log_xyz_given_time(), input time is after flight log end time.  Input time =', str(input_time), '; Flight log end time =', str(time))
        assert False
    ...
    def find_gps_velocity_xy_change_maxima(self, sign):  # sign is +1 or -1.
        ...
        for idx in self.flight_log_df.index:
            if (idx > 4) and (idx < (len(self.flight_log_df.index) - 5)):
                ...
                if (prev5_delta_velocity_angle_xy <= this_delta_velocity_angle_xy) and \
                   (prev4_delta_velocity_angle_xy <= this_delta_velocity_angle_xy) and \
                   (prev3_delta_velocity_angle_xy <= this_delta_velocity_angle_xy) and \
                   (prev2_delta_velocity_angle_xy <= this_delta_velocity_angle_xy) and \
                   (prev1_delta_velocity_angle_xy <= this_delta_velocity_angle_xy) and \
                   (this_delta_velocity_angle_xy >= self.delta_velocity_angle_xy_peak_threshold) and \
                   (next1_delta_velocity_angle_xy <= this_delta_velocity_angle_xy) and \
                   (next2_delta_velocity_angle_xy <= this_delta_velocity_angle_xy) and \
                   (next3_delta_velocity_angle_xy <= this_delta_velocity_angle_xy) and \
                   (next4_delta_velocity_angle_xy <= this_delta_velocity_angle_xy) and \
                   (next5_delta_velocity_angle_xy <= this_delta_velocity_angle_xy):
    ...
    def find_velocity_xy_change_maximum_to_minimum_scan_passes(self):  # ?? SCAFFOLDING RCB -- THIS ROUTINE DUPLICATES ITS PAIR.  CAN THEY BE MERGED, WHILE RETAINING CORRECTNESS AND CLARITY?
        ...
        for maximum_dict in self.gps_velocity_xy_change_maxima:  # Here "maximum" means "maximum_velocity_xy_change"
            ...
            time_maximum = maximum_dict['time']  # Here "time_maximum" means "time of the maximum_velocity_xy_change," not the minimum time.
                ...
                if interval_delta_time >= self.minimum_scan_pass_time:
                    ...
                    for idx in self.flight_log_df.index:
                        ...
                        if (interval_start_time <= time) and (time <= interval_end_time):
                            ...
                            speed_count += 1
                    if speed_count <= 0:
                        print('In TrajectoryAnalysis.__init__(), encountered unexpected non-positive speed_count =', speed_count)
                        assert False
...
# print('In TrajectoryAnalysis.__init__(), [min_scan_speed, max_scan_speed] = [',min_scan_speed, ',',max_scan_speed,']; interval_average_speed = ', interval_average_speed)  # ?? SCAFFOLDING RCB -- TEMPORARY
                    if (min_scan_speed <= interval_average_speed) and (interval_average_speed <= max_scan_speed):
                        ...
                        if velocity_z_count <= 0:
                            print('In TrajectoryAnalysis.__init__(), encountered unexpected non-positive velocity_z_count =', velocity_z_count)
                            assert False
 ...
 # print('In TrajectoryAnalysis.__init__(), [min_scan_velocity_z, max_scan_velocity_z] = [',min_scan_velocity_z, ',',max_scan_velocity_z,']; interval_average_velocity_z = ', interval_average_velocity_z)  # ?? SCAFFOLDING RCB -- TEMPORARY
                        if (min_scan_velocity_z <= interval_average_velocity_z) and (interval_average_velocity_z <= max_scan_velocity_z):
    ...
    def find_velocity_xy_change_minimum_to_maximum_scan_passes(self):  # ?? SCAFFOLDING RCB -- THIS ROUTINE DUPLICATES ITS PAIR.  CAN THEY BE MERGED, WHILE RETAINING CORRECTNESS AND CLARITY?
        ...
        for minimum_dict in self.gps_velocity_xy_change_minima:  # Here "minimum" means "minimum_velocity_xy_change"
            ...
            time_minimum = minimum_dict['time']  # Here "time_minimum" means "time of the minimum_velocity_xy_change," not the maximum time.
...
# print('In TrajectoryAnalysis.__init__(), [min_scan_velocity_z, max_scan_velocity_z] = [',min_scan_velocity_z, ',',max_scan_velocity_z,']; interval_average_velocity_z = ', interval_average_velocity_z)  # ?? SCAFFOLDING RCB -- TEMPORARY
    ...
    def convert_gps_minima_maxima_passes_to_scan_passes(self):
        ...
        for start_stop_pass_pair in start_stop_pass_pair_list:
            ...
            list_of_xyzt_lists_1 = self.split_xyzt_list_where_inter_point_speed_below_minimum(xyzt_list, self.minimum_gps_pass_inter_point_speed)
            # Discard lists that are too short.
            list_of_xyzt_lists_2 = self.discard_xyzt_lists_where_length_below_minimum(list_of_xyzt_lists_1, self.minimum_gps_pass_number_of_points)
            # Construct scan pass objects.
            for xyzt_list in list_of_xyzt_lists_2:
                # Construct the scan pass object.
                if len(xyzt_list) >= 2:
                    gps_scan_pass = self.scan_pass_given_xyzt_list(xyzt_list, n_start_margin=self.gps_pass_start_margin, n_stop_margin=self.gps_pass_stop_margin)
                    # Add to list.
    ...
    def scan_pass_given_xyzt_list(self, xyzt_list, n_start_margin, n_stop_margin):  # ?? SCAFFOLDING RCB -- THIS START_MARGIN, STOP_MARGIN IS A STUB HACK.
        ...
        stable_begin_xyzt,     \
            ...
            rms_distance_to_line = self.refine_xyzt_list(xyzt_list, n_start_margin, n_stop_margin)
        # Construct scan pass object.
        scan_pass = {}  # ?? SCAFFOLDING RCB -- THIS SHOULD BE A CLASS, SUITABLE FOR BOTH GPS SCAN PASSES AND TRAJECTORY FRAGMENTS.
    ...
    def refine_xyzt_list(self, xyzt_list, n_start_margin, n_stop_margin=20):
        if (len(xyzt_list) < 2):
            print('ERROR: In TrajectoryAnalysis.refine_xyzt_list(), encountered insufficient xyzt points.  len(xyzt_list) =', len(xyzt_list))
            assert False
        if (len(xyzt_list) <= (n_start_margin + n_stop_margin)):  # ?? SCAFFOLDING RCB -- STUB HACK.  REPLACE THIS WITH A CALCULATION THAT REMOVED POINTS BASED ON WHETHER THEY ARE INLIERS OR OUTLIERS.
            print('WARNING: In TrajectoryAnalysis.refine_xyzt_list(), encountered insufficient xyzt points; skipping margins.  len(xyzt_list) =', len(xyzt_list))
            inlier_xyzt_list = xyzt_list
        else:
            inlier_xyzt_list = xyzt_list[n_start_margin: len(xyzt_list) - n_stop_margin]
        stable_begin_xyzt = inlier_xyzt_list[0]  # ?? SCAFFOLDING RCB -- STUB HACK.  THIS SHOULD USE A FIT LINE, NOT FIRST AND LAST POINTS.
        stable_end_xyzt = inlier_xyzt_list[-1]  # ?? SCAFFOLDING RCB -- STUB HACK.  THIS SHOULD USE A FIT LINE, NOT FIRST AND LAST POINTS.
        line_3d_xyz_1 = stable_begin_xyzt[0:3]
        line_3d_xyz_2 = stable_end_xyzt[0:3]
        line_3d = g3d.construct_line_3d_given_two_points(line_3d_xyz_1, line_3d_xyz_2)  # ?? SCAFFOLDING RCB -- LINE-3D SHOULD BE A CLASS.
        ...
        for hel_name in self.solar_field.heliostat_name_list():
            if ft.directory_exists(self.input_reconstructed_heliostats_dir):
                ...
                if ft.directory_exists(reconstructed_hel_dir):
                    frame_parameters_dir_body_ext = os.path.join(reconstructed_hel_dir, hel_name + '_frame_dict_parameters.csv')
                    if not ft.file_exists(frame_parameters_dir_body_ext):
                        print('ERROR: In TrajectoryAnalysis.load_trajectory_fragments(), expected file does not exist: ' + frame_parameters_dir_body_ext)
                        assert False
                    else:
    ...
    def add_camera_transforms(self, hel_frames_dict):
        for hel_name in hel_frames_dict.keys():
            ...
            for frame_id in frames_dict.keys():
                ...
                transform[0, 0] = rvec_rotation_matrix[0, 0]  # ?? SCAFFOLDING RCB -- MAYBE THERE'S A CLEANER/FASTER WAY TO DO THIS.
    ...
    def initialize_synchronization_pair_list(self):
        ...
        for synch_gps_pass_idx, synch_camera_hel_name in zip(synch_gps_pass_idx_list, synch_camera_hel_name_list):
            ...
            # Pair.
            synch_pair = [synch_gps_spec, synch_camera_spec]  # ?? SCAFFOLDING RCB -- THIS SHOULD BE A CLASS, PERHAPS WITH TWO SUBCLASSES.
    ...
    def find_gps_synch_time(self, synch_gps_pass_type, synch_gps_pass_idx, synch_gps_start_or_stop):
        if synch_gps_pass_type == 'min_to_max':
            ...
            print('ERROR: In TrajectoryAnalysis.find_gps_synch_time(), encountered unexpected synch_gps_pass_type =', str(synch_gps_pass_type))
            assert False
        ...
        if synch_gps_start_or_stop == 'start':
            ...
            print('ERROR: In TrajectoryAnalysis.find_gps_synch_time(), encountered unexpected synch_gps_start_or_stop =', str(synch_gps_start_or_stop))
            assert False
    ...
    def initialize_synchronization_constants(self):
        ...
        slope_01, intercept_01 = self.compute_synchronization_slope_and_intercept(self.synchronization_pair_list[0], self.synchronization_pair_list[1])
        slope_02, intercept_02 = self.compute_synchronization_slope_and_intercept(self.synchronization_pair_list[0], self.synchronization_pair_list[2])
        slope_12, intercept_12 = self.compute_synchronization_slope_and_intercept(self.synchronization_pair_list[1], self.synchronization_pair_list[2])
    ...
    def compute_synchronization_slope_and_intercept(self, synch_pair_1, synch_pair_2):
        ...
        if frame_id_2 == frame_id_1:
            print('ERROR: In TrajectoryAnalysis.compute_synchronization_slope_and_intercept(), identical frame_id_1 == frame_id_2 ==', frame_id_1)
            assert False
    ...
    def construct_heliostat_camera_passes(self):
        ...
        for hel_name in self.hel_frames_dict.keys():
            ...
            # Discard lists that are too short.
            list_of_camera_xyzt_lists_2 = self.discard_xyzt_lists_where_length_below_minimum(list_of_camera_xyzt_lists_1, self.minimum_camera_pass_number_of_points)
            ...
            for camera_xyzt_list in list_of_camera_xyzt_lists_2:
                # If there is a pause, break the list up.
                list_of_camera_xyzt_lists_A = self.split_xyzt_list_where_inter_point_speed_below_minimum(camera_xyzt_list, self.minimum_camera_pass_inter_point_speed)
                # Discard lists that are too short.
                list_of_camera_xyzt_lists_B = self.discard_xyzt_lists_where_length_below_minimum(list_of_camera_xyzt_lists_A, self.minimum_camera_pass_number_of_points)
            ...
            for camera_xyzt_list in list_of_camera_xyzt_lists_3:
                if len(camera_xyzt_list) >= self.minimum_camera_pass_number_of_points:
                    camera_pass = self.scan_pass_given_xyzt_list(camera_xyzt_list, n_start_margin=self.camera_pass_start_margin, n_stop_margin=self.camera_pass_stop_margin)
                    camera_pass_list.append(camera_pass)
            # Add to result.
            hel_camera_passes_dict[hel_name] = camera_pass_list
        return hel_camera_passes_dict

    def split_xyz_list_where_distance_exceeds_maximum(self, xyz_list, maximum_inter_point_distance):  # Split if inter-point distance is above this threshold.
        if len(xyz_list) == 0:
            return []
        elif len(xyz_list) == 1:
            return [xyz_list]
            ...
            d = np.sqrt((first_xyz[0] - first_remainder_xyz[0])**2 + (first_xyz[1] - first_remainder_xyz[1])**2 + (first_xyz[2] - first_remainder_xyz[2])**2)
    ...
    def split_xyzt_list_where_distance_exceeds_maximum(self, xyzt_list, maximum_inter_point_distance):  # Split if inter-point distance is above this threshold.
        if len(xyzt_list) == 0:
            return []
            ...
            d = np.sqrt((first_xyzt[0] - first_remainder_xyzt[0])**2 + (first_xyzt[1] - first_remainder_xyzt[1])**2 + (first_xyzt[2] - first_remainder_xyzt[2])**2)
            if d > maximum_inter_point_distance:
    ...
    def split_xyzt_list_where_inter_point_speed_below_minimum(self, xyzt_list, minimum_inter_point_speed):
        if len(xyzt_list) == 0:
            ...
            first_xyzt = xyzt_list[0]
            remainder = self.split_xyzt_list_where_inter_point_speed_below_minimum(xyzt_list[1:], minimum_inter_point_speed)  # Split if inter-point distance is above this threshold.
            first_remainder_xyzt_list = remainder[0]
            first_remainder_xyzt = first_remainder_xyzt_list[0]
            distance = np.sqrt((first_xyzt[0] - first_remainder_xyzt[0])**2 + (first_xyzt[1] - first_remainder_xyzt[1])**2 + (first_xyzt[2] - first_remainder_xyzt[2])**2)
            time_gap = first_remainder_xyzt[3] - first_xyzt[3]
            if time_gap == 0:
                print('ERROR: In TrajectoryAnalysis.split_xyzt_list_where_inter_point_speed_below_minimum(), unexpected zero time gap encountered.')
                assert False
            if time_gap < 0:
                print('ERROR: In TrajectoryAnalysis.split_xyzt_list_where_inter_point_speed_below_minimum(), encountered unexpected negative time gap =', time_gap)
                assert False
        ...
        for hel_name in dt.sorted_keys(self.hel_camera_passes_dict):
            ...
            for camera_pass in camera_pass_list:
                ...
                if matching_gps_pass == None:
                    print('WARNING: In TrajectoryAanlsysi.construct_hel_gps_camera_analysis_dict(), no matching GPS scan pass found for heliostat ' + str(hel_name) + ' camera_pass_idx =', camera_pass_idx)
                    pass
                else:
                    # Data structure for capturing GPS-camera pass analysis results.
                    gps_camera_analysis_dict = self.construct_gps_camera_analysis_dict(hel_name, matching_gps_pass, camera_pass)
                    list_of_gps_camera_analysis_dicts.append(gps_camera_analysis_dict)
                camera_pass_idx += 1
            if len(list_of_gps_camera_analysis_dicts) == 0:
                print('WARNING: In TrajectoryAnalysis.construct_hel_gps_camera_analysis_dict(), no matching GPS scans for heliostat ' + str(hel_name))
                pass
            else:
    ...
    def find_matching_gps_pass(self, camera_pass):
        ...
        for gps_pass in self.gps_scan_passes:
            gps_pass_begin_time = gps_pass['stable_begin_xyzt'][3]  # ?? SCAFFOLDING RCB -- HAVE THIS FETCH ORIGINAL FULL-LENGTH START AND STOP TIME INTERVAL.
            gps_pass_end_time = gps_pass['stable_end_xyzt'][3]  # ?? SCAFFOLDING RCB -- HAVE THIS FETCH ORIGINAL FULL-LENGTH START AND STOP TIME INTERVAL.
            # Check for overlap.
            if (gps_pass_begin_time <= camera_pass_end_time) and (camera_pass_begin_time <= gps_pass_end_time):
                ...
                if overlap_len > 0:
                    ...
                    if (matching_gps_pass == None):
                        ...
                        print('WARNING: In TrajectoryAnalysis.find_matching_gps_pass(), multiple matching GPS scan passes found, with overlaps',
                              matching_gps_match_overlap_len, ' and ', overlap_len, '; keeping the one with greater overlap.')
    ...
    def construct_gps_camera_analysis_dict(self, hel_name, gps_pass, camera_pass):  # ?? SCAFFOLDING RCB -- THIS SHOULD BE A CLASS.
        ...
        azimuth_from_model_begin,   \
            ...
            elevation_from_model_end = self.compute_model_azimuth_elevation(hel_name, time_begin, time_mid, time_end)
        ...
        azimuth_from_alignment,   \
            ...
            alignment_angle_error = self.compute_alignment_azimuth_elevation(gps_pass, camera_pass, azimuth_from_model_mid, elevation_from_model_mid)
        ...
        per_heliostat_transformed_camera_pass = self.scan_pass_given_xyzt_list(transformed_camera_xyzt_list, n_start_margin=0, n_stop_margin=0)  # We alrady shrunk, so don't shrink again.
        ...
        if camera_gps_distance_squared_count == 0:
            print('ERROR: In TrajectoryAnalysis.construct_gps_camera_analysis_dict(), encountered empty camera pass inlier xyzt list.')
            assert False
    ...
    def compute_model_azimuth_elevation(self, hel_name, time_begin, time_mid, time_end):
        azimuth_begin, \
            elevation_begin = self.compute_model_azimuth_elevation_given_time(hel_name, time_begin)
        azimuth_mid, \
            elevation_mid = self.compute_model_azimuth_elevation_given_time(hel_name, time_mid)
        azimuth_end, \
            elevation_end = self.compute_model_azimuth_elevation_given_time(hel_name, time_end)
        # Return.
        return azimuth_begin,   \
            elevation_begin, \
            azimuth_mid,     \
            elevation_mid,   \
            azimuth_end,     \
    ...
    def compute_model_azimuth_elevation_given_time(self, hel_name, time):
        ...
        h_config = heliostat.compute_tracking_configuration(self.aimpoint_xyz, self.solar_field.origin_lon_lat, when_ymdhmsz)
        # Return.
        return h_config.az, h_config.el

    def compute_alignment_azimuth_elevation(self, gps_pass, camera_pass, azimuth_from_model, elevation_from_model):  # ?? SCAFFOLDING RCB -- THIS SHOULD BE A CLASS.
    ...
    def check_az_el(self, az, el, gps_uxyz, camera_uxyz):
        ...
        delta_uxyz = np.sqrt((gps_uxyz[0] - rotated_camera_uxyz[0])**2 + (gps_uxyz[1] - rotated_camera_uxyz[1])**2 + (gps_uxyz[2] - rotated_camera_uxyz[2])**2)
        return delta_uxyz
    ...
    def compute_log_azimuth_elevation(self, hel_name, time_begin, time_mid, time_end):
        azimuth_begin,          \
            ...
            elevation_target_begin = self.compute_log_azimuth_elevation_given_time(hel_name, time_begin)
        azimuth_mid,          \
            elevation_mid,        \
            azimuth_target_mid,   \
            elevation_target_mid = self.compute_log_azimuth_elevation_given_time(hel_name, time_mid)
        azimuth_end,          \
            elevation_end,        \
            azimuth_target_end,   \
            elevation_target_end = self.compute_log_azimuth_elevation_given_time(hel_name, time_end)
        # Return.
        return azimuth_begin,          \
            elevation_begin,        \
            azimuth_mid,            \
    ...
    def set_per_heliostat_estimates_of_camera_xyz_given_overall_time(self):
        ...
        for hel_name in self.hel_frames_dict.keys():
            ...
            for frame_id in frames_dict.keys():
                ...
                camera_xyz_world = list(heliostat.transform_xyz(camera_xyz_heliostat))  # transform_xyz() returns a numpy array.
                frame_parameters_dict['per_heliostat_estimate_of_camera_xyz_in_world_coords_overall_time'] = camera_xyz_world
        # Camera passes.
        ...
        for hel_name in self.hel_camera_passes_dict.keys():
            ...
            for camera_pass in camera_pass_list:
                ...
                transformed_camera_pass = self.scan_pass_given_xyzt_list(transformed_camera_xyzt_list, n_start_margin=0, n_stop_margin=0)
    ...
    def print_gps_flight_log_zero_seconds(self):
        print('In TrajectoryAnalysis.print_gps_flight_log_zero_seconds(), zero_time_ymdhmsz =', self.gps_ymdhmsz_given_flight_log_zero_seconds)
    ...
    def print_gps_scan_pass_summary(self):
        ...
        for gps_scan_pass in self.gps_scan_passes:
            print('In TrajectoryAnalysis.print_gps_scan_pass_summary(), gps_scan_passes ' + str(gps_scan_pass_idx) + '  length =',
                  gps_scan_pass['line_3d']['length'], '  rms_distance_to_line =', gps_scan_pass['rms_distance_to_line'])
        ...
        print('In TrajectoryAnalysis.print_gps_scan_pass_summary(), gps_scan_pass overall rms_distance_to_line =', overall_rms)
    ...
    def print_hel_frames_dict_aux(self, hel_frames_dict):
        dt.print_dict_of_dict_of_dicts(hel_frames_dict,       # Dictionary to print.
                                       ...
                                       indent_3=4)             # Number of additional blanks to print for each third-level line.

    def print_synchronization_pair_list(self):
        print('\nIn TrajectoryAnalysis.print_synchronization_pair_list(), self.synchronization_pair_list:', self.synchronization_pair_list)
        print('In TrajectoryAnalysis.print_synchronization_pair_list(), self.synchronization_slope =', self.synchronization_slope)
        print('In TrajectoryAnalysis.print_synchronization_pair_list(), self.synchronization_intercept =', self.synchronization_intercept)
    ...
    def print_hel_camera_passes_dict_aux(self, hel_camera_passes_dict, max_heliostats):
        ...
        for hel_name in dt.sorted_keys(hel_camera_passes_dict):
            ...
            for camera_pass in camera_pass_list:
                print('    ' + str(camera_pass['stable_begin_xyzt']) + '-->' + str(camera_pass['stable_end_xyzt']) + '; rms_to_line = ' + str(camera_pass['rms_distance_to_line']))
            hel_count += 1
    ...
    def print_hel_gps_camera_analysis_dict_aux(self, hel_gps_camera_analysis_dict, max_heliostats):
        ...
        for hel_name in dt.sorted_keys(hel_gps_camera_analysis_dict):
            ...
            for gps_camera_analysis_dict in list_of_gps_camera_analysis_dicts:
                transformed_camera_begin_xyzt = gps_camera_analysis_dict['per_heliostat_transformed_camera_pass']['stable_begin_xyzt']
                transformed_camera_end_xyzt = gps_camera_analysis_dict['per_heliostat_transformed_camera_pass']['stable_end_xyzt']
                ...
                print('    Transformed camera pass:', transformed_camera_begin_xyzt, ' --> ', transformed_camera_end_xyzt)
    ...
    def draw_and_save_solar_field_suite(self):
        ...
        self.heliostat_up_style = rch.normal_outline(color='lightblue')  # Use normal_outline() to include surface normal needles, or outline() for just outlines.
        self.heliostat_down_style = rch.normal_outline(color='salmon')     # Use normal_outline() to include surface normal needles, or outline() for just outlines.
        ...
        self.trajectory_fragment_color_wheel = ['coral', 'lawngreen', 'cyan', 'violet', 'gold']  # Lighter version of each camera pass color.
        ...
        self.draw_and_save_solar_field_trajectories('AA. ' + self.solar_field.short_name + ' Solar Field, In Stow',
                                                    draw_control_dict,
        ...
        self.draw_and_save_solar_field_trajectories('AB. ' + self.solar_field.short_name + ' Solar Field, Face Up',
                                                    draw_control_dict,
                                                    limits_3d_list=limits_3d_list,
                                                    limits_xy_list=limits_xy_list,
                                                    limits_xz_list=limits_xz_list,
        ...
        self.solar_field.set_full_field_tracking(aimpoint_xyz=self.aimpoint_xyz, when_ymdhmsz=self.when_ymdhmsz_minus_1_hour)
        ...
        date_str = '{m}/{d}/{y}'.format(m=self.when_ymdhmsz_minus_1_hour[1], d=self.when_ymdhmsz_minus_1_hour[2], y=self.when_ymdhmsz_minus_1_hour[0])
        time_str = '{h:d}:{m:02d}:{s:02d}'.format(h=self.when_ymdhmsz_minus_1_hour[3], m=self.when_ymdhmsz_minus_1_hour[4], s=self.when_ymdhmsz_minus_1_hour[5])
        self.draw_and_save_solar_field_trajectories('AC. ' + self.solar_field.short_name + ' Solar Field, Tracking to ' + aimpoint_str + ' on ' + date_str + ' at ' + time_str,
                                                    draw_control_dict,
                                                    limits_3d_list=limits_3d_list,
                                                    limits_xy_list=limits_xy_list,
                                                    limits_xz_list=limits_xz_list,
        ...
        self.solar_field.set_full_field_tracking(aimpoint_xyz=self.aimpoint_xyz, when_ymdhmsz=self.when_ymdhmsz_minus_30_minutes)
        self.solar_field_style = rcsf.heliostat_normals_outlines(color='k')
        aimpoint_str = '({x},{y},{z})'.format(x=self.aimpoint_xyz[0], y=self.aimpoint_xyz[1], z=self.aimpoint_xyz[2])
        date_str = '{m}/{d}/{y}'.format(m=self.when_ymdhmsz_minus_30_minutes[1], d=self.when_ymdhmsz_minus_30_minutes[2], y=self.when_ymdhmsz_minus_30_minutes[0])
        time_str = '{h:d}:{m:02d}:{s:02d}'.format(h=self.when_ymdhmsz_minus_30_minutes[3], m=self.when_ymdhmsz_minus_30_minutes[4], s=self.when_ymdhmsz_minus_30_minutes[5])
        self.draw_and_save_solar_field_trajectories('AD. ' + self.solar_field.short_name + ' Solar Field, Tracking to ' + aimpoint_str + ' on ' + date_str + ' at ' + time_str,
                                                    draw_control_dict,
                                                    limits_3d_list=limits_3d_list,
                                                    limits_xy_list=limits_xy_list,
                                                    limits_xz_list=limits_xz_list,
        ...
        time_str = '{h:d}:{m:02d}:{s:02d}'.format(h=self.when_ymdhmsz[3], m=self.when_ymdhmsz[4], s=self.when_ymdhmsz[5])
        self.draw_and_save_solar_field_trajectories('AE. ' + self.solar_field.short_name + ' Solar Field, Tracking to ' + aimpoint_str + ' on ' + date_str + ' at ' + time_str,
                                                    draw_control_dict,
                                                    limits_3d_list=limits_3d_list,
                                                    limits_xy_list=limits_xy_list,
                                                    limits_xz_list=limits_xz_list,
        ...
        date_str = '{m}/{d}/{y}'.format(m=self.when_ymdhmsz[1], d=self.when_ymdhmsz[2], y=self.when_ymdhmsz[0])
        time_str = '{h:d}:{m:02d}:{s:02d}'.format(h=self.when_ymdhmsz[3], m=self.when_ymdhmsz[4], s=self.when_ymdhmsz[5])
        self.draw_and_save_solar_field_trajectories('AF. Actual ' + self.solar_field.short_name + ' Field Configuration on ' + date_str + ' at ' + time_str,
                                                    draw_control_dict,
                                                    limits_3d_list=limits_3d_list,
                                                    limits_xy_list=limits_xy_list,
                                                    limits_xz_list=limits_xz_list,
        ...
        self.heliostat_up_style = rch.outline(color='lightblue')  # Use normal_outline() to include surface normal needles, or outline() for just outlines.
        self.heliostat_down_style = rch.outline(color='salmon')     # Use normal_outline() to include surface normal needles, or outline() for just outlines.
        self.solar_field_style.heliostat_styles.add_special_names(up_heliostats, self.heliostat_up_style)
        self.solar_field_style.heliostat_styles.add_special_names(down_heliostats, self.heliostat_down_style)
        aimpoint_str = '({x},{y},{z})'.format(x=self.aimpoint_xyz[0], y=self.aimpoint_xyz[1], z=self.aimpoint_xyz[2])
        date_str = '{m}/{d}/{y}'.format(m=self.when_ymdhmsz[1], d=self.when_ymdhmsz[2], y=self.when_ymdhmsz[0])
        time_str = '{h:d}:{m:02d}:{s:02d}'.format(h=self.when_ymdhmsz[3], m=self.when_ymdhmsz[4], s=self.when_ymdhmsz[5])
        self.draw_and_save_solar_field_trajectories('AG. Actual ' + self.solar_field.short_name + ' Field Configuration on ' + date_str + ' at ' + time_str,
                                                    draw_control_dict,
                                                    limits_3d_list=limits_3d_list,
                                                    limits_xy_list=limits_xy_list,
                                                    limits_xz_list=limits_xz_list,
        ...
        self.draw_and_save_solar_field_trajectories('AH. GPS Trajectory Over Solar Field',
                                                    draw_control_dict,
                                                    limits_3d_list=limits_3d_list,
                                                    limits_xy_list=limits_xy_list,
                                                    limits_xz_list=limits_xz_list,
        ...
        self.draw_and_save_solar_field_trajectories('AI. GPS Scan Pass Min/Max Construction',
                                                    draw_control_dict,
                                                    limits_3d_list=limits_3d_list,
                                                    limits_xy_list=limits_xy_list,
                                                    limits_xz_list=limits_xz_list,
        ...
        self.solar_field_style.heliostat_styles.add_special_names(down_heliostats, self.heliostat_down_style)
        self.flight_log_style.set_color('grey')
        self.draw_and_save_solar_field_trajectories('AJ. Inferred GPS Scan Passes',
                                                    draw_control_dict,
                                                    limits_3d_list=limits_3d_list,
                                                    limits_xy_list=limits_xy_list,
                                                    limits_xz_list=limits_xz_list,
        ...
        self.solar_field_style.heliostat_styles.add_special_names(down_heliostats, self.heliostat_down_style)
        self.flight_log_style.set_color('grey')
        self.draw_and_save_solar_field_trajectories('AK. Inferred GPS Scan Passes (All Heliostats Face Up)',
                                                    draw_control_dict,
                                                    limits_3d_list=limits_3d_list,
                                                    limits_xy_list=limits_xy_list,
                                                    limits_xz_list=limits_xz_list,
        ...
        self.solar_field_style.heliostat_styles.add_special_names(down_heliostats, self.heliostat_down_style)
        self.flight_log_style.set_color('grey')
        self.draw_and_save_solar_field_trajectories('AL. Inferred GPS Scan Pass Reference Rendering (All Heliostats Face Up)',
                                                    draw_control_dict,
                                                    limits_3d_list=limits_3d_list,
                                                    limits_xy_list=limits_xy_list,
                                                    limits_xz_list=limits_xz_list,
        ...
        self.draw_and_save_solar_field_trajectories('AM. Time Synchronization Points',
                                                    draw_control_dict,
        ...
        self.solar_field_style.heliostat_styles.add_special_names(down_heliostats, self.heliostat_down_style)
        self.flight_log_style.set_color('grey')
        self.draw_and_save_solar_field_trajectories('AN. Trajectory Fragments for Each Heliostat Connected (Face Up)',
                                                    draw_control_dict,
                                                    limits_3d_list=limits_3d_list,
                                                    limits_xy_list=limits_xy_list,
                                                    limits_xz_list=limits_xz_list,
        ...
        self.solar_field_style.heliostat_styles.add_special_names(down_heliostats, self.heliostat_down_style)
        self.flight_log_style.set_color('grey')
        self.draw_and_save_solar_field_trajectories('AO. Per Heliostat Scan Fragments Including Partial (Face Up)',
                                                    draw_control_dict,
                                                    limits_3d_list=limits_3d_list,
                                                    limits_xy_list=limits_xy_list,
                                                    limits_xz_list=limits_xz_list,
        ...
        self.solar_field_style.heliostat_styles.add_special_names(down_heliostats, self.heliostat_down_style)
        self.flight_log_style.set_color('grey')
        self.draw_and_save_solar_field_trajectories('AP. Per Heliostat Scan Fragments Including Partial (Az South, Elev Tracking Nominal Minus 1 hour)',
                                                    draw_control_dict,
                                                    limits_3d_list=limits_3d_list,
                                                    limits_xy_list=limits_xy_list,
                                                    limits_xz_list=limits_xz_list,
        ...
        self.solar_field_style.heliostat_styles.add_special_names(down_heliostats, self.heliostat_down_style)
        self.flight_log_style.set_color('grey')
        self.draw_and_save_solar_field_trajectories('AQ. Per Heliostat Scan Fragments Including Partial (Tracking Nominal Minus 1 hour)',
                                                    draw_control_dict,
                                                    limits_3d_list=limits_3d_list,
                                                    limits_xy_list=limits_xy_list,
                                                    limits_xz_list=limits_xz_list,
        ...
        self.solar_field_style.heliostat_styles.add_special_names(down_heliostats, self.heliostat_down_style)
        self.flight_log_style.set_color('grey')
        self.draw_and_save_solar_field_trajectories('AR. Per Heliostat Scan Fragments Including Partial (Tracking Nominal Minus 30 Minutes)',
                                                    draw_control_dict,
                                                    limits_3d_list=limits_3d_list,
                                                    limits_xy_list=limits_xy_list,
                                                    limits_xz_list=limits_xz_list,
        ...
        self.solar_field_style.heliostat_styles.add_special_names(down_heliostats, self.heliostat_down_style)
        self.flight_log_style.set_color('grey')
        self.draw_and_save_solar_field_trajectories('AS. Per Heliostat Scan Fragments Including Partial (Tracking Nominal Time)',
                                                    draw_control_dict,
                                                    limits_3d_list=limits_3d_list,
                                                    limits_xy_list=limits_xy_list,
                                                    limits_xz_list=limits_xz_list,
        ...
        self.solar_field_style.heliostat_styles.add_special_names(down_heliostats, self.heliostat_down_style)
        self.flight_log_style.set_color('grey')
        self.draw_and_save_solar_field_trajectories('AT. Per Heliostat Scan Fragments After Discarding Partial (Tracking Nominal Time)',
                                                    draw_control_dict,
                                                    limits_3d_list=limits_3d_list,
                                                    limits_xy_list=limits_xy_list,
                                                    limits_xz_list=limits_xz_list,
        ...
        self.solar_field_style.heliostat_styles.add_special_names(down_heliostats, self.heliostat_down_style)
        self.flight_log_style.set_color('grey')
        self.draw_and_save_solar_field_trajectories('AU. Per Heliostat Scan Fragments After Discarding Partial (Tracking Nominal Time)',
                                                    draw_control_dict,
                                                    limits_3d_list=limits_3d_list,
                                                    limits_xy_list=limits_xy_list,
                                                    limits_xz_list=limits_xz_list,
        ...
        self.solar_field_style.heliostat_styles.add_special_names(down_heliostats, self.heliostat_down_style)
        self.flight_log_style.set_color('grey')
        self.draw_and_save_solar_field_trajectories('AV. Per Heliostat Scan Pass Fit Lines (Tracking Nominal Time)',
                                                    draw_control_dict,
                                                    limits_3d_list=limits_3d_list,
                                                    limits_xy_list=limits_xy_list,
                                                    limits_xz_list=limits_xz_list,
        ...
        self.solar_field_style.heliostat_styles.add_special_names(down_heliostats, self.heliostat_down_style)
        self.flight_log_style.set_color('grey')
        self.draw_and_save_solar_field_trajectories('AW. Camera Scan Associations with GPS Scan Lines, Full Trajectory Fragments (Tracking Nominal Time)',
                                                    draw_control_dict,
                                                    limits_3d_list=limits_3d_list,
                                                    limits_xy_list=limits_xy_list,
                                                    limits_xz_list=limits_xz_list,
        ...
        self.draw_and_save_solar_field_trajectories('AX. Camera Scan Associations with GPS Scan Lines (Tracking Nominal Time)',
                                                    draw_control_dict,
                                                    limits_3d_list=limits_3d_list,
                                                    limits_xy_list=limits_xy_list,
                                                    limits_xz_list=limits_xz_list,
        ...
        self.flight_log_style.set_color('grey')
        self.scan_pass_style = rcps.outline(color=self.scan_pass_color, linewidth=0.1)
        self.draw_and_save_solar_field_trajectories('AY. Camera Scan Associations with GPS Scan Lines (Tracking Nominal Minus 1 hour)',
                                                    draw_control_dict,
                                                    limits_3d_list=limits_3d_list,
                                                    limits_xy_list=limits_xy_list,
                                                    limits_xz_list=limits_xz_list,
        ...
        self.flight_log_style.set_color('grey')
        self.scan_pass_style = rcps.outline(color=self.scan_pass_color, linewidth=0.1)
        self.draw_and_save_solar_field_trajectories('AZ. Camera Scan Lines (Tracking Nominal Minus 1 hour)',
                                                    draw_control_dict,
                                                    limits_3d_list=limits_3d_list,
                                                    limits_xy_list=limits_xy_list,
                                                    limits_xz_list=limits_xz_list,
        ...
        self.flight_log_style.set_color('grey')
        self.scan_pass_style = rcps.outline(color=self.scan_pass_color, linewidth=0.1)
        self.draw_and_save_solar_field_trajectories('BA. Camera Scan Lines (Face Up)',
                                                    draw_control_dict,
                                                    limits_3d_list=limits_3d_list,
                                                    limits_xy_list=limits_xy_list,
                                                    limits_xz_list=limits_xz_list,
        ...
        self.flight_log_style.set_color('grey')
        self.scan_pass_style = rcps.outline(color=self.scan_pass_color, linewidth=0.1)
        self.draw_and_save_solar_field_trajectories('BB. GPS-Camera Alignment Results, Showing Connections (Face Up)',
                                                    draw_control_dict,
                                                    limits_3d_list=limits_3d_list,
                                                    limits_xy_list=limits_xy_list,
                                                    limits_xz_list=limits_xz_list,
        ...
        self.flight_log_style.set_color('grey')
        self.scan_pass_style = rcps.outline(color=self.scan_pass_color, linewidth=0.1)
        self.draw_and_save_solar_field_trajectories('BC. GPS-Camera Alignment Results, Showing Connections (Per-Heliostat Tracking)',
                                                    draw_control_dict,
                                                    limits_3d_list=limits_3d_list,
                                                    limits_xy_list=limits_xy_list,
                                                    limits_xz_list=limits_xz_list,
        ...
        self.flight_log_style.set_color('grey')
        self.scan_pass_style = rcps.outline(color=self.scan_pass_color, linewidth=0.1)
        self.draw_and_save_solar_field_trajectories('BD. GPS-Camera Alignment Results, with Trajectory Fragments (Per-Heliostat Tracking)',
                                                    draw_control_dict,
                                                    limits_3d_list=limits_3d_list,
                                                    limits_xy_list=limits_xy_list,
                                                    limits_xz_list=limits_xz_list,
        ...
        self.flight_log_style.set_color('grey')
        self.scan_pass_style = rcps.outline(color=self.scan_pass_color, linewidth=0.1)
        self.draw_and_save_solar_field_trajectories('BE. GPS-Camera Alignment Results (Per-Heliostat Tracking)',
                                                    draw_control_dict,
                                                    limits_3d_list=limits_3d_list,
                                                    limits_xy_list=limits_xy_list,
                                                    limits_xz_list=limits_xz_list,
        ...
        self.solar_field_style.heliostat_styles.add_special_names(up_heliostats, self.heliostat_up_style)
        self.solar_field_style.heliostat_styles.add_special_names(down_heliostats, self.heliostat_down_style)
        self.draw_and_save_solar_field_trajectories('BF. Measured Solar Field Configuration After GPS-Camera Alignment (Per-Heliostat Tracking)',
                                                    draw_control_dict,
                                                    limits_3d_list=limits_3d_list,
                                                    limits_xy_list=limits_xy_list,
                                                    limits_xz_list=limits_xz_list,
    ...
    def draw_and_save_solar_field_trajectories(self,
                                               ...
                                               limits_yz_list=None):  # Form:  [ None, [[x_min1,x_max1],[y_min1,y_max1]], ...]
        # Filename.
        ...
        if limits_3d_list != None:
            ...
            view_3d.show_and_save_multi_axis_limits(self.output_data_dir, output_figure_body, limits_list=limits_3d_list, grid=True)
        # xy view.
        if limits_xy_list != None:
            view_xy = self.draw_solar_field_trajectories(title, draw_control_dict, vs.view_spec_xy())
            view_xy.show_and_save_multi_axis_limits(self.output_data_dir, output_figure_body, limits_list=limits_xy_list, grid=False)
        # xz_view.
        if limits_xz_list != None:
            view_xz = self.draw_solar_field_trajectories(title, draw_control_dict, vs.view_spec_xz())
            view_xz.show_and_save_multi_axis_limits(self.output_data_dir, output_figure_body, limits_list=limits_xz_list, grid=True)
        # yz view.
        if limits_yz_list != None:
            view_yz = self.draw_solar_field_trajectories(title, draw_control_dict, vs.view_spec_yz())
            view_yz.show_and_save_multi_axis_limits(self.output_data_dir, output_figure_body, limits_list=limits_yz_list, grid=True)

    def draw_solar_field_trajectories(self, title, draw_control_dict, view_spec, grid=False):  # ?? SCAFFOLDING RCB -- MOVE ANALYSIS OUT OF THIS RENDERING ROUTINE.
        # Solar field.
        # Required, because this creates the view.
        view = self.solar_field.draw_figure(self.figure_control, self.axis_control_m, view_spec, title, self.solar_field_style, grid=grid)
        # GPS flight log.
        ...
        if draw_control_dict['draw_trajectory_fragments'] or \
           draw_control_dict['draw_synchronization_points'] or \
           draw_control_dict['draw_camera_passes']:
            idx = 0
            for hel_name in self.hel_frames_dict.keys():
                ...
                if draw_control_dict['draw_trajectory_fragments'] and \
                   ((draw_control_dict['trajectory_fragment_selected_heliostats'] == None) or (hel_name in draw_control_dict['trajectory_fragment_selected_heliostats'])):
                    ...
                    if draw_control_dict['connect_trajectory_fragments']:
                        ...
                        list_of_camera_xyz_lists = self.split_xyz_list_where_distance_exceeds_maximum(camera_xyz_world_list, draw_control_dict['trajectory_fragment_disconnect_threshold'])
                    ...
                    if draw_control_dict['draw_synchronization_points']:
                        ...
                        synchronization_camera_xyz_world = synchronization_frame_parameters_dict['per_heliostat_estimate_of_camera_xyz_in_world_coords_overall_time']
                        style = copy.deepcopy(self.synchronization_point_style)
                        style.set_color(camera_pass_color)  # Similar color, but slightly darker than trajectory fragment.
        ...
        if draw_control_dict['draw_gps_camera_analysis']:
            ...
            for hel_name in self.hel_gps_camera_analysis_dict:
                ...
                list_of_gps_camera_analysis_dicts = self.hel_gps_camera_analysis_dict[hel_name]
                for gps_camera_analysis_dict in list_of_gps_camera_analysis_dicts:
                    per_heliostat_transformed_camera_pass = gps_camera_analysis_dict['per_heliostat_transformed_camera_pass']
                    ...
                    if draw_control_dict['draw_gps_camera_pass']:
                        ...
                        if draw_control_dict['draw_gps_transformed_camera_pass_connections']:
                            style = copy.deepcopy(self.pass_connection_style)
                            style.set_color(trajectory_fragment_color)
                            transformed_camera_gps_point_pair_list = gps_camera_analysis_dict['camera_gps_point_pair_list']
    ...
    def draw_and_save_gps_log_analysis_plots(self):
        ...
        data_markersize = 1
        # Position.
        figure_record = pp.dataframe_plot(self.figure_control,
                                          self.flight_log_df,
                                          title='GPS Trajectory Positions',
                                          x_column='time(sec)',
                                          y_column_label_styles=[['x(m)', 'x component', rcps.outline(color='b', linewidth=data_linewidth)],
        ...
        figure_record = pp.dataframe_plot(self.figure_control,
                                          ...
                                          x_column='time(sec)',
                                          y_column_label_styles=[['velocity_average_x(m/sec)', 'x component', rcps.outline(color='b', linewidth=data_linewidth)],
        ...
        figure_record = pp.dataframe_plot(self.figure_control,
                                          ...
                                          x_column='time(sec)',
                                          y_column_label_styles=[['speed_average(m/sec)', None, rcps.outline(color='b', linewidth=data_linewidth)],
        ...
        figure_record = pp.dataframe_plot(self.figure_control,
                                          ...
                                          x_column='time(sec)',
                                          y_column_label_styles=[['delta_velocity_angle_xy(rad)', None, rcps.outline(color='b', linewidth=data_linewidth)]],
                                          x_axis_label='time (sec)',
        ...
        for max_min_pass_pair in self.maximum_to_minimum_pass_pair_list:
            ...
            gp.add_xy_list_to_plot(figure_record,
        ...
        figure_record = pp.dataframe_plot(self.figure_control,
                                          ...
                                          x_column='time(sec)',
                                          y_column_label_styles=[['delta_velocity_angle_z(rad)', None, rcps.outline(color='b', linewidth=data_linewidth)]],
                                          x_axis_label='time (sec)',
...
#             view.show()
    ...
    def save_enhanced_flight_log(self):
        if not (os.path.exists(self.output_data_dir)):
            ...
            input_flight_log_dir, input_flight_log_body, input_flight_log_ext = ft.path_components(self.input_flight_log_dir_body_ext)
            ...
            print('In TrajectoryAnalysis.save_enhanced_flight_log(), saving enhanced flight log file :', output_flight_log_plus_dir_body_ext)
            # ?? SCAFFOLDING RCB -- FLIGHT_LOG SHOULD BE A CLASS, SUPPORTING FUNCTIONS SUCH A MID-TIME, ALTITUDE AND POSITION LIMITS, ETC.  SHOULD ALSO HANDLE LOGS FROM DIFFERENT UAS SOURCES.
    ...
    def save_gps_ymdhmsz_given_flight_log_zero_seconds(self):
        ...
        return ft.write_dict_file(explain,                      # Explanatory string to include in notification output.  None to skip.
    ...
    def save_gps_velocity_xy_change_points(self, list_of_gps_velocity_xy_change_dicts, minima_or_maxima_str):
        ...
        return dt.save_list_of_one_level_dicts(list_of_gps_velocity_xy_change_dicts, self.output_data_dir, output_body, explain, error_if_dir_not_exist=False)
    ...
    def save_gps_velocity_xy_change_pairs(self, pass_pair_list, maximum_to_minimum_or_minimum_to_maximum_str):
        output_body = self.input_video_body + '_gps_velocity_xy_change_' + maximum_to_minimum_or_minimum_to_maximum_str + '_pass_pair_list'
        explain = 'GPS velocity xy change ' + maximum_to_minimum_or_minimum_to_maximum_str + ' pass pair file'
        return dt.save_list_of_one_level_dict_pairs(pass_pair_list, self.output_data_dir, output_body, explain, error_if_dir_not_exist=False)
    ...
    def save_gps_scan_passes(self):
        ...
        output_dir_body_ext = ft.write_csv_file(explain,                       # Explanatory string to include in notification output.  None to skip.
                                                self.output_data_dir,          # Directory to write file.  See below if not exist.
                                                output_body,                   # Body of output filename; extension is ".csv"
                                                heading_line,                  # First line to write to file.  None to skip.
                                                data_lines,                    # Subsequent lines to write to file.
                                                error_if_dir_not_exist=False)  # If True, error if not exist.  If False, create dir if necessary.
        # Return.
    ...
    def gps_scan_pass_data_line(self, scan_pass):
        # Stable begin point.
        data_line_str = ''
        data_line_str += str(scan_pass['stable_begin_xyzt'][0]) + ',' + str(scan_pass['stable_begin_xyzt'][1]) + ',' + \
            str(scan_pass['stable_begin_xyzt'][2]) + ',' + str(scan_pass['stable_begin_xyzt'][3])
        # Stable end point.
        data_line_str += ','
        data_line_str += str(scan_pass['stable_end_xyzt'][0]) + ',' + str(scan_pass['stable_end_xyzt'][1]) + ',' + str(scan_pass['stable_end_xyzt'][2]) + ',' + str(scan_pass['stable_end_xyzt'][3])
        # Embedding 3-d line.
        data_line_str += ','
        data_line_str += str(scan_pass['line_3d']['length']) + ',' + str(scan_pass['line_3d']['uxyz'][0]) + ',' + str(scan_pass['line_3d']['uxyz'][1]) + \
            ',' + str(scan_pass['line_3d']['uxyz'][2]) + ',' + str(scan_pass['line_3d']['theta']) + ',' + str(scan_pass['line_3d']['eta'])
        # RMS point-to-line distance.
        data_line_str += ','
        ...
        if n_points != len(distance_to_line_list):
            print('ERROR: In gps_scan_pass_data_line(), unexpected number of distance values encountered.  n_points =', n_points, '; len(distance_to_line_list) =', len(distance_to_line_list))
        for d in distance_to_line_list:
            data_line_str += ',' + str(d)
        # List of individual points.
        inlier_xyzt_list = scan_pass['inlier_xyzt_list']
        if n_points != len(inlier_xyzt_list):
            print('ERROR: In gps_scan_pass_data_line(), unexpected number of inliers encountered.  n_points =', n_points, '; len(inlier_xyzt_list) =', len(inlier_xyzt_list))
    ...
    def save_hel_frames_dict(self):
        ...
        return ft.write_pickle_file(explain, self.output_data_dir, output_body, self.hel_frames_dict, error_if_dir_not_exist=False)
    ...
    def save_hel_camera_passes_dict(self):
        ...
        return ft.write_pickle_file(explain, self.output_data_dir, output_body, self.hel_camera_passes_dict, error_if_dir_not_exist=False)
    ...
    def save_hel_gps_camera_analysis_dict(self):
        ...
        return ft.write_pickle_file(explain, self.output_data_dir, output_body, self.hel_gps_camera_analysis_dict, error_if_dir_not_exist=False)
    ...
    def check_pickle_files(self):
        ...
        self.print_hel_camera_passes_dict_aux(pickle.load(open(self.hel_camera_passes_dict_dir_body_ext, 'rb')), max_heliostats=2)
        # GPS-camera analysis.
        print('\nIn TrajectoryAnalysis.check_pickle_files(), loaded hel_gps_camera_analysis_dict:')
        self.print_hel_gps_camera_analysis_dict_aux(pickle.load(open(self.hel_gps_camera_analysis_dict_dir_body_ext, 'rb')), max_heliostats=2)
...
if __name__ == "__main__":
    ...
    log_dir_body_ext = experiment_dir() + '2020-12-03_FastScan1/3_Post/Answers/20201203/1544_NS_U/190_TrajectoryAnalysis/mavic_zoom/log/TrajectoryAnalysis_log.txt'
    ...
    # Input/output sources.
    specifications = Dspec.nsttf_specifications()  # Solar field parameters.
    aimpoint_xyz = [60.0, 8.8, 60.0]  # ?? SCAFFOLDING RCB -- READ THIS FROM THE 030_Aimpoints DIRECTORY.  SUPPORT MULTIPLE HELIOSTATS.
    #                                            year, month, day, hour, minute, second, zone]
    when_ymdhmsz = [2020, 12, 3, 15, 45, 0, -7]  # Nominal time, refined by computation.  Recommend use mid-point of flight.
    ...
    input_video_dir_body_ext = experiment_dir() + '2020-12-03_FastScan1/2_Data/20201203/1544_NS_U/mavic_zoom/DJI_427t_428_429.MP4'
    input_flight_log_dir_body_ext = experiment_dir() + '2020-12-03_FastScan1/3_Post/Answers/20201203/1544_NS_U/060_FlightData/data/_F08_log_2020-12-03_15-44-13_v2.csv'
    input_reconstructed_heliostats_dir = experiment_dir() + '2020-12-03_FastScan1/3_Post/Answers/20201203/1544_NS_U/180_Heliostats3d/mavic_zoom/data/'
    output_data_dir = experiment_dir() + '2020-12-03_FastScan1/3_Post/Answers/20201203/1544_NS_U/190_TrajectoryAnalysis/mavic_zoom/data/'
    output_render_dir = experiment_dir() + '2020-12-03_FastScan1/3_Post/Answers/20201203/1544_NS_U/190_TrajectoryAnalysis/mavic_zoom/render/'
    output_construction_dir = experiment_dir() + '2020-12-03_FastScan1/3_Post/Construction/20201203/1544_NS_U/190c_TrajectoryAnalysis/mavic_zoom/'
