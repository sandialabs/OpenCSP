...
class FramesNoDuplicates:
    ...
    def __init__(
        ...
        if (input_video_dir_body_ext == None) or (len(input_video_dir_body_ext) == 0):
            raise ValueError(
                'In FramesNoDuplicates.__init__(), null input_video_dir_body_ext encountered.'
            )
        if (input_frame_dir == None) or (len(input_frame_dir) == 0):
            raise ValueError(
                'In FramesNoDuplicates.__init__(), null input_frame_dir encountered.'
            )
        if (output_data_dir == None) or (len(output_data_dir) == 0):
            raise ValueError(
                'In FramesNoDuplicates.__init__(), null output_data_dir encountered.'
            )
        if (output_render_dir == None) or (len(output_render_dir) == 0):
            raise ValueError(
                'In FramesNoDuplicates.__init__(), null output_render_dir encountered.'
            )
        if (output_frame_dir == None) or (len(output_frame_dir) == 0):
            raise ValueError(
                'In FramesNoDuplicates.__init__(), null output_frame_dir encountered.'
            )
        ...
        self.non_duplicate_dir_body_ext = os.path.join(
            self.output_data_dir, self.non_duplicate_body_ext
        )
        ...
        self.duplicate_dir_body_ext = os.path.join(
            self.output_data_dir, self.duplicate_body_ext
        )
    ...
    def filter_frames_and_write_data(self):
        ...
        if ft.directory_is_empty(self.output_frame_dir):
            print(
                'In FramesNoDuplicates.filter_frames_and_write_data(), filtering frames...'
            )

            # Identify duplicate frames.
            (non_duplicate_frame_files, duplicate_frame_files) = (
                vm.identify_duplicate_frames(
                    self.input_frame_dir,
                    self.output_frame_dir,
                    self.tolerance_image_size,
                    self.tolerance_image_pixel,
                )
            )
            ...
            for frame_file in non_duplicate_frame_files:
                ...
                ft.copy_file(
                    os.path.join(input_frame_dir, frame_file), output_frame_dir
                )
            ...
            print(
                'In FramesNoDuplicates.filter_frames_and_write_data(), writing frame summary statistics...'
            )
            ft.write_dict_file(
                'frame summary statistics (duplicates removed)',
                self.output_data_dir,
                self.dict_body,
                summary_dict,
            )
            ft.write_text_file(
                'non-duplicate frames',
                self.output_data_dir,
                self.non_duplicate_body,
    ...
    def read_data(self):
        print(
            'In FramesNoDuplicates.read_data(), reading frame statistics: ',
