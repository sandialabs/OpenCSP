...
class VideoTracks():
    ...
    def __init__(self,
                 ...
                 render_control_confirmed):       # Flags to control rendering on this run, for the confirmed video.
        ...
        self.video_projected_tracks_dir_body_ext = os.path.join(self.output_data_dir, self.video_projected_tracks_body_ext)
        # Confirmed.
        ...
        self.video_confirmed_tracks_dir_body_ext = os.path.join(self.output_data_dir, self.video_confirmed_tracks_body_ext)
        ...
        # Projected.
        print('In VideoTracks.__init__(), reading found key frame projected tracks directory: ', self.input_key_projected_tracks_dir)
        ...
        for key_projected_tracks_body_ext in key_projected_tracks_body_ext_list:
            ...
            if (self.specific_frame_ids is None) or (key_frame_id in self.specific_frame_ids):
                key_projected_tracks_dir_body_ext = os.path.join(self.input_key_projected_tracks_dir, key_projected_tracks_body_ext)
                print('In KeyTracks.__init__(), reading key frame projected tracks file:     ', key_projected_tracks_dir_body_ext)
                key_projected_track_fnxl = fnxl.FrameNameXyList()
                key_projected_track_fnxl.load(key_projected_tracks_dir_body_ext)
                key_projected_track_fnxl_sorted_frame_id_list = key_projected_track_fnxl.sorted_frame_id_list()  # Save repeat calls to this.
                # Store results.
                key_frame_dict = {}
                ...
                self.key_projected_tracks_dict[key_frame_id]['key_projected_track_body_ext']      = key_projected_tracks_body_ext
                self.key_projected_tracks_dict[key_frame_id]['key_projected_track_dir_body_ext']  = key_projected_tracks_dir_body_ext
                self.key_projected_tracks_dict[key_frame_id]['key_projected_track_fnxl']          = key_projected_track_fnxl
                self.key_projected_tracks_dict[key_frame_id]['key_projected_track_n_frames']      = len(key_projected_track_fnxl_sorted_frame_id_list)
                self.key_projected_tracks_dict[key_frame_id]['key_projected_track_min_frame_id']  = key_projected_track_fnxl_sorted_frame_id_list[0]
                self.key_projected_tracks_dict[key_frame_id]['key_projected_track_max_frame_id']  = key_projected_track_fnxl_sorted_frame_id_list[-1]
        ...
        # Confirmed.
        print('In VideoTracks.__init__(), reading found key frame confirmed tracks directory: ', self.input_key_confirmed_tracks_dir)
        ...
        for key_confirmed_tracks_body_ext in key_confirmed_tracks_body_ext_list:
            ...
            key_frame_id     = upf.frame_id_given_frame_id_str(key_frame_id_str)
            if (self.specific_frame_ids is None) or (key_frame_id in self.specific_frame_ids):
                key_confirmed_tracks_dir_body_ext = os.path.join(self.input_key_confirmed_tracks_dir, key_confirmed_tracks_body_ext)
                print('In KeyTracks.__init__(), reading key frame confirmed tracks file:     ', key_confirmed_tracks_dir_body_ext)
                key_confirmed_track_fnxl = fnxl.FrameNameXyList()
                key_confirmed_track_fnxl.load(key_confirmed_tracks_dir_body_ext)
                key_confirmed_track_fnxl_sorted_frame_id_list = key_confirmed_track_fnxl.sorted_frame_id_list()  # Save repeat calls to this.
                # Store results.
                key_frame_dict = {}
                ...
                self.key_confirmed_tracks_dict[key_frame_id]['key_confirmed_track_body_ext']      = key_confirmed_tracks_body_ext
                self.key_confirmed_tracks_dict[key_frame_id]['key_confirmed_track_dir_body_ext']  = key_confirmed_tracks_dir_body_ext
                self.key_confirmed_tracks_dict[key_frame_id]['key_confirmed_track_fnxl']          = key_confirmed_track_fnxl
                self.key_confirmed_tracks_dict[key_frame_id]['key_confirmed_track_n_frames']      = len(key_confirmed_track_fnxl_sorted_frame_id_list)
                self.key_confirmed_tracks_dict[key_frame_id]['key_confirmed_track_min_frame_id']  = key_confirmed_track_fnxl_sorted_frame_id_list[0]
                self.key_confirmed_tracks_dict[key_frame_id]['key_confirmed_track_max_frame_id']  = key_confirmed_track_fnxl_sorted_frame_id_list[-1]
        # Confirm what was read.
    ...
    def construct_and_save_video_tracks(self):
        ...
        if self.force_construction or (not ft.directory_exists(self.output_data_dir)) or ft.directory_is_empty(self.output_data_dir):
    ...
    def construct_and_save_video_tracks_aux(self, key_tracks_dict, projected_or_confirmed_str):
        print('In VideoTracks.construct_and_save_video_tracks_aux(), constructing ' + projected_or_confirmed_str + ' video tracks...')
        ...
        for key_frame_id in key_frame_ids_to_process:
            ...
            for frame_id in key_track_fnxl.sorted_frame_id_list():  # Use sorted list so status output is easier to understand.
                video_tracks_fnxl.merge_list_of_name_xy_lists(frame_id, key_track_fnxl.list_of_name_xy_lists(frame_id), 
                                                                warn_if_common_name=True, 
                                                                skip_if_common_name=True,
                                                                error_if_common_name=False)

        # Summarize construction result.
        print('In VideoTracks.construct_and_save_video_tracks_aux(), constructed ' + projected_or_confirmed_str + ' video_tracks_fnxl:')
    ...
    def save_video_tracks(self, video_tracks_fnxl, projected_or_confirmed_str):
        ...
        if projected_or_confirmed_str == 'projected':
            ...
            msg = 'In VideoTracks.save_video_tracks(), encountered projected_or_confirmed_str="'+str(projected_or_confirmed_str)+'" which was neither "projected" or "confirmed".'
            print('ERROR: ' + msg)
    ...
    def save_data(self, video_tracks_fnxl, projected_or_confirmed_str):
        ...
        if projected_or_confirmed_str == 'projected':
            ...
            msg = 'In VideoTracks.save_data(), encountered projected_or_confirmed_str="'+str(projected_or_confirmed_str)+'" which was neither "projected" or "confirmed".'
            print('ERROR: ' + msg)
            raise ValueError(msg)
        # Statistics.
        summary_dict = {}
        summary_dict['n_video_track_frames'] = video_tracks_fnxl.number_of_frames()
        print('In VideoTracks.save_data(), writing video frame ' + projected_or_confirmed_str + ' summary statistics...')
        ft.write_dict_file('video ' + projected_or_confirmed_str + ' tracks summary statistics', self.output_data_dir, dict_body, summary_dict)
        # Heliostats per video frame.
        heliostats_per_video_frame_dict = video_tracks_fnxl.heliostats_per_frame()
        print('In VideoTracks.save_data(), writing ' + projected_or_confirmed_str + ' heliostats per video frame:', os.path.join(self.output_data_dir, hpvf_body_ext))
        ...
        print('In VideoTracks.save_data(), writing points per video frame:    ', os.path.join(self.output_data_dir, ppvf_body_ext))
    ...
    def read_video_tracks(self):
        # Projected.
        print('In VideoTracks.read_video_tracks(), reading video projected tracks file: ', self.video_projected_tracks_dir_body_ext)
        ...
        # Confirmed.
        print('In VideoTracks.read_video_tracks(), reading video confirmed tracks file: ', self.video_confirmed_tracks_dir_body_ext)
    ...
    def render(self):
        print('In VideoTracks.render(), self.output_construction_projected_dir=', self.output_construction_projected_dir)  # ?? SCFFOLDING RCB -- TEMPORARY
        print('In VideoTracks.render(), self.output_construction_confirmed_dir=', self.output_construction_confirmed_dir)  # ?? SCFFOLDING RCB -- TEMPORARY
        # Projected.
        if self.render_control_projected.draw_video_tracks and self.generated_video_projected_tracks:  # Don't render unless we generated.
            self.render_aux(self.video_projected_tracks_fnxl, self.output_construction_projected_dir, self.output_video_projected_dir_body, self.render_control_projected, 'projected')
        # Confirmed.
        if self.render_control_confirmed.draw_video_tracks and self.generated_video_confirmed_tracks:  # Don't render unless we generated.
            self.render_aux(self.video_confirmed_tracks_fnxl, self.output_construction_confirmed_dir, self.output_video_confirmed_dir_body, self.render_control_confirmed, 'confirmed')


    def render_aux(self, video_tracks_fnxl, output_construction_dir, output_video_dir_body, render_control, projected_or_confirmed_str):
        print('In VideoTracks.render_aux(), rendering video ' + projected_or_confirmed_str + ' tracks...')
        print('In VideoTracks.render_aux(), output_construction_dir=', output_construction_dir)  # ?? SCFFOLDING RCB -- TEMPORARY
        # Descriptive strings.
        ...
        style_dict = {}
        style_dict['point_seq'] = rcps.marker(marker=render_control.video_tracks_points_marker, 
                                              markersize=render_control.video_tracks_points_markersize,
                                              color=render_control.video_tracks_points_color)
        style_dict['text'] = rctxt.RenderControlText(horizontalalignment=render_control.video_tracks_label_horizontalalignment,
        ...
        # Draw the frames.
        video_tracks_fnxl.draw_frames(self.single_processor,
...
# output_construction_dir        = experiment_dir() + '2020-12-03_FastScan1/3_Post/Construction/20201203/1544_NS_U/Small_160c_VideoTracks/mavic_zoom/'
    log_dir_body_ext               = experiment_dir() + '2020-12-03_FastScan1/3_Post/Answers/20201203/1544_NS_U/160_VideoTracks/mavic_zoom/log/VideoTracks_log.txt'
    # Input/output sources.
    input_video_dir_body_ext       = experiment_dir() + '2020-12-03_FastScan1/2_Data/20201203/1544_NS_U/mavic_zoom/DJI_427t_428_429.MP4'
    input_key_projected_tracks_dir = experiment_dir() + '2020-12-03_FastScan1/3_Post/Answers/20201203/1544_NS_U/150_KeyTracks/mavic_zoom/data/key_frame_projected_tracks/'
    input_key_confirmed_tracks_dir = experiment_dir() + '2020-12-03_FastScan1/3_Post/Answers/20201203/1544_NS_U/150_KeyTracks/mavic_zoom/data/key_frame_confirmed_tracks/'
    input_frame_dir                = experiment_dir() + '2020-12-03_FastScan1/3_Post/Construction/20201203/1544_NS_U/080c_FramesNoDuplicates/mavic_zoom/frames/'
    input_frame_id_format          = '06d' # Note different from format used in ffmpeg call, which is '.%06d'
    output_data_dir                = experiment_dir() + '2020-12-03_FastScan1/3_Post/Answers/20201203/1544_NS_U/160_VideoTracks/mavic_zoom/data/'
    output_render_dir              = experiment_dir() + '2020-12-03_FastScan1/3_Post/Answers/20201203/1544_NS_U/160_VideoTracks/mavic_zoom/render/'
