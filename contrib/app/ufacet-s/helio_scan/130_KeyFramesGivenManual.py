...
class KeyFramesGivenManual():
    ...
    def __init__(self,
        ...
        if (input_original_keyinfo_dir_body_ext == None) or (len(input_original_keyinfo_dir_body_ext) == 0):
            raise ValueError('In KeyFramesGivenManual.__init__(), null input_original_keyinfo_dir_body_ext encountered.')
        ...
        if not ft.file_exists(self.key_frames_dir_body_ext):
            ...
            if False:  # Set to True to force conversion from original data.
                ...
                for instance_key in keyinfo_dict.keys():
                    ...
                    key_frame_id, list_of_name_polygons = self.construct_key_frame_entry(instance_key, keyinfo_dict, instance_frame_correspondence_dict)
                ...
                print('In KeyFramesGivenManual.convert_and_save_original_keyinfo_file(), reading already-converted keyframe specification...')
                # Read keyframe file after editing.
                print('In KeyFramesGivenManual.convert_and_save_original_keyinfo_file(), reading key frames: ', self.input_edited_key_frames_dir_body_ext)
                key_frames_fnxl = fnxl.FrameNameXyList()
                ...
                key_frames_fnxl.print(max_keys=7,
                                      max_value_length=200,
                                      indent=4)

            # Write key frame file.
            print('In KeyFramesGivenManual.convert_and_save_original_keyinfo_file(), writing key frames: ', self.key_frames_dir_body_ext)
            ...
            print('In KeyFramesGivenManual.convert_and_save_original_keyinfo_file(), writing key frame summary statistics...')
            ...
            print('In KeyFramesGivenManual.convert_and_save_original_keyinfo_file(), writing heliostats per key frame:', os.path.join(self.output_data_dir, self.hpkf_body_ext))
            ...
            if line != 'None' and line != '':
                ...
                if self.input_original_keyinfo_boxHD:
                    if self.input_original_keyinfo_hd_shape == None:
                        print('ERROR: In KeyFramesGivenManual.read_keyinfo(), input_original_keyinfo_boxHD == True, but input_original_keyinfo_hd_shape == None.')
                        assert False
    ...
    def construct_key_frame_entry(self, instance_key, keyinfo_dict, instance_frame_correspondence_dict):
        ...
        keyinfo = keyinfo_dict[instance_key]  # Tuple (frame_id_within_instance, number_of_heliosats, box_coords, heliostat_names)
    ...
    def construct_key_frame_entry_aux(self, number_of_heliostats, heliostat_names, box_coords):
        ...
        if number_of_heliostats != len(heliostat_names):
            print('ERROR: In KeyFramesGivenManual.construct_key_frame_entry_aux(), number_of_heliostats=' +
                  str(number_of_heliostats) + ' is not equal to len(heliostat_names)=' + str(len(heliostat_names)) + '.')
            assert False
        if (number_of_heliostats * 4) != len(box_coords):
            print('ERROR: In KeyFramesGivenManual.construct_key_frame_entry_aux(), number_of_heliostats=' +
                  str(number_of_heliostats) + ' is not equal to 1/4 len(box_coords)=' + str(len(box_coords)) + '.')
            assert False
    ...
    def render(self):
        if self.render_control.draw_key_frames:
            ...
            style_dict = {}
            style_dict['point_seq'] = rcps.outline(color=self.render_control.key_frame_polygon_color,
                                                   linewidth=self.render_control.key_frame_polygon_linewidth)
            style_dict['text'] = rctxt.RenderControlText(horizontalalignment=self.render_control.key_frame_label_horizontalalignment,
            ...
            # Draw the frames.
            self.key_frames_fnxl.draw_frames(self.single_processor,
...
if __name__ == "__main__":
    ...
    log_dir_body_ext = experiment_dir() + '2020-12-03_FastScan1/3_Post/Answers/20201203/1544_NS_U/130_KeyFrames/mavic_zoom/log/KeyFrames_log.txt'
    # Input/output sources.
    input_video_dir_body_ext = experiment_dir() + '2020-12-03_FastScan1/2_Data/20201203/1544_NS_U/mavic_zoom/DJI_427t_428_429.MP4'
    input_original_keyinfo_dir_body_ext = experiment_dir() + '2020-12-03_FastScan1/3_Post/Answers/20201203/1544_NS_U/130_KeyFrames/mavic_zoom/data/original/vflight_12_2020_keyinfo_with_instances_edited.txt'
    input_original_keyinfo_boxHD = False
    input_original_keyinfo_hd_shape = None
    input_edited_key_frames_dir_body_ext = experiment_dir() + '2020-12-03_FastScan1/3_Post/Answers/20201203/1544_NS_U/130_KeyFrames/mavic_zoom/data/original/DJI_427t_428_429_key_frames_CONVERTED_FROM_ORIGINAL_CORRECTED.csv'
    ...
    output_data_dir = experiment_dir() + '2020-12-03_FastScan1/3_Post/Answers/20201203/1544_NS_U/130_KeyFrames/mavic_zoom/data/'
    output_render_dir = experiment_dir() + '2020-12-03_FastScan1/3_Post/Answers/20201203/1544_NS_U/130_KeyFrames/mavic_zoom/render/'
    # Render control.
