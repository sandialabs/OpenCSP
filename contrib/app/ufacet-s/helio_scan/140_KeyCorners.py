...
class KeyCorners():
    ...
    def __init__(self,
                 ...
                 # Render control.
                 render_control):               # Flags to control rendering on this run.
        ...
        self.all_key_frames_corners_dir_body_ext = os.path.join(self.output_data_dir, self.all_key_frames_corners_body_ext)
        ...
        self.key_frames_fnxl.print(max_keys=7,
    ...
    def find_and_save_key_corners(self):
        ...
        if self.force_construction or (not ft.directory_exists(self.output_data_dir)) or ft.directory_is_empty(self.output_data_dir):
            ...
            if self.specific_frame_ids == None:
                key_frame_ids_to_process = self.key_frames_fnxl.sorted_frame_id_list()  # We sort only to better observe progress.  Not required.
            else:
            ...
            # Remove "None" entries.
            list_of_fnxl_or_None_results = [result_dict['pair_projected_fnxl_or_None'] for result_dict in list_of_result_dicts]
            key_frame_fnxls = [x for x in list_of_fnxl_or_None_results if x is not None]
            print('In KeyCorners.search_key_frames(), key frame corner search done.  len(key_frame_fnxls) =', len(key_frame_fnxls))

            # Extract mismatched heliostat frame_ids.
            mismatched_key_frame_id_or_None_list = [result_dict['mismatched_key_frame_id_or_None'] for result_dict in list_of_result_dicts]
            mismatched_key_frame_ids = [x for x in mismatched_key_frame_id_or_None_list if x is not None]
            print('In KeyCorners.search_key_frames(), mismatched corner frames extracted.  len(mismatched_key_frame_ids) =', len(mismatched_key_frame_ids))
            print('In KeyCorners.search_key_frames(), mismatched corner frames extracted.  mismatched_key_frame_ids =', mismatched_key_frame_ids)  # ?? SCAFFOLDING RCB -- TEMPORARY
            ...
            print('\nIn KeyCorners.find_and_save_key_corners(), all_key_frames_corners_fnxl.number_of_frames() =', all_key_frames_corners_fnxl.number_of_frames())
    ...
    def search_key_frame(self, key_frame_id):
        ...
        key_frame_body_ext_1 = upf.frame_file_body_ext_given_frame_id(self.input_video_body, key_frame_id, self.input_frame_id_format)
        if (key_frame_body_ext_1 not in self.all_frame_file_list):
            logt.log_and_raise_value_error(self.logger, 'In KeyCorners.search_key_frames(), the full frame file directory:\n   ', self.input_frame_dir, '\ndoes not contain key frame file:\n   ', key_frame_body_ext_1)
        ...
        if (key_frame_file_2_idx >= len(self.all_frame_file_list)):
            logt.log_and_raise_value_error(self.logger, 'In KeyCorners.search_key_frames(), key_frame_id='+str(key_frame_id)+' is the last file the full frame file directory:\n   ', self.input_frame_dir, '\nso there is no second frame file available.\nSelecting the final video frame as a key frame is not allowed.')
        ...
        try:
            search_result_1 = self.search_key_frame_aux(local_logger, key_frame_id_1, list_of_name_polygons, key_frame_body_ext_1, output_construction_dir_1)
        except:
            error_type, error_instance, traceback = sys.exc_info()
            logt.log_and_raise_value_error(local_logger, 'In KeyCorners.search_key_frame(), Key Frame 1 Processing Exception for key_frame_id=' + str(key_frame_id_1) + ', ' + str(error_instance.args[0]))

        # Search for key_frame 2 corners.
        try:
            search_result_2 = self.search_key_frame_aux(local_logger, key_frame_id_2, list_of_name_polygons, key_frame_body_ext_2, output_construction_dir_2)
        except:
            error_type, error_instance, traceback = sys.exc_info()
            logt.log_and_raise_value_error(local_logger, 'In KeyCorners.search_key_frame(), Key Frame 2 Processing Exception for key_frame_id=' + str(key_frame_id_2) + ', ' + str(error_instance.args[0]))
        ...
        if (not search_result_1.successful()) or (not search_result_2.successful()):
            ...
            pair_projected_fnxl_or_None, mismatched = self.construct_merged_fnxl_synchronizing_heliostat_names(local_logger, 
    ...
    def search_key_frame_aux(self, local_logger, key_frame_id, list_of_name_polygons, key_frame_body_ext, output_construction_dir):
        ...
        if key_frame_img is None:
            logt.log_and_raise_value_error(self.logger, 'In KeyCorners.search_key_frame_aux(), error reading image file:', key_frame_body_ext)
        # Initialize output directory.
        ft.create_directories_if_necessary(output_construction_dir)
        # Execute search.
        logt.info(local_logger, 'In KeyCorners.search_key_frame_aux(), searching for corners in key_frame_id='+str(key_frame_id)+'...')
        search_result = kfcs.KeyFrameCornerSearch(key_frame_id=key_frame_id,
        ...
        logt.info(local_logger, 'In KeyCorners.search_key_frame_aux(), corners done key_frame_id='+str(key_frame_id)+'.')
    ...
    def construct_merged_fnxl_synchronizing_heliostat_names(self, local_logger, fnxl_1, fnxl_2):
        ...
        if fnxl_1.number_of_frames() != 1:
            logt.log_and_raise_value_error(local_logger, 'In KeyCorners.construct_merged_fnxl_synchronizing_heliostat_names(), fnxl_1 does not have the expected singleton frame.  Its frames are:', fnxl_1.sorted_frame_id_list())
        if fnxl_2.number_of_frames() != 1:
            logt.log_and_raise_value_error(local_logger, 'In KeyCorners.construct_merged_fnxl_synchronizing_heliostat_names(), fnxl_2 does not have the expected singleton frame.  Its frames are:', fnxl_2.sorted_frame_id_list())
        frame_id_1 = fnxl_1.sorted_frame_id_list()[0]  # It's tempting to use frame_ids_unsorted() here, but the problem is that 
        frame_id_2 = fnxl_2.sorted_frame_id_list()[0]  # routine returns a dict_keys object, which does not support subscripts.
    ...
    def save_key_corners(self, all_key_frames_corners_fnxl, key_frame_fnxls):
        ...
        print('In KeyCorners.save_key_corners(), writing all-frame summary of found key corners:  ', self.all_key_frames_corners_dir_body_ext)
        ...
        for key_frame_fnxl in key_frame_fnxls:
            if key_frame_fnxl is not None:
                ...
                if len(frame_id_list) != 2:
                    raise ValueError('In KeyCorners.save_key_corners(), encountered key_frame_fnxl with frames != 2.  len(frame_id_list) =', len(frame_id_list))
                ...
                print('In KeyCorners.save_key_corners(), writing found frame_id key corners:              ', key_frame_corners_dir_body_ext) # Pad spaces to line up output.
    ...
    def save_data(self, all_key_frames_corners_fnxl, mismatched_key_frame_ids):
        ...
        ft.write_text_file('mismatched key frame ids', self.output_data_dir, self.mismatched_ids_body, mismatched_key_frame_ids)
        # Heliostats per key frame.
        heliostats_per_key_frame_dict = all_key_frames_corners_fnxl.heliostats_per_frame()
        print('In KeyCorners.save_data(), writing heliostats per key frame:', os.path.join(self.output_data_dir, self.hpkf_body_ext))
        ...
        print('In KeyCorners.save_data(), writing points per key frame:    ', os.path.join(self.output_data_dir, self.ppkf_body_ext))
    ...
    def read_key_corners(self):
        print('In KeyCorners.read_key_corners(), reading all-frame summary of found key corners:  ', self.all_key_frames_corners_dir_body_ext)
        ...
        self.all_key_frames_corners_fnxl.print(max_keys=7,
                                               max_value_length=200,
                                               indent=4)
        ...
        for key_frame_corners_body_ext in key_frame_corners_body_ext_list:
            ...
            print('In KeyCorners.read_key_corners(), reading found key corners file:      ', key_frame_corners_dir_body_ext)
    ...
    def render(self):
        if self.render_control.draw_key_corners and self.generated_key_corners:  # Don't render unless we generated.
            ...
            style_dict = {}
            style_dict['point_seq'] = rcps.marker(marker=self.render_control.key_corners_points_marker, 
                                                  markersize=self.render_control.key_corners_points_markersize,
                                                  color=self.render_control.key_corners_points_color)
            style_dict['text'] = rctxt.RenderControlText(horizontalalignment=self.render_control.key_corners_label_horizontalalignment,
            ...
            # Draw the frames.
            self.all_key_frames_corners_fnxl.draw_frames(self.single_processor,
...
if __name__ == "__main__":
    ...
    log_dir_body_ext              = experiment_dir() + '2020-12-03_FastScan1/3_Post/Answers/20201203/1544_NS_U/140_KeyCorners/mavic_zoom/log/KeyCorners_log.txt'
    # Input/output sources.
    input_video_dir_body_ext      = experiment_dir() + '2020-12-03_FastScan1/2_Data/20201203/1544_NS_U/mavic_zoom/DJI_427t_428_429.MP4'
    input_key_frames_dir_body_ext = experiment_dir() + '2020-12-03_FastScan1/3_Post/Answers/20201203/1544_NS_U/130_KeyFrames/mavic_zoom/data/DJI_427t_428_429_key_frames_fnxl.csv'
    ...
    output_data_dir               = experiment_dir() + '2020-12-03_FastScan1/3_Post/Answers/20201203/1544_NS_U/140_KeyCorners/mavic_zoom/data/'
    output_render_dir             = experiment_dir() + '2020-12-03_FastScan1/3_Post/Answers/20201203/1544_NS_U/140_KeyCorners/mavic_zoom/render/'
    output_construction_dir       = experiment_dir() + '2020-12-03_FastScan1/3_Post/Construction/20201203/1544_NS_U/140c_KeyCorners/mavic_zoom/'
    # Render control.
