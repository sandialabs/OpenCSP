...
def check_eta(eta):
    # Check result.
    if np.rad2deg(eta) > 0:
        print('WARNING: In construct_vanity_scan_parameters(), positive gaze angle = ' +str(np.rad2deg(eta)) + ' deg encountered.')
        print('         Positive gaze angles look up.  Continuing...')
        print('         Continuing...')
    eta_max = np.deg2rad(30.0)
    if eta > eta_max:
        print('ERROR: In construct_vanity_scan_parameters(), high positive gaze angle = ' +str(np.rad2deg(eta)) + 'encountered.')
        print('       We reject positive gaze angles exceeding ' + str(np.rad2deg(eta_max)) + ' degrees.')
        assert False
    eta_min = np.rad2deg(-90.0)
    if eta < eta_min:
        print('ERROR: In construct_vanity_scan_parameters(), excessive negative gaze angle = ' +str(np.rad2deg(eta)) + 'encountered.')
        print('       We reject gaze angles less than ' + str(np.rad2deg(eta_min)) + ' degrees.')
        assert False


def construct_vanity_scan_parameters(vanity_scan_parameter_file,
    ...
    if vanity_scan_parameter_file == 'NSTTF':
      ...
      print("ERROR: In construct_vanity_scan_parameters(), unexpected vanity_scan_parameter_file = '"  + vanity_scan_parameter_file + "' encountered.")
      assert False
