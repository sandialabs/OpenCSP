...
def plan_and_render_scan(solar_field,
                         aimpoint_xyz, 
                         when_ymdhmsz,
                         launch_name,
                         elevation_offset,
...
#         scan_short_name += '_Deta={0:.1f}'.format(np.rad2deg(ufacet_control_parameters['delta_eta']))
    ...
    if scan_type == 'UFACET':
        ...
        scan,            \
        scan_parameters, \
        ufacet_scan_construction = psu.construct_ufacet_scan(solar_field, 
                                                             aimpoint_xyz,
                                                             when_ymdhmsz,
        ...
        # Construct raster survey scan.
        scan, scan_parameters = psv.construct_vanity_scan(solar_field, 
    ...
    if (scan_type == 'UFACET'):
        if render_control_top_level.draw_ufacet_xy_analysis:
            ...
            psuxyar.draw_ufacet_xy_analysis(figure_control,
                                            solar_field,
                                            aimpoint_xyz,
...
#            psuscr.draw_construct_ufacet_sections(figure_control, solar_field, section_list, vs.view_spec_3d(), render_control_scan_section_setup)
            psuscr.draw_construct_ufacet_sections(figure_control, solar_field, section_list, vs.view_spec_xy(), render_control_scan_section_setup)
...
def scan_plan_trial(tile_array,
                    solar_field_spec,
                    aimpoint_xyz,
                    when_ymdhmsz,
                    synch_azelhnames,
    ...
    solar_field = sf.setup_solar_field(solar_field_spec,
                                       aimpoint_xyz,
                                       when_ymdhmsz,
                                       synch_azelhnames,
                                       up_azelhnames)
    ...
    plan_and_render_scan(solar_field,
                         aimpoint_xyz, 
                         when_ymdhmsz,
                         launch_name,
                         elevation_offset,
