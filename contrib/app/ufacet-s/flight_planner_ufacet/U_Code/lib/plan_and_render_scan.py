...
def plan_and_render_scan(
    ...
    if scan_type == 'UFACET':
        ...
        print(
            'ERROR: In plan_and_render_scan(), unexpected scan_type="'
            + str(scan_type)
            + '" encountered.'
        )
        assert False
    ...
    output_path = os.path.join(
        render_control_top_level.figure_output_path, scan_short_name
    )
    ...
    if scan_type == 'UFACET':
        ...
        scan, scan_parameters = psr.construct_raster_scan(
            solar_field, raster_scan_parameter_file
        )
        ...
        print(
            'ERROR: In plan_and_render_scan, unexpected scan.type = '
            + str(scan.type)
            + ' encountered.'
        )
        assert False
    ...
    flight_plan = fp.construct_flight_plan_from_scan(
        scan_name, scan_short_name, launch_name, scan
    )
    ...
    if scan_type == 'UFACET':
        if render_control_top_level.draw_ufacet_xy_analysis:
            ...
            list_of_best_fit_segment_xys = ufacet_scan_construction[
        ...
        if render_control_top_level.draw_ufacet_scan:
            ...
            psur.draw_ufacet_scan(
                figure_control, scan, render_control_scan_section_analysis
            )
    ...
    if render_control_top_level.draw_flight_plan:
        ...
        rcfosf_vfield = rcfosf.RenderControlFlightOverSolarField(
            solar_field_style=xy_solar_field_style
        )
        # Draw.
...
def scan_plan_trial(
    ...
    solar_field = sf.setup_solar_field(
