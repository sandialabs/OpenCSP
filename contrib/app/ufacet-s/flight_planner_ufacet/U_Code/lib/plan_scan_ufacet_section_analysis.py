...
def construct_minimum_safe_altitude_line(section_context, heliostat_name_list):
    ...
    corrected_first_q = g2d.homogeneous_line_y_given_x(first_p, above_all_origin_line)  # base origin.  Heliostat origin is at center 
...
def fetch_key_points(section_context,
                     h_a_name,
                     h_t_name_list,
                     h_b_name):
    # Fetch required parameters.
...
def single_heliostat_gaze_angle_analysis(section_context, ab_pq, at_pq, s_locus, e_locus, constraints):
    ...
    constraints['ray_min_eta_list'] = ray_min_eta_list  # Ray pointing from path start point to assessed heliostat top edge, function of C.
    constraints['ray_max_eta_list'] = ray_max_eta_list  # Ray pointing from path end point to assessed heliostat bottom edge, function of C.
...
def single_heliostat_gaze_angle_selection(section_context, constraints):
    ...
    if len(min_etaC_list) != len(max_etaC_list):
        print('ERROR: In, single_heliostat_gaze_angle_selection(), mismatched min_etaC and max_etaC lengths encountered.')
        assert False
    ...
    if gaze_type == 'constant':
        ...
        if selected_cacg_C > max_C:
            ...
            if len(shifted_max_etaC_list) < 2:
                print('ERROR: In single_heliostat_gaze_angle_selection(), unexpected short shifted_max_etaC_list encountered.')
                assert False
            ...
            for max_etaC in shifted_max_etaC_list:
                C = max_etaC[1]
                if not (C > prev_C):
                    print('ERROR: In single_heliostat_gaze_angle_selection(), shifted_max_etaC_list is not in order of ascending C.')
                    assert False
        ...
        print('ERROR: In single_heliostat_gaze_angle_selection(), unexpected gaze_type="'+str(gaze_type)+'" encountered.')
        assert False
...
def single_heliostat_section_analysis(section_context,
    ...
    ab_pq,     \
    t_pq_list, \
    bb_pq = fetch_key_points(section_context, h_a_name, h_t_name_list, h_b_name)
    
    # If there are no target points, createa a fictitious target to simplify downstream computation.
    if len(t_pq_list) == 0:
        if bb_pq == None:
            print('ERROR: In single_heliostat_section_analysis(), No-target, no background case not supported.  Use vanity mode.')
            assert False
        dp = at_pq[0] - bb_pq[0]
    ...
    for h_a_name in per_heliostat_constraints.keys():
        ...
        if len(pass_min_etaC_list) == 0:
            ...
            if len(pass_max_etaC_list) != len(h_a_max_etaC_list):
                ...
                assert False
            for pass_min_etaC, h_a_min_etaC, idx in zip(pass_min_etaC_list, h_a_min_etaC_list, range(0,len(pass_min_etaC_list))):
                if h_a_min_etaC[0] < pass_min_etaC[0]:
                    pass_min_etaC_list[idx] = h_a_min_etaC.copy()
            for pass_max_etaC, h_a_max_etaC, idx in zip(pass_max_etaC_list, h_a_max_etaC_list, range(0,len(pass_max_etaC_list))):
...
def multi_heliostat_gaze_angle_selection(section_context, pass_constraints):
    ...
    if len(pass_min_etaC_list) != len(pass_max_etaC_list):
        print('ERROR: In, multi_heliostat_gaze_angle_selection(), mismatched min_etaC and max_etaC lengths encountered.')
        assert False
    ...
    if gaze_type == 'constant':
        ...
        if selected_cacg_C > max_C:
            ...
            # Pick altitude, then lookup eta that ensures mirror will be seen intially facing all sky.
            if len(shifted_max_etaC_list) < 2:
                print('ERROR: In multi_heliostat_gaze_angle_selection(), unexpected short shifted_max_etaC_list encountered.')
                assert False
            selected_cacg_C = max_C  # m.
            if shifted_max_etaC_list[0][1] > selected_cacg_C:
        ...
        print('ERROR: In multi_heliostat_gaze_angle_selection(), unexpected gaze_type="'+str(gaze_type)+'" encountered.')
        assert False
...
def multi_heliostat_construct_flight_path(section_context, assess_heliostat_name_list, pass_constraints):
    ...
    selected_cacg_start_pq = g2d.intersect_lines(selected_cacg_line, g2d.homogeneous_line(first_s_locus[0], first_s_locus[1]))
    ...
    selected_cacg_end_pq   = g2d.intersect_lines(selected_cacg_line, g2d.homogeneous_line(last_e_locus[0], last_e_locus[1]))
...
def multi_heliostat_vertical_fov_analysis(pass_constraints):
    ...
    # Check input.
    if len(pass_min_etaC_list) != len(pass_max_etaC_list):
        print('ERROR: In, multi_heliostat_vertical_fov_analysis(), mismatched min_etaC and max_etaC lengths encountered.')
        assert False
...
def section_analysis(section_context,
                     heliostat_name_list,
                     assess_heliostat_name_list):
    # Notify progress.
    ...
    for assess_heliostat_name in assess_heliostat_name_list:
        constraints = single_heliostat_section_analysis(section_context,
                                                        heliostat_name_list,
                                                        assess_heliostat_name)
    ...
    pass_constraints = multi_heliostat_construct_flight_path(section_context, assess_heliostat_name_list, pass_constraints)
