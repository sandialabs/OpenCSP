...
def construct_minimum_safe_altitude_line(section_context, heliostat_name_list):
    ...
    for heliostat_name in heliostat_name_list:
        ...
        signed_distance = g2d.homogeneous_line_signed_distance_to_xy(
            heliostat_pq, origin_line
        )
...
def construct_flight_path_family(section_context):
    ...
    A = -above_all_origin_line[
        0
    ]  # Flip orientation, so increasing C implies increasing altitude.
    ...
    C_mxsa = (
        maximum_safe_altitude  # m.  Inexact, because this doesn't account for slope.
    )
    C_min = C_mnsa  # m.
    C_max = C_mxsa  # m.  (C_max - C_min) is arbitrary, and simply controls range covereed in computation.
    C_step = (
        1.0  # m   C_step is arbitrary, and controls altitude resolution of analysis.
    )
...
def construct_reflected_point(mirror_pq, nu, target_pq):
    ...
    theta = math.atan2(
        dq_mirror_to_target, dp_mirror_to_target
    )  # Angle ccw from p axis to ray [mirror --> target].
...
def single_heliostat_gaze_angle_analysis(
    section_context, ab_pq, at_pq, s_locus, e_locus, constraints
):
    ...
    while C <= C_max:
        path = [A, B, C]
        path_s_pq = g2d.intersect_lines(
            path, g2d.homogeneous_line(s_locus[0], s_locus[1])
        )
        path_e_pq = g2d.intersect_lines(
            path, g2d.homogeneous_line(e_locus[0], e_locus[1])
        )
    ...
    constraints['path_s_pq_list'] = (
        path_s_pq_list  # Path sacn region start point, as a function of C.
    )
    constraints['path_e_pq_list'] = (
        path_e_pq_list  # Path sacn region start point, as a function of C.
    )
    ...
    constraints['min_etaC_list'] = (
        min_etaC_list  # Lower bound on required gaze angle interval, as a function of C.
    )
    constraints['max_etaC_list'] = (
        max_etaC_list  # Upper bound on required gaze angle interval, as a function of C.
    )
...
def single_heliostat_gaze_angle_selection(section_context, constraints):
    ...
    for shifted_min_etaC, shifted_max_etaC in zip(
        shifted_min_etaC_list, shifted_max_etaC_list
    ):
        if shifted_min_etaC[0] >= shifted_max_etaC[0]:
            envelope_min_etaC_list.append(
                shifted_max_etaC
            )  # Lines crossed, so we swap min.max sense.
    ...
    for envelope_min_etaC, envelope_max_etaC in zip(
        envelope_min_etaC_list, envelope_max_etaC_list
    ):
    ...
    for envelope_min_etaC, envelope_max_etaC in zip(
        shrunk_min_etaC_list, shrunk_max_etaC_list
    ):
        C = envelope_min_etaC[1]
        envelope_min_eta = envelope_min_etaC[0]
        envelope_max_eta = envelope_max_etaC[0]
    ...
    if gaze_type == 'constant':
        ...
        if len(clipped_min_etaC_list) > 0:
            ...
            print(
                'WARNING: In single_heliostat_gaze_angle_selection(), infeasible case encountered.'
            )
            print(
                '               len(envelope_min_etaC_list) = ',
                len(envelope_min_etaC_list),
            )
            print(
                '               len(shrunk_min_etaC_list)   = ',
                len(shrunk_min_etaC_list),
            )
            print(
                '               len(clipped_min_etaC_list)  = ',
                len(clipped_min_etaC_list),
            )
        ...
        if selected_cacg_C > max_C:
            print(
                'NOTE: In single_heliostat_gaze_angle_selection(), clipping to max altitude.'
            )
            ...
            if shifted_max_etaC_list[0][1] > selected_cacg_C:
                print(
                    'ERROR: In single_heliostat_gaze_angle_selection(), shifted_max_etaC_list starts above C.'
                )
                assert False
            ...
            if selected_cacg_eta == math.inf:
                print(
                    'ERROR: In single_heliostat_gaze_angle_selection(), unexpected selected_cacg_eta == math.inf.'
                )
                assert False
            if prev_C > selected_cacg_C:
                print(
                    'ERROR: In single_heliostat_gaze_angle_selection(), unexpected prev_C > selected_cacg_C.'
                )
                assert False
            if C < selected_cacg_C:
                print(
                    'ERROR: In single_heliostat_gaze_angle_selection(), unexpected C < selected_cacg_C.'
                )
                assert False
            # Shrink.
    ...
    constraints['selected_cacg_etaC'] = (
        selected_cacg_etaC  # "cacg" == "constant altitude, constant gaze"
    )
...
def single_heliostat_section_analysis(
    ...
    if h_a_idx < (n_names - 1):
        ...
        h_b_name = None
    at_pq, ab_pq, t_pq_list, bb_pq = fetch_key_points(
        section_context, h_a_name, h_t_name_list, h_b_name
    )
    ...
    if len(t_pq_list) == 0:
        ...
        fictitous_t_pq = [
            (ab_pq[0] + dp),
            at_pq[1],
        ]  # Imitate [assessed --> background] relative postion.
    ...
    tau = math.atan2(
        dq_abat, dp_abat
    )  # Angle from p axis to assessed heliostat tangent.
    ...
    ts_ub_list = [
        [ab_pq, construct_reflected_point(ab_pq, nu, t_pq)] for t_pq in t_pq_list
    ]
    if len(ts_ub_list) >= 1:
    ...
    if tsm_ub:
        ...
        length = (
            10  # Arbitrary positive number.  Unimportant because it will be extended.
        )
        ...
        print(
            'ERROR: In single_heliostat_section_analysis(), No-target start locus case not supported.\n'
        )
        assert False
    ...
    te_lb_list = [
        [at_pq, construct_reflected_point(at_pq, nu, t_pq)] for t_pq in t_pq_list
    ]
    if len(te_lb_list) >= 1:
    ...
    if te_lb:
        ...
        print(
            'ERROR: In single_heliostat_section_analysis(), No-target end critical altitude case not supported (1).'
        )
        assert False
    ...
    if te_lb:
        ...
        print(
            'ERROR: In single_heliostat_section_analysis(), No-target, end critical altitude case not supported (2).'
        )
        assert False
    ...
    constraints = {}
    constraints['h_a_idx'] = (
        h_a_idx  # Assessed heliostat index in assess_heliostat_name_list.
    )
    constraints['h_a_name'] = h_a_name  # Assessed heliostat name.
    constraints['h_t_name_list'] = (
        h_t_name_list  # Reflected target heliostat name list.
    )
    ...
    constraints['t_pq_list'] = (
        t_pq_list  # List of reflection target points.  Might include a fictitious point.
    )
    constraints['bb_pq'] = bb_pq  # Bottom corner of background heliostat.
    constraints['nu'] = (
        nu  # Angle from p axis to assessed heliostat surface normal, measured ccw.
    )
    constraints['abv_lb'] = abv_lb  # Assessed bottom visibility, p lower bound.
    constraints['abvm_lb'] = (
        abvm_lb  # Assessed bottom visibility margin, p lower bound.
    )
    ...
    constraints['ts_ub_list'] = (
        ts_ub_list  # Target reflection start list, p upper bound.
    )
    ...
    constraints['C_critical'] = (
        C_critical  # Critical altitude, considering both start and end.
    )

    # GAZE ANGLE ANALYSIS
    constraints = single_heliostat_gaze_angle_analysis(
        section_context, ab_pq, at_pq, s_locus, e_locus, constraints
    )
    ...
    for h_a_name in per_heliostat_constraints.keys():
        ...
        if len(pass_min_etaC_list) == 0:
            ...
            if len(pass_min_etaC_list) != len(h_a_min_etaC_list):
                print(
                    'ERROR: In, multi_heliostat_gaze_angle_analysis(), mismatched min_etaC lengths encountered.'
                )
                assert False
            if len(pass_max_etaC_list) != len(h_a_max_etaC_list):
                print(
                    'ERROR: In, multi_heliostat_gaze_angle_analysis(), mismatched max_etaC lengths encountered.'
                )
                assert False
...
def multi_heliostat_gaze_angle_selection(section_context, pass_constraints):
    ...
    if gaze_type == 'constant':
        ...
        if len(clipped_min_etaC_list) > 0:
            ...
            print(
                'WARNING: In multi_heliostat_gaze_angle_selection(), infeasible case encountered.'
            )
            print(
                '               len(envelope_min_etaC_list) = ',
                len(envelope_min_etaC_list),
        ...
        if selected_cacg_C > max_C:
            print(
                'NOTE: In multi_heliostat_gaze_angle_selection(), clipping to max altitude.'
            )
            # Pick altitude, then lookup eta that ensures mirror will be seen intially facing all sky.
            if len(shifted_max_etaC_list) < 2:
                print(
            ...
            if shifted_max_etaC_list[0][1] > selected_cacg_C:
                print(
                    'ERROR: In multi_heliostat_gaze_angle_selection(), shifted_max_etaC_list starts above C.'
                )
                assert False
            # Search for point on eta = f(C) curve.
            selected_cacg_eta = math.inf
            ...
            if selected_cacg_eta == math.inf:
                print(
                    'ERROR: In multi_heliostat_gaze_angle_selection(), unexpected selected_cacg_eta == math.inf.'
                )
                assert False
            if prev_C > selected_cacg_C:
                print(
                    'ERROR: In multi_heliostat_gaze_angle_selection(), unexpected prev_C > selected_cacg_C.'
                )
                assert False
            if C < selected_cacg_C:
                print(
                    'ERROR: In multi_heliostat_gaze_angle_selection(), unexpected C < selected_cacg_C.'
                )
                assert False
            # Shrink.
            # Recall max and min swapped.
    ...
    pass_constraints['selected_cacg_etaC'] = (
        selected_cacg_etaC  # "cacg" == "constant altitude, constant gaze"
    )
...
def assemble_single_heliostat_etaC_dict(pass_constraints):
    ...
    for h_a_name in per_heliostat_constraints.keys():
        h_a_constraints = per_heliostat_constraints[h_a_name]
        selected_cacg_etaC = h_a_constraints[
            'selected_cacg_etaC'
        ]  # "cacg" == "constant altitude, constant gaze"
...
def section_analysis(section_context, heliostat_name_list, assess_heliostat_name_list):
    # Notify progress.
    print(
        'Constructing UFACET section '
        + heliostat_name_list[0]
        + '-'
        + heliostat_name_list[-1]
        + ' analysis...'
    )

    # Construct minimum safe altitude line for this section.
    section_context = construct_minimum_safe_altitude_line(
    ...
    for assess_heliostat_name in assess_heliostat_name_list:
        constraints = single_heliostat_section_analysis(
            section_context, heliostat_name_list, assess_heliostat_name
        )
    ...
    pass_constraints = multi_heliostat_gaze_angle_selection(
        section_context, pass_constraints
    )
