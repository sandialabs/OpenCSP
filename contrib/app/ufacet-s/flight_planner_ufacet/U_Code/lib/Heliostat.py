...
class Heliostat:
    ...
    def __init__(
        ...
        self.bottom_left_facet = self.facets[
    ...
    def compute_tracking_configuration(
        self, aimpoint_xyz, location_lon_lat, when_ymdhmsz
    ):
        ...
        n_xyz = sun_track.tracking_surface_normal_xyz(
    ...
    def set_configuration(self, h_config, clear_tracking=True):
        ...
        surface_normal = hel_rotation.dot(
            [0, 0, 1]
    ...
    def set_corner_positions_in_space(self):
        ...
        self.top_left_corner = hel_centroid + hel_rotation.dot(
            np.array(self.top_left_corner_offset)
        )
        self.top_right_corner = hel_centroid + hel_rotation.dot(
            np.array(self.top_right_corner_offset)
        )
        self.bottom_right_corner = hel_centroid + hel_rotation.dot(
            np.array(self.bottom_right_corner_offset)
        )
        self.bottom_left_corner = hel_centroid + hel_rotation.dot(
            np.array(self.bottom_left_corner_offset)
        )
    ...
    def draw(self, view, heliostat_styles):
        ...
        if heliostat_style.draw_facets:
            ...
            for facet in self.facets:
                facet.draw(
                    view, heliostat_style.facet_styles, hel_centroid, hel_rotation
                )
        ...
        if heliostat_style.draw_surface_normal:
            ...
            surface_normal_ray = self.surface_normal_ray(
                self.centroid, heliostat_style.surface_normal_length
            )
            # Draw ray and its base.
            view.draw_xyz(
                self.centroid, style=heliostat_style.surface_normal_base_style
            )
            view.draw_xyz_list(
        ...
        if heliostat_style.draw_name:
            view.draw_xyz_text(
                self.centroid, self.name, style=heliostat_style.name_style
            )
