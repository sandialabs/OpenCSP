...
def construct_ufacet_scan_parameters(
    ...
    if ufacet_scan_parameter_file == 'NSTTF':
        ...
        for key in ufacet_control_parameters.keys():
            if key in scan_parameters.keys():
                print(
                    'ERROR: In construct_ufacet_scan_parameters(1), duplicate key="'
                    + str(key)
                    + '" encountered.'
                )
                assert False
        ...
        if (
            scan_parameters['maximum_altitude']
            > scan_parameters['maximum_safe_altitude']
        ):
            print(
            ...
            scan_parameters['maximum_altitude'] = scan_parameters[
