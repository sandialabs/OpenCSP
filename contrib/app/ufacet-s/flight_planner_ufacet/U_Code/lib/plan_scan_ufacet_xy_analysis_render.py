...
def draw_ufacet_xy_analysis(
    ...
    if render_control_scan_xy_analysis.draw_xy_segment_analysis:
        ...
        view_xy = sf.draw_solar_field(
            figure_control, solar_field, rcsf_vfield, vs.view_spec_xy()
        )
        # Dense vector field.
        ...
        grid_xydxy = [
            [
                ...
                sun_track.tracking_surface_normal_xy(
                    p + [0], aimpoint_xyz, field_origin_lon_lat, when_ymdhmsz
                ),
            ]
            for p in grid_xy
        ]
        view_xy.draw_pqdpq_list(
            grid_xydxy, style=rcps.vector_field(color='k', vector_scale=5.0)
        )
        # Key points.
        view_xy.draw_pq_list(
            curve_key_xy_list, style=rcps.marker(markersize=5, color='r')
        )
        # Ideal gaze curves.
