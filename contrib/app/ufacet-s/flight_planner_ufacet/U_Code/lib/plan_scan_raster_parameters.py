...
def check_eta(eta):
    # Check result.
    if np.rad2deg(eta) > 0:
        print('WARNING: In construct_raster_scan_parameters(), positive gaze angle = ' +str(np.rad2deg(eta)) + ' deg encountered.')
        print('         Positive gaze angles look up.  Continuing...')
        print('         Continuing...')
    eta_max = np.deg2rad(30.0)
    if eta > eta_max:
        print('ERROR: In construct_raster_scan_parameters(), high positive gaze angle = ' +str(np.rad2deg(eta)) + 'encountered.')
        print('       We reject positive gaze angles exceeding ' + str(np.rad2deg(eta_max)) + ' degrees.')
        assert False
    eta_min = np.rad2deg(-90.0)
    if eta < eta_min:
        print('ERROR: In construct_raster_scan_parameters(), excessive negative gaze angle = ' +str(np.rad2deg(eta)) + 'encountered.')
...
def construct_raster_scan_parameters(raster_scan_parameter_file):
    ...
    if raster_scan_parameter_file == 'NSTTF':
      ...
      print("ERROR: In construct_raster_scan_parameters(), unexpected raster_scan_parameter_file = '"  + raster_scan_parameter_file + "' encountered.")
      assert False
