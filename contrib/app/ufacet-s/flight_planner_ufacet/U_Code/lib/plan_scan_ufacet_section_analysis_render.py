...
def draw_key_points(view, at_pq, ab_pq, t_pq_list, bb_pq):
    ...
    view.draw_pq_text(at_pq, 'at', style=rct.RenderControlText(color=color_at, horizontalalignment='left', verticalalignment='top'))
    view.draw_pq_text(ab_pq, 'ab', style=rct.RenderControlText(color=color_ab, horizontalalignment='left', verticalalignment='top'))

    t_idx = 1
    for t_pq in t_pq_list:
        view.draw_pq(t_pq, style=rcps.marker(color=color_t), label='t{0:d}: Target {0:d} top'.format(t_idx))
        view.draw_pq_text(t_pq, 
                          't{0:d}'.format(t_idx),
                          style=rct.RenderControlText(color=color_t, horizontalalignment='left', verticalalignment='top'))
        t_idx += 1

    if bb_pq:
        view.draw_pq_text(bb_pq, 'bb', style=rct.RenderControlText(color=color_bb, horizontalalignment='left', verticalalignment='top'))
...
def draw_sca_point(view, sca_pq, color):
    ...
    view.draw_pq_text(sca_pq, 'sca', style=rct.RenderControlText(color=color, horizontalalignment='left', verticalalignment='top'))


def draw_eca_point(view, eca_pq, color):
    view.draw_pq(eca_pq, style=rcps.marker(color=color), label='eca: End critical altitude')
    view.draw_pq_text(eca_pq, 'eca', style=rct.RenderControlText(color=color, horizontalalignment='left', verticalalignment='top'))
...
def draw_heliostat_section(figure_control,
                           section_context,
                           heliostat_name_list,
                           analysis_render_control):
    # Setup view.
    fig_record = fm.setup_figure_for_3d_data(figure_control, rca.meters(), section_context['view_spec'], title='N-S Pass Section')
    ...
    if analysis_render_control.draw_context_mnsa_ray:
        view.draw_pq_list(section_context['mnsa_ray'], style=rcps.RenderControlPointSeq(linestyle='--', color='brown', marker=None))
    if analysis_render_control.draw_context_mxsa_ray:
        view.draw_pq_list(section_context['mxsa_ray'], style=rcps.RenderControlPointSeq(linestyle='--', color='r', marker=None))
...
def draw_single_heliostat_constraint_analysis(figure_control,
                                              section_context,
                                              heliostat_name_list,
                                              ...
                                              analysis_render_control):
    # Setup view.
    fig_record = fm.setup_figure_for_3d_data(figure_control, 
                                             rca.meters(), 
                                             section_context['view_spec'], 
                                             title=(assess_heliostat_name+' in '+heliostat_name_list[0]+'-'+heliostat_name_list[-1]+' Pass Section'))
    view = fig_record.view
    ...
    if analysis_render_control.draw_single_heliostat_constraints_detail:
        ...
        if abv_lb:
            draw_constraint_lower_bound(section_context, view, abv_lb,  color='r',
                                        label='Assessed bottom visibility, p lower bound')
        if abvm_lb:
            draw_constraint_lower_bound(section_context, view, abvm_lb,  color='pink', 
                                        label='Assessed bottom visibility, p lower bound')
        if atv_lb:
            draw_constraint_lower_bound(section_context, view, atv_lb,  color='b',
                                        label='Assessed top visibility, p lower bound')
        if atvm_lb:
            draw_constraint_lower_bound(section_context, view, atvm_lb,  color='skyblue', 
                                        label='Top visibility margin, p lower bound')
        if len(ts_ub_list) > 0:
            # The first target dominates.
            draw_constraint_upper_bound(section_context, view, ts_ub,  color='g', 
                                        label='Target reflection start, p upper bound')
            if len(ts_ub_list) > 1:
                if analysis_render_control.draw_single_heliostat_constraints_all_targets:
                    for ts_ub2 in ts_ub_list[1:]:  # Differentiate from ts_ub.
                        draw_constraint_upper_bound(section_context, view, ts_ub2,  color='g', 
                                                    label='Target reflection start, p upper bound')
        if tsm_ub:
            draw_constraint_upper_bound(section_context, view, tsm_ub, color='c', 
                                        label='Target reflection margin, p upper bound')
        if len(te_lb_list) > 0:
            # The first target dominates.
            draw_constraint_lower_bound(section_context, view, te_lb,  color='g', 
                                        label='Target reflection end, p lower bound')
            if len(te_lb_list) > 1:
                if analysis_render_control.draw_single_heliostat_constraints_all_targets:
                    for te_lb2 in te_lb_list[1:]:  # Differentiate from te_lb.
                        # These are nearly superimposed.
                        draw_constraint_lower_bound(section_context, view, te_lb2,  color='g', 
                                                    label='Target reflection end, p lower bound')
        if tem_lb:
            draw_constraint_upper_bound(section_context, view, tem_lb, color='olive', 
                                        label='Target reflection margin, p upper bound')
        draw_constraint_upper_bound(section_context, view, pln_ub,  color='c', 
                                    label='Mirror plane, p upper bound') # Reflection end dominates.
    ...
    if analysis_render_control.draw_single_heliostat_constraints_gaze_example:
        ...
        for path_s_pq, path_e_pq, ray_min_eta, ray_max_eta, min_etaC, max_etaC in zip(constraints['path_s_pq_list'],
            ...
            if min_etaC[1] > C_example:  # Inexact match.
                view.draw_pq_list([path_s_pq, path_e_pq], style=rcps.RenderControlPointSeq(linestyle='--', color='g', marker=None))
...
def draw_single_heliostat_gaze_angle_analysis(figure_control,
                                              section_context, 
                                              h_a_name, 
                                              constraints, 
                                              analysis_render_control):
    ...
    # Setup figure.
    fig_record = fm.setup_figure_for_3d_data(figure_control, 
                                             rca.RenderControlAxis(x_label='eta (deg)',
                                                                   y_label='C (m)',
                                                                   z_label='',
                                                                   grid=True), 
                                             vs.view_spec_xy(),
    ...
    if analysis_render_control.draw_single_heliostat_gaze_angle_mxsa:
        view.draw_pq_list(a.p2deg([[draw_eta_min, C_mxsa], [draw_eta_max, C_mxsa]]),
                          style=rcps.RenderControlPointSeq(linestyle='--', color='r', marker=None), label='Maximum Safe')
    if analysis_render_control.draw_single_heliostat_gaze_angle_mnsa:
        view.draw_pq_list(a.p2deg([[draw_eta_min, C_mnsa], [draw_eta_max, C_mnsa]]),
                          style=rcps.RenderControlPointSeq(linestyle='--', color='brown', marker=None), label='Minimum Safe')

    # Critical altitude.
    if analysis_render_control.draw_single_heliostat_gaze_angle_critical:
        C_critical = constraints['C_critical']
        view.draw_pq_list(a.p2deg([[draw_eta_min, C_critical], [draw_eta_max, C_critical]]),
                          style=rcps.RenderControlPointSeq(linestyle='--', color='orange', marker=None), label='Critical')
    ...
    if analysis_render_control.draw_single_heliostat_gaze_angle_example:
        ...
        for min_etaC, max_etaC in zip(min_etaC_list, max_etaC_list):
            if min_etaC[1] > C_example:  # Inexact match.
                view.draw_pq_list(a.p2deg([min_etaC, max_etaC]),
...
def draw_single_heliostat_select_gaze(figure_control,
                                      section_context, 
                                      h_a_name,
                                      constraints, 
                                      analysis_render_control):
    ...
    if analysis_render_control.draw_single_heliostat_select_gaze_shifted:
        ...
        view.draw_pq_list(a.p2deg(shifted_min_etaC_list),
                          style=rcps.data_curve(color='b', linewidth=0.5),
                          label='Shifted Pass eta_min')
        view.draw_pq_list(a.p2deg(shifted_max_etaC_list),
    ...
    if analysis_render_control.draw_single_heliostat_select_gaze_envelope:
        ...
        view.draw_pq_list(a.p2deg(envelope_min_etaC_list),
                          style=rcps.data_curve(color='b'),
                          label='Gaze Envelope eta_min')
        view.draw_pq_list(a.p2deg(envelope_max_etaC_list),
                          style=rcps.data_curve(color='r'),
                          label='Gaze Envelope eta_max')
    ...
    if analysis_render_control.draw_single_heliostat_select_gaze_shrunk:
        ...
        view.draw_pq_list(a.p2deg(shrunk_min_etaC_list),
                          style=rcps.data_curve(color='b', linewidth=1.5),
                          label='Shrunk eta_min')
        view.draw_pq_list(a.p2deg(shrunk_max_etaC_list),
                          style=rcps.data_curve(color='r', linewidth=1.5),
                          label='Shrunk eta_max')
    ...
    if analysis_render_control.draw_single_heliostat_select_gaze_clipped:
        ...
        view.draw_pq_list(a.p2deg(clipped_min_etaC_list),
                          style=rcps.data_curve(color='b', linewidth=1.5),
                          label='clipped eta_min')
        view.draw_pq_list(a.p2deg(clipped_max_etaC_list),
                          style=rcps.data_curve(color='r', linewidth=1.5),
                          label='clipped eta_max')
    ...
    if analysis_render_control.draw_single_heliostat_select_gaze_selected:
        ...
        view.draw_pq(a.p2deg(selected_cacg_etaC),
...
def draw_multi_heliostat_gaze_angle_analysis(figure_control,
                                             section_context, 
                                             pass_constraints, 
                                             analysis_render_control):
    # Setup figure.
    ...
    if analysis_render_control.draw_multi_heliostat_gaze_angle_per_heliostat:
        ...
        for assess_heliostat_name in per_heliostat_constraints.keys():
            ...
            min_etaC_list = constraints['min_etaC_list']
            view.draw_pq_list(a.p2deg(min_etaC_list), 
        ...
        for assess_heliostat_name in per_heliostat_constraints.keys():
            constraints = per_heliostat_constraints[assess_heliostat_name]
            max_etaC_list = constraints['max_etaC_list']
            view.draw_pq_list(a.p2deg(max_etaC_list), 
                              style=rcps.outline(color=c.color(color_idx), linewidth=0.5), 
    ...
    if analysis_render_control.draw_multi_heliostat_gaze_angle_envelope:
        ...
        view.draw_pq_list(a.p2deg(pass_min_etaC_list),
                          style=rcps.data_curve(color='b'),
                          label='Pass eta_min')
        view.draw_pq_list(a.p2deg(pass_max_etaC_list),
                          style=rcps.data_curve(color='r'),
                          label='Pass eta_max')
...
def draw_required_vertical_field_of_view(figure_control,
                                         section_context, 
                                         pass_constraints,
                                         analysis_render_control):
    fig_record = fm.setup_figure_for_3d_data(figure_control, 
                                             rca.RenderControlAxis(x_label='Minimum Required Vertical FOV (deg)',
                                                                   y_label='C (m)',
                                                                   z_label='',
                                                                   grid=True), 
                                             vs.view_spec_xy(),
    ...
    view.draw_pq_list(a.p2deg(vertical_fovC_list), 
                      style=rcps.RenderControlPointSeq(linestyle='-', color='b', marker='.'), label='From constraints')
    ...
    if analysis_render_control.draw_multi_heliostat_vertical_fov_required_mxsa:
        C_mxsa = section_context['path_family_C_mxsa']
        view.draw_pq_list(a.p2deg([[draw_fov_min, C_mxsa], [draw_fov_max, C_mxsa]]),
                          style=rcps.RenderControlPointSeq(linestyle='--', color='r', marker=None), label='Maximum Safe')
    if analysis_render_control.draw_multi_heliostat_vertical_fov_required_mnsa:
        C_mnsa = section_context['path_family_C_mnsa']
        view.draw_pq_list(a.p2deg([[draw_fov_min, C_mnsa], [draw_fov_max, C_mnsa]]),
                          style=rcps.RenderControlPointSeq(linestyle='--', color='brown', marker=None), label='Minimum Safe')
    ...
    if analysis_render_control.draw_multi_heliostat_vertical_fov_required_critical:
        ...
        for assess_heliostat_name in per_heliostat_constraints.keys():
            constraints = per_heliostat_constraints[assess_heliostat_name]
            C_critical = constraints['C_critical']
            view.draw_pq_list(a.p2deg([[draw_fov_min, C_critical], [draw_fov_max, C_critical]]),
                              style=rcps.RenderControlPointSeq(linestyle='--', color=c.color(color_idx), linewidth=0.5, marker=None),
                              label='{0:s} C_critical'.format(assess_heliostat_name))
            color_idx += 1
    ...
    if analysis_render_control.draw_multi_heliostat_vertical_fov_required_camera:
        ...
        if vertical_fov_min == vertical_fov_max:
            view.draw_pq_list(a.p2deg([[vertical_fov_min, draw_C_min], [vertical_fov_min, draw_C_max]]),
            ...
            view.draw_pq_list(a.p2deg([[vertical_fov_max, draw_C_min], [vertical_fov_max, draw_C_max]]),
                            style=rcps.RenderControlPointSeq(linestyle='--', color='g', linewidth=1.5, marker=None), 
...
def draw_multi_heliostat_result(figure_control,
                                section_context,
                                heliostat_name_list,
                                pass_constraints,
                                analysis_render_control):
    ...
    if analysis_render_control.draw_multi_heliostat_result_selected_cacg_line:
        ...
        view.draw_pq_list(selected_cacg_line_ray, style=rcps.RenderControlPointSeq(linestyle='--', color='g', marker=None))
...
def draw_single_heliostat_etaC_dict(figure_control,
                                    pass_constraints):
...
def draw_section_analysis(figure_control,
                          section_context,
                          heliostat_name_list,
                          pass_constraints,
                          analysis_render_control):
        ...
        for assess_heliostat_name in per_heliostat_constraints.keys():
            if ( (analysis_render_control.draw_single_heliostat_analysis_list == None) or 
                ...
                if analysis_render_control.draw_single_heliostat_constraints:
                    draw_single_heliostat_constraint_analysis(figure_control,
                                                              section_context,
                                                              heliostat_name_list,
                                                              assess_heliostat_name,
                                                              constraints,
                ...
                if analysis_render_control.draw_single_heliostat_gaze_angle:
                    draw_single_heliostat_gaze_angle_analysis(figure_control,
                                                              section_context, 
                                                              assess_heliostat_name, 
                                                              constraints, 
                                                              analysis_render_control)
                ...
                if analysis_render_control.draw_single_heliostat_select_gaze:
                    draw_single_heliostat_select_gaze(figure_control,
                                                      section_context, 
                                                      assess_heliostat_name, 
                                                      constraints, 
                                                      analysis_render_control)
    ...
    if analysis_render_control.draw_multi_heliostat_gaze_angle:
        draw_multi_heliostat_gaze_angle_analysis(figure_control,
                                                 section_context, 
                                                 pass_constraints, 
                                                 analysis_render_control)
    ...
    if analysis_render_control.draw_multi_heliostat_vertical_fov_required:
        draw_required_vertical_field_of_view(figure_control,
                                             section_context, 
                                             pass_constraints, 
                                             analysis_render_control)
    ...
    if analysis_render_control.draw_multi_heliostat_result:
        draw_multi_heliostat_result(figure_control,
                                    section_context,
                                    heliostat_name_list,
                                    pass_constraints,
                                    analysis_render_control)
    ...
    if analysis_render_control.draw_single_heliostat_etaC_dict:
        draw_single_heliostat_etaC_dict(figure_control,
                                        pass_constraints)
