...
def construct_ufacet_scan_pass(solar_field, lead_in, run_past):
    # Control parameters.
    scan_parameters = {}
    scan_parameters['locale'] = 'Sandia NSTTF' # Information needed to convert (x,y,z into global (longitude, latitude) coordinates.
    ...
    scan_parameters['maximum_safe_altitude'] = 90.0  # meters.  # ?? SCAFFOLDING -- BASE THIS ON TECHNICAL FACTORS:  SOLAR FLUX, ETC
    scan_parameters['maximum_target_lookback'] = 3  # Number of heliostats to look back for reflection targets.
    scan_parameters['gaze_tolerance'] = np.deg2rad(1)  # Uncertainty in gaze angle.  True angle is +/- tolerance from nominal.
...
# # ufacet_pass_E04 = usp.UfacetScanPass(solar_field, segment_xy_E04, scan_parameters)  # ?? SCAFFOLDING RCB -- CRASHES
    ...
    # Construct the scan.
    scan = Scan.construct_scan_given_UFACET_scan_passes(ufacet_scan_pass_list, 
                                                        lead_in,
                                                        run_past)
    # Return.
    return scan
...
#    figure_control = rcfg.RenderControlFigure(tile_array=(2,1), tile_square=False)
    ...
    solar_field = sf.SolarField(origin_lon_lat=field_origin_lon_lat,
...
#    curve_keys_x = [92.7] # ?? SCAFFOLDING RCB -- TEMPORARY
    ...
    list_of_best_fit_segment_xys = pusxya.ufacet_xy_analysis(solar_field, aimpoint_xyz, when_ymdhmsz, curve_key_xy_list)

    # Draw UFACET (x,y) analysis.
    render_control_scan_xy_analysis = setup_render_control_scan_xy_analysis()
    pusxyar.draw_ufacet_xy_analysis(figure_control,
                                    solar_field,
                                    aimpoint_xyz,
    ...
    section_list = pussc.construct_ufacet_sections(solar_field, 
                                                   list_of_best_fit_segment_xys,
                                                   candidate_margin_w,
                                                   discard_threshold_p)
    ...
    pusscr.draw_construct_ufacet_sections(figure_control, solar_field, section_list, vs.view_spec_3d(), render_control_scan_section_setup)
    pusscr.draw_construct_ufacet_sections(figure_control, solar_field, section_list, vs.view_spec_xy(), render_control_scan_section_setup)
    pusscr.draw_construct_ufacet_sections(figure_control, solar_field, section_list, None, render_control_scan_section_setup)  # Use section view.
