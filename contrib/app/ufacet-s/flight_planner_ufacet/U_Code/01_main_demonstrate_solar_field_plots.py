...
def draw_demonstration_figures(figure_control:rcfg.RenderControlFigure,
    ...
    if draw_single_heliostat:
        ...
        heliostat_name = '5E10'
        # View setup
        fig_record = fm.setup_figure_for_3d_data(figure_control, axis_control_m, view_spec, title=('Heliostat ' + heliostat_name))
    ...
    if draw_solar_field_h_centroids:
        ...
        # Comment
        fig_record.comment.append("Heliostat centroids, which at NSTTF appear to be at the midpoint of the torque tube.")
        # Draw
        solar_field.draw(view, solar_field_style)
        view.show()
    ...
    if draw_solar_field_h_centroids_names:
        # View setup
        fig_record = fm.setup_figure_for_3d_data(figure_control, axis_control_m, view_spec, title='Heliostat Labelled Centroids')
        view = fig_record.view
        # Tracking setup
        # Not required since we're not drawing heliostat shape.
        # Style setup
    ...
    if draw_heliostat_vector_field:
        # View setup
        fig_record = fm.setup_figure_for_3d_data(figure_control, axis_control_m, view_spec, title='Heliostat Vector Field')
        view = fig_record.view
        # Tracking setup
        solar_field.set_full_field_tracking(aimpoint_xyz=aimpoint_xyz, when_ymdhmsz=when_ymdhmsz)
        # Style setup
    ...
    if draw_dense_vector_field:
        # View setup (xy only)
        fig_record = fm.setup_figure_for_3d_data(figure_control, axis_control_m, vs.view_spec_xy(), title='Dense Tracking Vector Field')
        ...
        grid_xydxy = [[p, sun_track.tracking_surface_normal_xy(p+[0], aimpoint_xyz, solar_field.origin_lon_lat, when_ymdhmsz)] for p in grid_xy]
